...
- `src/nanobind/` contains the active Python bindings built with nanobind.
- `src/pybind/` contains the former pybind11 bindings. It is retained for
  reference during the transition, but new Python functionality should be added
  to nanobind unless there is an explicit reason to touch pybind.
...
- Prefer Bazel for builds. Portable example from `src`:

  ```shell
  bazel build -c opt //Molecule_Tools:common_names
  ```

- Local CPU-specific builds can use `--config=native`:

  ```shell
  bazel build -c opt --config=native //Molecule_Tools:common_names
  ```

  This is appropriate for local development and benchmarking, but avoid it for
  portable/container build artifacts unless native CPU targeting is intended.

- **Build with `-c opt` if you are going to run the executable tests.** The
  expected outputs under `test/` were generated by `build_linux.sh`, which
  builds with `-c opt`. Bazel defaults to `fastbuild` when no compilation mode
  is given, and `.bazelrc` does not set one, so a plain `bazel build` can
  produce a binary whose floating point results differ from the expected
  outputs in the last printed digit.

  The effect is tiny but it does not have to be large to fail a test. A value
  sitting on a printing boundary flips whenever the last bit moves. In
  `test/iwdescr/default`, `w_alogp` is 1.6875 and `w_obalance` is -162.7625, so
  a fastbuild binary prints 1.688 and -162.762 where the expected output has
  1.687 and -162.763, and the case fails. Nothing is wrong with the code or
  with the expected output.

  If a test fails on one or two values in the last decimal place, check the
  compilation mode before concluding anything. For the same reason, regenerate
  expected outputs only from an optimised build.
...
- Python bindings are staged into `${LILLYMOL_HOME}/lib` with
  `src/copy_shared_libraries.sh`. Use `${LILLYMOL_HOME}/run_python.sh` to run
  scripts against the staged modules; it sets `PYTHONPATH` and
  `LD_LIBRARY_PATH` and checks `lib/lillymol.so.soabi` against the active Python
  interpreter ABI.

- For nanobind changes, run focused tests where practical, for example:
...
- Avoid destructive git operations. The worktree often contains user changes and
  generated files.
- For tools that expose reusable logic, prefer splitting behavior into a library
  with a thin CLI wrapper. This keeps command-line behavior stable while making
  the same implementation available to Python bindings and unit tests.
...
- Ring-system shape tools identify exit vectors as ring-system atoms connected
  to atoms outside that ring system. Isotope labels may be used as inclusion
  masks for selected ring atoms; preserve documented isotope semantics when
  changing these tools.
- GFP/TDT tools commonly use records such as `$SMI<`, `PCN<`, and `DIST<`, with
  items separated by `|`.
- Python tests require an environment with the expected packages and staged
