...
```
 ...
 -I inc=<n>       increment existing isotopic label before labelling join points - one time
...
### -I inc=\<n\> -I INC=\<n\>
The options increment isotopic labels. The lowercase version, `-I inc=\<n\>`
will do a one-time increment. Once it detects that the isotopic value has
been applied to an atom, no further increments are made. So if `-I inc=10` is
specified, no change will be made if, on any break, an isotope of 10 or
greater is found. This is used with the mcs tool.

Note that if the starting molecule has an existing isotope 10, `-I inc=10`
will have no effect.

If `-I INC=\<n\>` is specified, every time a bond is broken the isotopic
value is incremented. In this case, the resulting isotope is a measure
of the number of bonds removed from the atom. But these are added to
any isotope in the starting molecule.
