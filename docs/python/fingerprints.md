...
There are two common ways to start with LillyMol GFP fingerprints from Python.

### Read an existing GFP file

Use this when fingerprints have already been generated by `gfp_make.sh`,
`gfp_make.pl`, or another LillyMol GFP-producing tool.
...
for hit in gfp.nearest_neighbours(0, 5):
  print(hit.index, hit.distance, gfp.id(hit.index))
```

`GFPList.from_file()` is the right entry point when the file is the source of
truth. The first fingerprint record defines the schema, and every later record
must contain compatible fingerprint components.

### Generate standard GFP fingerprints from molecules

Use this when you have `Molecule` objects and want the standard LillyMol GFP
...
query_fp = ctx.fingerprint(MolFromSmiles("CCC query"))
for hit in gfp.nearest_neighbours(query_fp, 3):
  print(hit.index, hit.distance, gfp.id(hit.index))
```

For the common standard-fingerprint case, `GFPList.standard_from_molecules()` is
...
If the input is already a list of SMILES strings and no identifiers are needed,
`add_smiles()` avoids the explicit `Molecule` construction in Python:
...
`Molecule` objects into C++ rather than making a copied `std::vector<Molecule>`.
That keeps large collections much cheaper to fingerprint. Fingerprint generation
may update normal LillyMol cached/perceived state on the molecule, but it should
not make destructive structural changes.
