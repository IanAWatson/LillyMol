...
test4(const char* fname) {
  ...
  uint64_t nr = input.records_remaining();
...
// #define DEBUG_TEST4
    ...
    uint64_t r = input.records_remaining();
    if (r != nr - ndx) {
...
static int
test_echo_bytes(const char* fname) {
  iwstring_data_source input(fname);
  ...
  std::unique_ptr<char[]> free_b(b);

  cerr << "File " << fname << " contains " << total_records << " records, " << file_size
       << " bytes\n";
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    for (int j = 0; j < x; ++j) {
      if (!input.next_record(buffer)) {
        cerr << "Yipes, cannot read record " << j << " from '" << fname
             << "', read first " << x << " iteration " << i << endl;
        return 0;
      }
    }
    ...
    if (0 != as_read.strncmp(b, bytes_read)) {
      cerr << "Mismatch on data, expected " << bytes_read << " bytes, got "
           << as_read.length() << endl;
      return 0;
    }
  }
...
test_push_record(const char* fname) {
  ...
  const uint64_t nr = input.records_remaining();
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    for (int j = 0; j < x; ++j) {
      if (!input.next_record(buffer)) {
        cerr << "test_push_record:cannot read record " << j << " file contains " << nr
             << " iteration " << i << endl;
        return 0;
      }
    }
    ...
    if (input.records_remaining() != nr - x) {
      cerr << "test_push_record:read " << x << " of " << nr << " records, but have "
           << input.records_remaining() << " remaining records\n";
      return 0;
    }
...
tdss(int argc, char** argv) {
  ...
  cerr << "Size of off_t " << sizeof(off_t) << " and streampos " << sizeof(std::streampos)
       << endl;
