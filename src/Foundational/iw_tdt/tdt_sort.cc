...
class TAG_and_Direction : public IWString {
 ...
 public:
  ...
  int
  which_one_to_retrieve() const {
    return _which_one_to_retrieve;
  }
...
TAG_and_Direction::construct_from_command_line_token(const const_IWSubstring& t) {
  ...
  if (myt.starts_with(',') || myt.ends_with(',')) {
    cerr << "TAG_and_Direction::construct_from_command_line_token:invalid specification '"
         << t << "'\n";
    return 0;
  }
  ...
  if (!myt.split(s1, ',', s2)) {
    cerr << "TAG_and_Direction::construct_from_command_line_token:invalid directive '"
         << t << "'\n";
    return 0;
  }
  ...
  if (_which_one_to_retrieve > 0)
    ...
    cerr << "TAG_and_Direction::construct_from_command_line_token:invalid index "
...
int
TAG_and_Direction::build(const IW_TDT& tdt, float& f) const {
  ...
  if (tdt.dataitem_value(*this, tag_value, _which_one_to_retrieve))
    ...
    cerr << "TAG_and_Direction::build:cannot extract " << _which_one_to_retrieve
         << " ocurrence of '" << (*this) << "' from tdt\n";
    return 0;
  }
  ...
  if (strip_leading_non_numerics_from_strings &&
      numeric_after_stripping_leading_non_numerics(tag_value, f)) {
    return 1;
  }
...
int
TAG_and_Direction::build(const IW_TDT& tdt, IWString& f) const {
...
int
TAG_and_Direction::do_comparison(const IWString& s1, const IWString& s2) const {
  return s1.strcmp(s2);
}
...
int
Offset_Value_Float::build(off_t offset, IW_TDT& tdt, int& missing_values_found) {
  _pos = offset;
...
int
Offset_Value_String::build(off_t offset, IW_TDT& tdt, int& missing_values_found) {
  _pos = offset;
...
// clang-format off
  ...
  // clang-format off
  cerr << __FILE__ << " compiled " << __DATE__ << " " << __TIME__ << '\n';
  cerr << R"(Sorts a TDT file based on values of a tag
 -T <tag>       specify the sort field tag. A '<' will be added if needed.
 -T +tag        sort on tag in ascending order
 -T -tag        sort on tag in ascending order
 -T <tag>,col=n sort on column <n> of <tag>
 -T <n>,<tag>   which instance of <tag> to use
 -grepc <rx>    sort on number of matches to <rx> in each TDT
 -c <token>     sort key is column <token> in <tag>
 -r             reverse order
 -m <value>     value to be assigned to missing values
 -s             string comparisons rather than numeric
 -y             sort by number of records in each TDT
 -v             verbose output
)";
  // clang-format on
...
template <typename T>
int
echo_the_tdts(iwstring_data_source& input, T* ov, int ntdts, int output_fd) {
...
template <typename T>
int
read_the_tdts(iwstring_data_source& input, T* ov, int& ntdts) {
  ntdts = 0;
...
static int
do_grepc(const IW_TDT& tdt, re2::RE2& rx) {
...
static int
read_the_tdts_grepc_version(iwstring_data_source& input, Offset_Value_Int* ov,
                            int& ntdts) {
  ntdts = 0;
  off_t offset = input.tellg();
...
static int
read_the_tdts_nrecords_version(iwstring_data_source& input, Offset_Value_Float* ov,
                               int& ntdts) {
  ntdts = 0;
  off_t offset = input.tellg();
...
static int
tdt_sort(iwstring_data_source& input, int output_fd) {
  ...
  if (numeric_sort) {
    Offset_Value_Float* ov = new Offset_Value_Float[ntdts];
    std::unique_ptr<Offset_Value_Float[]> free_ov(ov);
    if (nullptr == ov) {
      cerr << "Very bad news, cannot allocate " << ntdts << " offset/value pairs\n";
      return 0;
    }
...
static int
tdt_sort(const char* fname, int output_fd) {
...
tdt_sort(int argc, char** argv) {
  ...
  if (cl.option_present('c')) {
    if (!cl.value('c', default_global_column_for_sort_key) ||
        default_global_column_for_sort_key < 1) {
      cerr << "INvalid identifier column (-c option)\n";
      usage(3);
    }

    if (verbose) {
      cerr << "The sort key will be column " << default_global_column_for_sort_key
           << " in the tag\n";
    }
  ...
  if (cl.option_present('y') && cl.option_present('T')) {
    cerr << "Sorry, the -y and -T options are mutually exclusive, contact LillyMol on "
