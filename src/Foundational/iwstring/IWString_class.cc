...
const_IWSubstring::debug_print(std::ostream& os) const {
  os << "const_IWSubstring::debug_print: " << _nchars << " at " << std::hex << &(_data[0])
     << " " << _data << endl;
...
static int
_strcasecmp(const char* s1, int l1, const char* s2, int l2) {
...
static int
_strncmp(const char* s1, int len1, const char* s2, int len2, int chars_to_compare) {
  ...
  {
    cerr << "_strncmp:comparison length " << chars_to_compare
         << " longer than string length " << len1 << " impossible\n";
...
IWString::strncmp(const IWString& rhs, int chars_to_compare) const {
  return _strncmp(_things, _number_elements, rhs._things, rhs._number_elements,
                  chars_to_compare);
}
...
static int
_common_matches_ignore_case(const char* needle, size_t len_needle, const char* haystack,
                            size_t len_haystack, int offset) {
...
int
IWString::matches_ignore_case(const char* s, size_t lens, int offset) const {
  return _common_matches_ignore_case(s, lens, _things, _number_elements, offset);
}

int
IWString::matches_ignore_case(const IWString& s, int offset) const {
  return _common_matches_ignore_case(s._things, s._number_elements, _things,
                                     _number_elements, offset);
}

int
const_IWSubstring::matches_ignore_case(const char* s, size_t lens, int offset) const {
  return _common_matches_ignore_case(s, lens, _data, _nchars, offset);
}

static int
common_equals_ignore_case(const char* s1, size_t lens1, const char* s2, size_t lens2) {
  if (lens1 != lens2) {
    return 0;
  }
...
static int
do_find(const char* haystack, int size_of_haystack, const char* needle,
        int size_of_needle) {
  assert(needle);
...
template int string_class_is_unsigned_int_4<unsigned long>(char const*, int,
                                                           unsigned long&);
...
string_class_is_int_4(const char* s, int nchars, T& result) {
  ...
  if (*s >
      '3') {  // int max is 2147483647, so any 10 digit number starting with 3 is invalid
    return 0;
  }
...
string_class_is_int_8(const char* s, int nchars, T& result) {
  ...
  // Now the case of 19 digits. just need to be careful. Use an unsigned value to get the
  // number then check to see if it is OK
...
template int string_class_is_unsigned_int_8<unsigned int>(char const*, int,
                                                          unsigned int&);
...
static double reciprocal_power_of_10[MAX_FRACTIONAL_DIGITS] = {
...
static double power_of_10[MAX_LEADING_DIGITS] = {
...
iw_parse_numeric_pos(const char* s, int nchars, double& result) {
  ...
  if ('.' == *s) {
    ...
    for (int i = 1; nchars && i < MAX_FRACTIONAL_DIGITS && isdigit(*s);
         i++, nchars--, s++) {
...
static int
common_nwords(const char* s, int nchars) {
  int rc = 0;
  int in_word = 0;
  for (int i = 0; i < nchars; i++) {
...
#endif
int
common_nwords_with_separator(const char* s, int nchars, char word_separator) {
  int rc = 0;
  int in_word = 0;
  for (int i = 0; i < nchars; i++) {
...
static int
locate_whitespace_word_boundaries(const char* s, int nchars, int which_word,
...
static int
common_locate_word_boundaries_rev(const char* s, int nchars, int which_word,
                                  char word_separator, int& word_start, int& word_stop) {
  assert(which_word < 0);
...
static int
common_locate_word_boundaries(const char* s, int nchars, int which_word,
                              char word_separator, int& word_start, int& word_stop) {
  if (which_word < 0) {
    return common_locate_word_boundaries_rev(s, nchars, which_word, word_separator,
...
int
IWString::_locate_word_boundaries(int which_word, char word_separator, int& word_start,
                                  int& word_stop) const {
  int in_word = 0;
  int word_count = 0;
  for (int i = 0; i < _number_elements; i++) {
...
IWString::word(int which_word, char word_separator) const {
  ...
  if (!common_locate_word_boundaries(_things, _number_elements, which_word,
                                     word_separator, word_start, word_stop)) {
...
const_IWSubstring::word(int which_word, char word_separator) const {
  ...
  if (!common_locate_word_boundaries(_data, _nchars, which_word, word_separator,
                                     word_start, word_stop)) {
    return const_IWSubstring("", 0);
  }
...
int
const_IWSubstring::word(int which_word, const_IWSubstring& result,
                        char word_separator) const {
  int word_start, word_stop;
...
const_IWSubstring::whitespace_delimited_word(int which_word, IWString& result) const {
  ...
  if (!locate_whitespace_word_boundaries(_data, _nchars, which_word, word_start,
                                         word_stop)) {
    result = "";
    return 0;
  }
...
static int
_locate_word_beginnings(const char* s, int nchars, resizable_array<int>& word_beginnings,
                        char word_separator) {
...
static int
common_locate_word_beginnings_single_delimiter(const char* s, int nchars,
...
int
const_IWSubstring::locate_word_beginnings_single_delimiter(
    resizable_array<int>& word_beginnings, char separator) const {
  return common_locate_word_beginnings_single_delimiter(_data, _nchars, word_beginnings,
                                                        separator);
}
...
IWString::locate_word_beginnings_single_delimiter(resizable_array<int>& word_beginnings,
                                                  char separator) const {
  return common_locate_word_beginnings_single_delimiter(_things, _number_elements,
                                                        word_beginnings, separator);
}
...
static int
internal_nextword(const char* s, int nchars, int& i, char separator, int& istart,
                  int& istop) {
  if (i >= nchars) {
    return 0;
  }
...
static int
internal_nextword(IWString& result, int& i, char separator, const char* s, int nchars) {
  ...
  // cerr << "After STRING determination, istart = " << istart << " istop = " << istop <<
  // endl;
...
#endif
int
internal_nextword(const_IWSubstring& result, int& i, char separator, const char* s,
                  int nchars) {
...
static int
internal_prevword(IWString& result, int& i, char separator, const char* s) {
  if (i <= 0) {
    return 0;
  }
...
const_IWSubstring::const_IWSubstring(const char* data) : _data(data) {
  if (nullptr != data) {
...
static int
common_remove_leading_words(const char* s, int nchars, int nremove, char word_separator) {
  int word_start, word_stop;

  // Most of the time, we will strip off everything before the first
  // word to be retained

  if (common_locate_word_boundaries(s, nchars, nremove, word_separator, word_start,
                                    word_stop)) {
    return word_start;
  }
...
static void
_cat(const char* lhs, int l1, const char* rhs, int l2, IWString& result) {
...
static int
_internal_ccount(const char* haystack, int nchars, char needle) {
  int rc = 0;
  for (int i = 0; i < nchars; i++) {
...
template <typename S>
int
common_split(const char* s, int nchars, S& before, char separator, S& after) {
  before = "";
  after = "";
...
int
IWString::split(const_IWSubstring& before, char separator,
                const_IWSubstring& after) const {
  return common_split(_things, _number_elements, before, separator, after);
}

int
IWString::split(IWString& before, char separator, IWString& after) const {
  return common_split(_things, _number_elements, before, separator, after);
}

int
const_IWSubstring::split(const_IWSubstring& before, char separator,
                         const_IWSubstring& after) const {
  return common_split(_data, _nchars, before, separator, after);
}

int
const_IWSubstring::split(IWString& before, char separator, IWString& after) const {
  return common_split(_data, _nchars, before, separator, after);
}
...
static int
common_split(resizable_array_p<const_IWSubstring>& tokens, char separator,
             const char* zdata, int nchars) {
  int rc = 0;
  ...
  while (internal_nextword(token, i, separator, zdata, nchars)) {
    const_IWSubstring* tmp =
        new const_IWSubstring(zdata + i - token.nchars(), token.nchars());
    tokens.add(tmp);
...
static int
common_split(resizable_array_p<IWString>& tokens, char separator, const char* zdata,
             int nchars) {
  int rc = 0;
...
int
IWString::split(resizable_array_p<const_IWSubstring>& tokens, char separator) const {
...
int
IWString::split(resizable_array_p<IWString>& tokens, char separator) const {
  tokens.resize_keep_storage(0);
...
template <typename T>
int
common_split(const char* s, int nchars, iwaray<T>& tokens, char separator) {
...
static int
common_split(const char* s, int nchars, IWString* tokens, char separator) {
...
template <typename T>
int
common_basename(const char* s, int nchars, T& zresult, char separator)
...
int
IWString::_common_gsub(const char* zfrom, int len_from, const char* zto, int len_to) {
  if (0 == _number_elements) {
    return 0;
  }
...
int
IWString::_common_gsub_getting_larger(const char* zfrom, int len_from, const char* zto,
                                      int len_to) {
  ...
  while (iptr < _number_elements - len_from + 1) {
    ...
    ::memmove(_things + iptr + len_to, _things + iptr + len_from,
              _number_elements - iptr - len_from);
...
int
IWString::_common_gsub_same_size(const char* zfrom, int len_from, const char* zto) {
  int rc = 0;
...
int
IWString::_common_gsub_getting_smaller(const char* zfrom, int len_from, const char* zto,
                                       int len_to) {
  ...
  while (readfrom < _number_elements - len_from + 1) {
    if (_things[readfrom] != zfrom[0] ||
        0 != ::strncmp(_things + readfrom, zfrom, len_from)) {
      if (readfrom != putback) {
...
IWString::gsub(const char* zfrom, const char* zto) {
  return _common_gsub(zfrom, static_cast<int>(::strlen(zfrom)), zto,
                      static_cast<int>(::strlen(zto)));
}
...
static int
common_matches_at_position(const char* s1, int lens1, int offset, const char* s2,
                           int lens2) {
...
int
const_IWSubstring::matches_at_position(int o, const char* s, int lens) const {
  return common_matches_at_position(_data, _nchars, o, s, static_cast<int>(lens));
}

int
const_IWSubstring::matches_at_position(int o, const IWString& s) const {
  return common_matches_at_position(_data, _nchars, o, s.rawchars(), s.length());
}

int
IWString::matches_at_position(int o, const char* s, int lens) const {
  return common_matches_at_position(_things, _number_elements, o, s,
                                    static_cast<int>(lens));
}

template <typename T>
int
common_nextword_single_delimiter(const char* s, int nchars, int& ndx, char separator,
                                 T& zresult) {
  if (ndx >= nchars) {
    return 0;
  }
...
IWString::nextword_single_delimiter(T& zresult, int& i, char separator) const {
  return common_nextword_single_delimiter(_things, _number_elements, i, separator,
                                          zresult);
}

template int IWString::nextword_single_delimiter<const_IWSubstring>(const_IWSubstring&,
                                                                    int&, char) const;
template int IWString::nextword_single_delimiter<IWString>(IWString&, int&, char) const;
template int const_IWSubstring::nextword_single_delimiter<const_IWSubstring>(
    const_IWSubstring&, int&, char) const;
template int const_IWSubstring::nextword_single_delimiter<IWString>(IWString&, int&,
                                                                    char) const;

template int common_nextword_single_delimiter<const_IWSubstring>(char const*, int, int&,
                                                                 char,
                                                                 const_IWSubstring&);
template int common_nextword_single_delimiter<IWString>(char const*, int, int&, char,
                                                        IWString&);
...
template int const_IWSubstring::NextWord<const_IWSubstring>(const_IWSubstring&, int&,
                                                            char) const;
...
void
char_name_to_char_usage(const IWString& optionTag) {
  cerr << "    " << optionTag << " tab" << endl;

  cerr << "    " << optionTag << " comma -  for a comma (\",\") separator" << endl;
  cerr << "    " << optionTag << " space -  for a space (\" \") separator" << endl;
  cerr << "    " << optionTag << " vbar or bar -  for a vertical bar (\"|\") separator"
       << endl;
  cerr << "    " << optionTag << " squote -  for a single quote  (\"'\") separator"
       << endl;
  ...
  cerr << "    " << optionTag << " and or amp -  for a apersand (\"&\") separator"
       << endl;
  cerr << "    " << optionTag << " dollar -  for a dollar sign (\"$\") separator" << endl;
  cerr << "    " << optionTag
       << " hash -  for a hash tag or number sign (\"#\") separator" << endl;
  cerr << "    " << optionTag << " at -  for an at sign (\"@\") separator" << endl;
  cerr << "    " << optionTag
       << " excl or bang -  for a exclmation point (\"!\") separator" << endl;
  cerr << "    " << optionTag << " squiggle or tilde -  for a tilda (\"~\") separator"
       << endl;
  cerr << "    " << optionTag << " star or asterisk -  for an asterisk (\"*\") separator"
       << endl;
  cerr << "    " << optionTag << " hat or caret -  for a caret (\"^\") separator" << endl;
  cerr << "    " << optionTag
       << " oparen or lparen -  for an open parenthesis (\"(\") separator" << endl;
  cerr << "    " << optionTag
       << " cparen or rparen -  for a close parenthesis (\")\") separator" << endl;
  ...
  cerr << "    " << optionTag
       << " ocbrace or lcbrace -  for an open curly brace (\"{\") separator" << endl;
  cerr << "    " << optionTag
       << " ccbrace or rcbrace -  for a close curly brace (\"}\") separator" << endl;
  cerr << "    " << optionTag
       << " osqb or lsqb -  for an opeen square bracket (\"[\") separator" << endl;
  cerr << "    " << optionTag
       << " csqb or rsqb -  for a close square bracket (\"]\") separator" << endl;
  cerr << "    " << optionTag << " bslash -  for a backslash (\"\\\") separator" << endl;
  cerr << "    " << optionTag << " fslash or slash -  for a slash (\"/\") separator"
       << endl;
  cerr << "    " << optionTag << " dot or period -  for a period (\".\") separator"
       << endl;
  cerr << "    " << optionTag
       << " oangle or less -  for a less than sign (\"<\") separator" << endl;
  cerr << "    " << optionTag
       << " cangle or greater -  for a greater than sign  (\">\") separator" << endl;
  cerr << "    " << optionTag << " qmark -  for a question mark (\"?\") separator"
       << endl;
  cerr << "    " << optionTag << " pct -  for a percent sign (\"%\") separator" << endl;
  cerr << "    " << optionTag
       << " grave or backquote -  for a back quote sign (\"`\") separator" << endl;
}
...
const_IWSubstring::expand_environment_variables(IWString& destination) const {
  ...
  for (int i = 0; i < _nchars; ++i) {
    ...
    if (needs_close_brace) {
      if (_data[i] != '}') {
        cerr << "const_IWSubstring::expand_environment_variables:unclosed open brace, "
                "got '"
             << v << "'\n";
        return 0;
      }
