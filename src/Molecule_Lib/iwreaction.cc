...
void
set_iwreaction_display_no_atoms_in_query_message(int s) {
...
//  in molecular_transformations, which uses this, we are now allowing the -Z option to
//  ignore reactions that have no reactants, so we need to allow NOT showing this message
//  - Tad Hurst 20180209
...
static int warn_no_query_in_reaction_object = 1;
void
set_warn_no_query_in_reaction_object(int s) {
  warn_no_query_in_reaction_object = s;
}
...
static int
write_numbered_smiles(Molecule& m, std::ostream& os) {
  int matoms = m.natoms();
...
IWReaction::number_products_per_scaffold_embedding() const {
  ...
  for (int i = 0; i < ns; i++) {
    ...
    if (0 == s->number_reagents()) {
      cerr << "IWReaction::number_products_per_scaffold_embedding: sidechain " << i
           << " with no reagents!\n";
    } else {
...
IWReaction::set_one_embedding_per_start_atom(const int s) {
  // cerr << "IWReaction::set_one_embedding_per_start_atom:setting " << s << " across " <<
  // _sidechains.size() << " sidechains\n";
...
IWReaction::reagent(const Reaction_Iterator& iterator) const {
  ...
  if (ns >
      1) {  // ambiguous as to what is the "current" sidechain. Should be more careful and
            // see if there is just one sidechain with varying reagents...
    return nullptr;
  }
...
Reaction_Change_Element::construct_from_msi_attribute(const msi_attribute* att) {
  ...
  if (2 != rce.nwords()) {
    cerr << "Reaction_Change_Element::construct_from_msi_attribute: must have two string "
            "attributes\n";
    cerr << (*att) << '\n';
    cerr << rce << '\n';
    return 0;
  ...
  if (!token.numeric_value(_atom) || _atom < 0) {
    cerr << "Reaction_Change_Element::construct_from_msi_attribute: atom specifiers must "
  ...
  if (nullptr == _element) {
    cerr
        << "Reaction_Change_Element::construct_from_msi_attribute: unrecognised element '"
        << token << "'\n";
    cerr << (*att) << '\n';
    return 0;
  }
...
int
Reaction_Change_Element::process(Molecule& result, const Set_of_Atoms& embedding,
                                 int offset) {
...
Reaction_Formal_Charge::construct_from_msi_attribute(const msi_attribute* att) {
  if (2 != att->number_int_values()) {
    cerr << "Reaction_Formal_Charge::construct_from_msi_attribute: must have two int "
            "attributes\n";
    cerr << (*att) << '\n';
    return 0;
  }
  ...
  if (_atom < 0) {
    cerr << "Reaction_Formal_Charge::construct_from_msi_attribute: atom specifiers must "
            "be non negative ints\n";
    cerr << (*att) << '\n';
    return 0;
  }
...
int
Reaction_Formal_Charge::process(Molecule& result, const Set_of_Atoms& embedding,
                                int offset) {
  assert(embedding.ok_index(_atom));
...
int
Reaction_Change_Formal_Charge::process(Molecule& result, const Set_of_Atoms& embedding,
                                       int offset) {
  assert(embedding.ok_index(_atom));
  ...
  // cerr << "Initially formal charge is " << result.formal_charge (a + offset) << " will
  // change to " << fc << '\n';
...
Reaction_Place_Isotope::construct_from_msi_attribute(const msi_attribute* att) {
  if (2 != att->number_int_values()) {
    cerr << "Reaction_Place_Isotope::construct_from_msi_attribute: must have two int "
            "attributes\n";
    cerr << (*att) << '\n';
    return 0;
  }
  ...
  if (_atom[0] < 0) {
    cerr << "Reaction_Place_Isotope::construct_from_msi_attribute: atom specifiers must "
            "be non negative ints\n";
    cerr << (*att) << '\n';
    return 0;
  }
...
int
Reaction_Place_Isotope::process(Molecule& result, const Set_of_Atoms& embedding,
                                int offset) {
...
int
Reaction_Increment_Isotope::process(Molecule& result, const Set_of_Atoms& embedding,
                                    int offset) {
  for (const int matched_atom : _atom) {
    const atom_number_t a = embedding[matched_atom];

    // cerr << "matched atom " << matched_atom << " current isotope is " <<
    // result.isotope(a+offset) << " delta " << _isotope << '\n';

    // The current isotope.
    isotope_t iso = result.isotope(a + offset);
    // cerr << "_only_change_if.is_set() " << _only_change_if.is_set() << " match " <<
    // _only_change_if.matches(iso) << '\n';
...
int
Reaction_Invert_Isotope::process(Molecule& result, const Set_of_Atoms& embedding,
                                 int offset) {
  for (const int matched_atom : _atom) {
...
static int
atom_in_list_of_bonds(const resizable_array_p<Bond>& bonds, int zatom) {
...
static int
check_atoms(const resizable_array<int>& atoms, int atoms_in_scaffold_query) {
  int n = atoms.number_elements();
  for (int i = 0; i < n; i++) {
    if (atoms[i] >= atoms_in_scaffold_query) {
      cerr << "IWReaction::check_internal_consistency: invalid atom specifier "
           << atoms[i] << '\n';
...
IWReaction::check_internal_consistency() {
  ...
  for (int i = 0; i < ns; i++) {
    if (!_sidechains[i]->check_internal_consistency(ns, atoms_in_scaffold_query,
                                                    _atoms_to_be_removed,
                                                    _fragments_to_be_removed)) {
      cerr << "IWReaction::check_internal_consistency: sidechain " << i
           << " is invalid\n";
      return 0;
    }
  }
...
static int
check_bonds(const resizable_array_p<Bond>& bond, int atoms_in_query) {
...
Reaction_Site::set_find_one_embedding_per_atom(int s) {
  // cerr << "Reaction_Site::set_find_one_embedding_per_atom " << s << " have " <<
...
Reaction_Site::check_internal_consistency() {
  ...
  if (atoms_in_query > 0)
    ...
    if (iwreaction_display_no_atoms_in_query_message) {
      cerr << "Reaction_Site::check_internal_consistency:no atoms in query. Probably "
              "fixed reagent in ISIS file\n";
    }
    return 1;
  }
  ...
  for (int i = 0; i < n; i++) {
    if (atom_in_list_of_bonds(_bonds_to_be_made, _atoms_to_be_removed[i])) {
      cerr << "Reaction_Site::check_internal_consistency: atom to be removed "
           << _atoms_to_be_removed[i] << " is involved in a bond to be made\n";
      return 0;
    }
  ...
  if (n && _fragments_to_be_kept.number_elements()) {
    cerr << "Reaction_Site::check_internal_consistency:cannot specify both fragments to "
            "remove and fragments to keep\n";
    return 0;
  }

  for (int i = 0; i < n; i++) {
    if (atom_in_list_of_bonds(_bonds_to_be_made, _fragments_to_be_removed[i])) {
      cerr << "Reaction_Site::check_internal_consistency: fragment to be removed "
           << _fragments_to_be_removed[i] << " is involved in a bond to be made\n";
      return 0;
    }
...
int
Sidechain_Reaction_Site::_check_inter_particle_atoms(
    const int atoms_in_scaffold_query, const int scaffold_atom,
  ...
  if (scaffold_atom >= atoms_in_scaffold_query) {
    cerr << "Sidechain_Reaction_Site::check_internal_consistency: invalid inter particle "
            "bond\n";
    cerr << "Scaffold query contains " << atoms_in_scaffold_query << " atoms, so "
         << scaffold_atom << " is invalid\n";
    return 0;
  }

  // Does the scaffold delete its end of an inter-particle bond

  if (scaffold_atoms_to_be_removed.contains(scaffold_atom) ||
      scaffold_fragments_to_be_removed.contains(scaffold_atom)) {
    cerr << "Sidechain_Reaction_Site::check_internal_consistency: invalid inter particle "
            "bond\n";
    ...
    for (int i = 0; i < scaffold_atoms_to_be_removed.number_elements(); i++) {
      cerr << "Atom " << scaffold_atoms_to_be_removed[i]
           << " in the scaffold will be removed\n";
    }
    return 0;
  }
  ...
  if (_atoms_to_be_removed.contains(sidechain_atom) ||
      _fragments_to_be_removed.contains(sidechain_atom)) {
    cerr << "Sidechain_Reaction_Site::check_internal_consistency: invalid inter particle "
            "bond\n";
...
int
Sidechain_Reaction_Site::check_internal_consistency(
    const int number_sidechains, const int atoms_in_scaffold_query,
    const resizable_array<int>& scaffold_atoms_to_be_removed,
    const resizable_array<int>& scaffold_fragments_to_be_removed) {
  if (!Reaction_Site::check_internal_consistency()) {
    return 0;
  ...
  for (int i = 0; i < _inter_particle_bonds.number_elements(); i++) {
    ...
    if (!_check_inter_particle_atoms(
            atoms_in_scaffold_query, a1.matched_atom(), scaffold_atoms_to_be_removed,
            scaffold_fragments_to_be_removed, a2.matched_atom())) {
      cerr << "Sidechain_Reaction_Site:check_internal_consistency:invalid inter particle "
              "bond\n";
      return 0;
    }
  ...
  for (int i = 0; i < _replace_atom.number_elements(); i++) {
    ...
    if (a1.in_scaffold() && a1.matched_atom() >= atoms_in_scaffold_query) {
      cerr << "Sidechain_Reaction_Site::check_internal_consistency: scaffold atom "
           << a1.matched_atom() << " is invalid - max " << atoms_in_scaffold_query
           << '\n';
      return 0;
    }
    ...
    if (a2.in_scaffold() && a2.matched_atom() >= atoms_in_scaffold_query) {
      cerr << "Sidechain_Reaction_Site::check_internal_consistency: scaffold atom "
           << a2.matched_atom() << " is invalid - max " << atoms_in_scaffold_query
           << '\n';
      return 0;
    }
...
Reaction_Site::ok() const {
  ...
  if (_atoms_to_be_removed.number_elements() ||
      _fragments_to_be_removed.number_elements()) {
    ...
    if (_atoms_to_be_removed.number_elements()) {
      ...
      for (int i = 0; i < na; i++) {
        if (!atom_in_list_of_bonds(_bonds_to_be_broken, _atoms_to_be_removed[i])) {
          cerr << "Reaction_Site::ok: atom to be removed " << _atoms_to_be_removed[i]
               << " not in bond breaking list\n";
          return 0;
        }
      }
    }
    ...
    if (_fragments_to_be_removed.number_elements()) {
      ...
      for (int i = 0; i < na; i++) {
        if (!atom_in_list_of_bonds(_bonds_to_be_broken, _fragments_to_be_removed[i])) {
          cerr << "Reaction_Site::ok: fragment to be removed "
               << _fragments_to_be_removed[i] << " not in bond breaking list\n";
          return 0;
        }
      }
    }
...
static int
common_bond_creation_thingy(int a1, int a2, bond_type_t bt,
...
int
Reaction_Site::add_toggle_kekule_form(int a1, int a2, bond_type_t bt) {
...
Reaction_Site::_atom_slated_for_removal(int a) const {
  if (_atoms_to_be_removed.contains(a)) {
    cerr << "Sidechain_Reaction_Site::_atom_slated_for_removal: matched atom " << a
         << " slated for atom removal\n";
    return 1;
  }

  if (_fragments_to_be_removed.contains(a)) {
    cerr << "Sidechain_Reaction_Site::_atom_slated_for_removal: matched atom " << a
         << " slated for fragment removal\n";
    return 1;
  }
...
Sidechain_Reaction_Site::ok() const {
  ...
  for (int i = 0; i < nb; i++) {
    ...
    if (_atom_slated_for_removal(b->a2())) {
      cerr << "Sidechain_Reaction_Site::ok: matched atom " << b->a2()
           << " slated for removal\n";
      return 0;
    }
  }
...
int
Sidechain_Reaction_Site::debug_print(std::ostream& os,
...
int
Reaction_Site::debug_print(std::ostream& os, const const_IWSubstring& ind) const {
...
static Bond*
parse_bond_attribute(const msi_attribute* att, bond_type_t bt = SINGLE_BOND) {
  if (2 != att->number_int_values()) {
...
static int
parse_atom_values(const msi_attribute* att, resizable_array<int>& zatoms) {
...
template <typename T>
int
process_attribute(const msi_attribute* att, resizable_array_p<T>& things) {
  T* t = new T;
...
template int process_attribute(const msi_attribute*,
                               resizable_array_p<Reaction_Change_Element>&);
template int process_attribute(const msi_attribute*,
                               resizable_array_p<Reaction_Formal_Charge>&);
template int process_attribute(const msi_attribute*,
                               resizable_array_p<Reaction_Change_Formal_Charge>&);
template int process_attribute(const msi_attribute*,
                               resizable_array_p<Reaction_Place_Isotope>&);
template int process_attribute(const msi_attribute*,
                               resizable_array_p<Reaction_Increment_Isotope>&);
template int process_attribute(const msi_attribute*,
                               resizable_array_p<Reaction_Invert_Isotope>&);
template int process_attribute(const msi_attribute*,
                               resizable_array_p<Reaction_Stereo_Centre>&);
template int process_attribute(const msi_attribute*,
                               resizable_array_p<Reaction_Dihedral_Angle>&);
template int process_attribute(const msi_attribute*,
                               resizable_array_p<Reaction_Bond_Length>&);
template int process_attribute(const msi_attribute*,
                               resizable_array_p<Reaction_Bond_Angle>&);
template int process_attribute(const msi_attribute*,
                               resizable_array_p<Reaction_3D_Replace>&);
...
Reaction_Site::_parse_recompute_implicit_hydrogens(const msi_attribute* att) {
  ...
  if (_unfix_implicit_hydrogens.number_elements() + n <
      _unfix_implicit_hydrogens.elements_allocated()) {
...
Reaction_Site::_read_inactive_from_query_file(const IWString& fname) {
  ...
  if (!q->read(tmp)) {
    cerr << "Reaction_Site::_read_inactive_from_query_file: cannot read query file '"
         << fname << "'\n";
    delete q;
...
Reaction_Site::_read_query_file_from_molecule(IWString& fname) {
  ...
  if (nullptr == m) {
    cerr << "Reaction_Site::_read_query_file_from_molecule:cannot read molecule from '"
         << fname << "'\n";
    return 0;
  }
  ...
  if (!Substructure_Query::create_from_molecule(*m, mqs)) {
    cerr << "Reaction_Site::_read_query_file_from_molecule:cannot build query from "
         << m->smiles() << '\n';
    return 0;
  }
...
Reaction_Site::_read_query_file(const IWString& fname,
  ...
  // cerr << "Parsing query file attribute, same " << query_files_in_current_directory <<
  // " rxn dir " << reaction_directory << " name '" << fname << "'\n";
  ...
  if (!Substructure_Query::read(query_fname)) {
    cerr << "Reaction_Site::_read_query_file_from_molecule: cannot process query '"
         << query_fname << "'\n";
    cerr << "In directory " << query_files_in_current_directory << " dir '"
         << reaction_directory << "'\n";
    return 0;
  }
...
Reaction_Site::construct_from_msi_object(const msi_object& msi,
  ...
  for (int i = 0; i < nmsi; i++) {
    ...
    if ("Query" == obj->name() || "query" == obj->name()) {
      ...
      found_query = 1;
    } else if (NAME_OF_NO_REACTION_OBJECT ==
               obj->name())  // will be parsed by the owning sidechain object
      ;
    else {
      cerr << "Reaction_Site::construct_from_msi_object: WHAT KIND OF OBJECT IS THIS '"
           << obj->name() << "'\n";
      return 0;
    }
  }
  ...
  if (_match_via_atom_map) {
    ...
    for (int i = 0; i <= h; ++i) {
      ...
      if (nullptr == a) {
        cerr << "Reaction_Site::construct_from_msi_object:no query atom with initial "
                "atom number "
             << i << '\n';
        return 0;
      }
  ...
  for (int i = 0; i < nat; i++) {
    ...
    if (NAME_OF_QUERY_FILE_ATTRIBUTE == att->name()) {
      ...
      if (!_read_query_file(att->stringval(), query_files_in_current_directory,
                            reaction_directory)) {
        cerr << "Reaction_Site::construct_from_msi_object:cannot process query file '"
             << att->stringval() << "'\n";
        return 0;
      }
      ...
      if (!att->value(max_matches_allowed) || max_matches_allowed < 1) {
        cerr << "IWReaction::_construct_from_msi_object: the '"
             << NAME_OF_REACTION_MAX_MATCHES_ATTRIBUTE
      ...
      if (!_read_inactive_query(att)) {
        cerr << "Reaction_Site::construct_from_msi_object: invalid "
             << NAME_OF_INACTIVE_ATTRIBUTE << " attribite " << (*att) << '\n';
        return 0;
      }
      ...
      if (!att->value(tmp)) {
        cerr << "Reaction_Site::construct_from_msi_object: invalid "
             << NAME_OF_EACH_COMPONENT_SEARCH_ATTRIBUTE << " attribute " << (*att)
             << '\n';
        return 0;
      }
      ...
      _noop_reaction = 1;
    } else if (NAME_OF_INTER_PARTICLE_BOND_ATTRIBUTE ==
               att->name())  // ignore these, handled by derived object only
      ;
    else if (NAME_OF_INTER_PARTICLE_SUBSTITUTE_ATOM_ATTRIBUTE ==
             att->name())  // ignore these, handled by derived object only
      ;
    else if (NAME_OF_SUBSTITUTE_ATOM_ATTRIBUTE ==
             att->name())  // ignore these, handled by derived object only
      ;
      ...
      if (!att->value(tmp) || tmp < 0) {
        cerr << "Reaction_Site::construct_from_msi_object: invalid "
             << NAME_OF_INVERT_STEREO_CENTRE_ATTRIBUTE << " value\n";
        return 0;
      }
      ...
      if (!_toggle_kekule_form.add_bond_from_msi_attribute(*att)) {
        cerr << "Reaction_Site::construct_from_msi_object:invalid toggke kekule form "
                "specification '"
             << (*att) << "'\n";
        return 0;
      }
      ...
      cerr << "Reaction_Site::construct_from_msi_object: unrecognised attribute '"
           << att->name() << "'\n";
      return 0;
    }
  }
  ...
  */
  ...
  if (warn_no_query_in_reaction_object) {
...
Scaffold_Reaction_Site::construct_from_msi_object(const msi_object& msi,
                                                  int query_files_in_current_directory,
                                                  const IWString& reaction_directory) {
  if (!Reaction_Site::construct_from_msi_object(msi, query_files_in_current_directory,
                                                reaction_directory)) {
    return 0;
  }
...
Inter_Particle_Bond::Inter_Particle_Bond(int frag1, atom_number_t z1, atom_number_t z2,
                                         bond_type_t b) {
  _default_values();
...
int
Inter_Particle_Bond::OkSidechainIsotopeConstraint(Molecule& m, atom_number_t a1,
                                                  atom_number_t a2) const {
...
/*
  Inter particle bonds will have two or three components.
    ...
    (A C join "S1.1 2")       make single bond between atom 1 in sidechain 1 and matched
  atom 2 (A C join "1 S.0 2")
*/
...
static Replace_Atom*
parse_replace_atom_attribute(const msi_attribute* att, int default_component) {
...
int
Replace_Atom::construct_from_msi_object(const msi_attribute* att, int default_component) {
...
int
Replace_Atom::adjust_matched_atoms_in_component(
...
static int
remove_hits_that_touch_inactive_matches(
    Molecule_to_Match& target, const resizable_array_p<Substructure_Query>& inactive_qry,
    Substructure_Results& sresults) {
  // int * inactive = new_int(target.natoms()); std::unique_ptr<int[]>
  // free_inactive(inactive);
...
int
Reaction_Site::substructure_search(Molecule& m, Substructure_Results& sresults) {
  Molecule_to_Match target(&m);
...
int
Reaction_Site::_determine_matched_atoms_checking_inactives(
    Molecule& m, Substructure_Results& sresults) {
  Molecule_to_Match target(&m);
...
int
Sidechain_Reaction_Site::construct_from_msi_object(
    const msi_object& msi, int query_files_in_current_directory,
    const IWString& reaction_directory, const Sidechain_Match_Conditions& smc) {
...
Sidechain_Reaction_Site::_add_reagents_from_smiles(const msi_attribute& att) {
  ...
  std::unique_ptr<Molecule_and_Embedding> reagent =
      std::make_unique<Molecule_and_Embedding>();
  ...
  if (1 == nhits)
    ...
    if (_noop_reaction)
      ...
      cerr << "Sidechain_Reaction_Site::construct_from_msi_object: yipes, no match to "
              "query\n";
      cerr << att;
      return 0;
    }
  } else if (nhits > 1) {
    cerr << "Sidechain_Reaction_Site::construct_from_msi_object: yipes, " << nhits
         << " matches to query\n";
    return 0;
  }
...
Sidechain_Reaction_Site::set_single_reagent(Molecule& m) {
  if (_reagents.number_elements()) {
    cerr << "Sidechain_Reaction_Site::set_single_reagent:already got reagents, "
            "impossible\n";
    return 0;
  }
  ...
  if (0 == nhits) {
    cerr << "Sidechain_Reaction_Site::set_single_reagent:no hits to query in "
         << m.smiles() << '\n';
    return 0;
  }

  if (nhits > 1) {
    cerr << "Sidechain_Reaction_Site::set_single_reagent: " << nhits
         << " hits to query in " << m.smiles() << '\n';
    return 0;
  }
...
int
Inter_Particle_Bond::adjust_matched_atoms_in_component(
    const extending_resizable_array<int>& xref) {
...
static int
write_array(std::ostream& os, const const_IWSubstring& ind,
            const resizable_array<int>& things, const const_IWSubstring& attribute_name) {
...
template <typename T>
int
write_set(std::ostream& os, const const_IWSubstring& ind,
          const resizable_array_p<T>& things, const const_IWSubstring& attribute_name) {
...
template int write_set(std::ostream&, const const_IWSubstring&,
                       const resizable_array_p<Reaction_Change_Element>&,
                       const const_IWSubstring&);
template int write_set(std::ostream&, const const_IWSubstring&,
                       const resizable_array_p<Reaction_Formal_Charge>&,
                       const const_IWSubstring&);
template int write_set(std::ostream&, const const_IWSubstring&,
                       const resizable_array_p<Reaction_Change_Formal_Charge>&,
                       const const_IWSubstring&);
template int write_set(std::ostream&, const const_IWSubstring&,
                       const resizable_array_p<Reaction_Place_Isotope>&,
                       const const_IWSubstring&);
template int write_set(std::ostream&, const const_IWSubstring&,
                       const resizable_array_p<Reaction_Increment_Isotope>&,
                       const const_IWSubstring&);
template int write_set(std::ostream&, const const_IWSubstring&,
                       const resizable_array_p<Reaction_Invert_Isotope>&,
                       const const_IWSubstring&);
template int write_set(std::ostream&, const const_IWSubstring&,
                       const resizable_array_p<Reaction_Stereo_Centre>&,
                       const const_IWSubstring&);
template int write_set(std::ostream&, const const_IWSubstring&,
                       const resizable_array_p<Reaction_Wedge_Bond>&,
                       const const_IWSubstring&);
template int write_set(std::ostream&, const const_IWSubstring&,
                       const resizable_array_p<Reaction_Dihedral_Angle>&,
                       const const_IWSubstring&);
template int write_set(std::ostream&, const const_IWSubstring&,
                       const resizable_array_p<Reaction_Bond_Length>&,
                       const const_IWSubstring&);
template int write_set(std::ostream&, const const_IWSubstring&,
                       const resizable_array_p<Reaction_Bond_Angle>&,
                       const const_IWSubstring&);
template int write_set(std::ostream&, const const_IWSubstring&,
                       const resizable_array_p<Reaction_3D_Replace>&,
                       const const_IWSubstring&);

static int
write_bond(std::ostream& os, const IWString& ind, const char* attribute_name,
           const Bond* b) {
...
static int
write_atoms(std::ostream& os, const IWString& ind, const char* attribute_name,
...
int
Inter_Particle_Bond::write_msi(std::ostream& os, const IWString& ind) const {
  os << ind << "  (A C " << NAME_OF_INTER_PARTICLE_BOND_ATTRIBUTE << " \"" << _a1 << ' '
     << _a2 << ' ' << character_representation_of_bond(_bt) << "\")\n";
...
operator<<(std::ostream& os, const Inter_Particle_Bond& b) {
  os << "Inter particle bond between " << b.a1() << " and " << b.a2() << " type "
     << character_representation_of_bond(b.btype());
...
int
Reaction_Site::_write_msi(std::ostream& os, int& object_id, const IWString& ind) {
  if (_comment.length()) {
  ...
  if (_unfix_implicit_hydrogens.number_elements()) {
    write_array(os, ind, _unfix_implicit_hydrogens,
                NAME_OF_RECOMPUTE_IMPLICIT_HYDROGENS_ATTRIBUTE);
  }
  ...
  if (_chiral_centres_to_remove.number_elements()) {
    write_array(os, ind, _chiral_centres_to_remove,
                NAME_OF_REMOVE_CHIRAL_CENTRE_ATTRIBUTE);
  }
  ...
  for (int i = 0; i < _stereo_centres_to_invert.number_elements(); i++) {
    os << ind << "  (A I " << NAME_OF_INVERT_STEREO_CENTRE_ATTRIBUTE << ' '
       << _stereo_centres_to_invert[i] << ")\n";
  }
...
int
Sidechain_Reaction_Site::write_msi(std::ostream& os, int& object_id, int indentation) {
  assert(ok());
  ...
  if (_make_implicit_hydrogens_explicit) {
    os << ind << "  (" << NAME_OF_MAKE_IMPLICIT_HYDROGEN_EXPLICIT_ATTRIBUTE << ' '
       << _make_implicit_hydrogens_explicit << ")\n";
  }
...
int
Scaffold_Reaction_Site::write_msi(std::ostream& os, int& object_id, int indentation) {
  assert(ok());
...
int
IWReaction::_construct_from_msi_object(
    const msi_object& msi, const Sidechain_Match_Conditions& sidechain_match_conditions) {
  assert(_sidechains.empty());

  int nmsi = msi.number_elements();
  // if (nmsi < 2)
  //{
  //   cerr << "IWReaction::construct_from_msi_object: a reaction object must have at
  ...
  for (int i = 0; i < nmsi; i++) {
    ...
    if (NAME_OF_SCAFFOLD_OBJECT == obj->name()) {
      ...
      if (!Scaffold_Reaction_Site::construct_from_msi_object(
              *obj, _query_files_in_current_directory, _reaction_directory)) {
        return 0;
      }
    } else if (NAME_OF_SIDECHAIN_OBJECT == obj->name() ||
               OLD_NAME_OF_SIDECHAIN_OBJECT == obj->name()) {
      ...
      if (!s->construct_from_msi_object(*obj, _query_files_in_current_directory,
                                        _reaction_directory,
...
int
IWReaction::construct_from_msi_object(
    const msi_object& msi, const Sidechain_Match_Conditions& sidechain_match_conditions) {
...
template void do_adjust_matched_atoms_in_component<Reaction_3D_Replace>(
    resizable_array_p<Reaction_3D_Replace>&, extending_resizable_array<int> const&);
template void do_adjust_matched_atoms_in_component<Inter_Particle_Bond>(
    resizable_array_p<Inter_Particle_Bond>&, extending_resizable_array<int> const&);
template void do_adjust_matched_atoms_in_component<Reaction_Dihedral_Angle>(
    resizable_array_p<Reaction_Dihedral_Angle>&, extending_resizable_array<int> const&);
template void do_adjust_matched_atoms_in_component<Reaction_Bond_Length>(
    resizable_array_p<Reaction_Bond_Length>&, extending_resizable_array<int> const&);
template void do_adjust_matched_atoms_in_component<Reaction_Bond_Angle>(
    resizable_array_p<Reaction_Bond_Angle>&, extending_resizable_array<int> const&);
template void do_adjust_matched_atoms_in_component<Replace_Atom>(
    resizable_array_p<Replace_Atom>&, extending_resizable_array<int> const&);

int
Reaction_Site::adjust_matched_atoms_in_component(
    const extending_resizable_array<int>& xref) {
...
int
Sidechain_Reaction_Site::adjust_matched_atoms_in_component(
    const extending_resizable_array<int>& xref) {
  Reaction_Site::adjust_matched_atoms_in_component(xref);
...
IWReaction::write_msi(std::ostream& os) {
  ...
  if (_find_kekule_forms_for_bad_valence) {
    os << "  (A I " << NAME_OF_FIND_KEKULE_FORM_FOR_INVALID_VALENCES_ATTRIBUTE << ' '
       << _find_kekule_forms_for_bad_valence << ")\n";
  }
  ...
  // Because the reaction stereo centres may have different attribute names, we do them
  // explicitly Hmmm, they should know their attribute name...
...
// #define DEBUG_DO_UNFIX_IMPLICIT_HYDROGENS
  ...
  cerr << "Reaction_Site::do_unfix_implicit_hydrogens:unfixing " << n
       << " implicit hydrogens in " << result.smiles() << '\n';
#endif
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (!embedding.ok_index(j)) {
      cerr << "Reaction_Site::do_unfix_implicit_hydrogens:invalid index " << j
           << " contain " << embedding.number_elements() << '\n';
      abort();
    }
...
IWReaction::_add_molecule(Molecule& result, const Molecule& added) {
  ...
  // cerr << "IWReaction::_add_molecule:_append_names " << _append_names << " name of
  // added " << added.name() << '\n';
  ...
  for (int i = 0; i < ns; i++) {
    if (0 == _sidechains[i]->number_reagents()) {
      if (iwreaction_display_take_first_reagent_from_each_sidechain_warning_message) {
        cerr << "IWReaction::_take_first_reagent_from_each_sidechain:no reagents with "
                "sidechain "
             << i << " in " << _comment << '\n';
      }
      return 0;
    ...
    //  cerr << " i = " << i << '\n';
    //  cerr << _sidechains[i] << " has " << _sidechains[i]->number_reagents() << "
...
IWReaction::perform_reaction(const Molecule* scaffold,
                             ...
                             const Reaction_Iterator& iter, Molecule& result) {
...
int
IWReaction::perform_reaction(const Molecule* scaffold, const Set_of_Atoms* e,
                             const Reaction_Iterator& iter, Molecule& result) {
  Enumeration_Temporaries etmp(_sidechains.number_elements());
...
int
IWReaction::perform_reaction(const Molecule* scaffold,
                             const Substructure_Results& sresults, Molecule& result) {
...
template <typename T>
int
do_transformations(Molecule& result, const Set_of_Atoms& embedding, int offset,
...
template int do_transformations(Molecule&, const Set_of_Atoms&, int,
                                const resizable_array_p<Reaction_Change_Element>&);
template int do_transformations(Molecule&, const Set_of_Atoms&, int,
                                const resizable_array_p<Reaction_Formal_Charge>&);
template int do_transformations(Molecule&, const Set_of_Atoms&, int,
...
int
IWReaction::_do_atom_removals(Molecule& result, Enumeration_Temporaries& etmp) const {
  ...
  result.recompute_implicit_hydrogens();  // do this even if no atoms being removed. We
                                          // may have added an atom to something whose
                                          // implicit H count will need to change

  const resizable_array<const Atom*>& fragments_to_be_removed =
      etmp.fragments_to_be_removed();
  ...
  if (1 == nf) {
    ...
    if (!result.ok_atom_number(j)) {
      cerr << "IWReaction::_do_atom_removals: very bad news, removing fragment " << 0
           << " of " << nf << " fragments to be removed\n";
      cerr << "Result molecule cannot find atom in " << result.smiles() << '\n';
      iwabort();
    }

    if (!result.remove_fragment_containing_atom(j)) {
      cerr << "IWReaction::_do_atom_removals: bad news, could not remove fragment with "
              "atom "
           << j << '\n';
      return 0;
    }
  ...
  for (int i = 0; i < nf; i++) {
    ...
    if (!result.ok_atom_number(j)) {
      cerr << "IWReaction::_do_atom_removals: very bad news, removing fragment " << i
           << " of " << nf << " fragments to be removed\n";
    ...
    if (0 == i) {
      ...
      cerr << "IWReaction::_do_atom_removals: fragment " << jfm
           << " being deleted multiple times, atom " << j << ", OK\n";
      cerr << _comment << '\n';
      continue;
    } else {
      fragments_removed.add(jfm);
...
int
IWReaction::_in_place_transformation(Molecule& m, const Set_of_Atoms* scaffold_embedding,
  ...
  for (int i = 0; i < ns; i++) {
    ...
    if (1 != s->number_reagents()) [[unlikely]] {  /// cannot do in place transformation
      cerr << "IWReaction::_in_place_transformation:sidechain has multiple reagents, "
              "cannot do in place\n";
      return 0;
    }
...
int
IWReaction::in_place_transformation(Molecule& m, const Set_of_Atoms* scaffold_embedding) {
  assert(nullptr != scaffold_embedding);

  if (scaffold_embedding->max_val() >= m.natoms()) {
    cerr << "IWReaction::in_place_transformation:invalid embedding "
         << (*scaffold_embedding) << " for reagent with " << m.natoms() << " atoms\n";
    return 0;
  }
...
int
IWReaction::in_place_transformations(
    Molecule& m, const resizable_array<const Set_of_Atoms*>& embeddings) {
  Enumeration_Temporaries etmp(_sidechains.number_elements());
...
int
IWReaction::in_place_transformations(Molecule& m, const Substructure_Results& sresults) {
  Enumeration_Temporaries etmp(_sidechains.number_elements());
...
IWReaction::_perform_scaffold_transformations(Molecule& result,
  ...
  // cerr << "IWReaction::_perform_scaffold_transformations begin with " <<
  // result.smiles() << '\n';

  do_transformations(result, *scaffold_embedding, 0, _elements_to_change);
  // do_transformations(result, *scaffold_embedding, 0, _formal_charges_to_change);    //
  // done in do_makes_breaks
  ...
  if (_reaction_stereo_centre.empty())
    ...
    cerr << "IWReaction::__perform_scaffold_transformations:reaction stereo centre "
            "problem\n";
    return 0;
  }
...
int
IWReaction::_perform_reaction(Molecule& result, const Set_of_Atoms* scaffold_embedding,
                              Enumeration_Temporaries& etmp) {
  assert(nullptr != scaffold_embedding);
  ...
  if (_toggle_kekule_form.active()) {
    ...
    if (!_toggle_kekule_form.process(result, *scaffold_embedding, changed)) {
      cerr << "IWReaction::_perform_reaction:cannot toggle Kekule form "
           << result.smiles() << '\n';
...
#ifdef DEBUG_PERFORM_REACTION
  cerr << "Are there any chiral centres? " << chiral_centres_present << " saved? "
       << _saved_chiral_centre.number_elements() << '\n';
#endif
  ...
  // Reaction_Site::remove_and_invert_stereo_centres(result, *scaffold_embedding, 0); done
  // in _perform_scaffold_transformations
  for (int i = 0; i < ns; i++) {
    int aap = atoms_in_growing_molecule[i];

    _sidechains[i]->remove_and_invert_stereo_centres(result, reagent[i]->embedding(),
                                                     aap);
  }
...
int
Scaffold_Reaction_Site::DoCopyIsotopes(Molecule& result,
                               ...
                               const Set_of_Atoms * sidechain_embedding)
{
  ...
  if (! _perform_reaction (result, nullptr, scaffold_embedding, sidechain,
sidechain_embedding, atoms_to_be_removed, fragments_to_be_removed)) return 0;
...
/*
  We frequently need to know whether or not a matched atom number is consistent with an
  embedding
*/
                           ...
                           const char * caller)
{
  ...
  cerr << caller << " invalid index. Embedding contains " << embedding.number_elements()
<< " items, so " << i << " is invalid\n"; return 0;
}*/

static int
valid_member_of_embedding(const Set_of_Atoms& embedding, int i, const char* caller,
                          atom_number_t& zatom) {
  if (embedding.ok_index(i)) {
    zatom = embedding[i];
    return 1;
  ...
  if (i < 0) {
    ...
    if (embedding.ok_index(j)) {
      //    cerr << "Matched atom " << i << " becomes atom " << j << " N = " <<
      //    embedding.number_elements() << " atom " << embedding[j] << '\n';
      zatom = embedding[j];
      return 1;
    }
  }
...
int
Reaction_Site::do_makes_breaks(Molecule& result, const Set_of_Atoms& embedding,
                               const int offset, Enumeration_Temporaries& etmp) {
...
#ifdef DEBUG_DO_MAKES_AND_BREAKS
  cerr << "Reaction_Site::do_makes_breaks: doing " << nb << " bond breaks, embedding "
       << embedding << '\n';
  ...
  int* chiral_centre_available = new_int(result.natoms());
  std::unique_ptr<int[]> free_chiral_centre_available(chiral_centre_available);

  if (chiral_centres_present) {
    result.at_centre_of_chiral_centre(chiral_centre_available, 1);

    if (_atoms_to_be_removed.number_elements() > 0) {
      _discern_chiral_centres_to_be_saved_around_removed_atoms(result, embedding, offset,
                                                               chiral_centre_available);
    }
  }
...
//  '\n';
    ...
    //  the bond we got a fatal error. Therefore only try the removal if the atoms are
    //  bonded

    if (result.are_bonded(a1, a2)) {
      result.remove_bond_between_atoms(a1, a2);
    } else if (warn_non_bonded_breaks) {
      cerr << "Reaction_Site::do_makes_breaks: atoms " << a1 << " and " << a2
           << " are not bonded. IGNORED. " << _comment << '\n';
      cerr << result.smiles() << '\n';
      cerr << result.smarts_equivalent_for_atom(a1) << ' '
           << result.smarts_equivalent_for_atom(a2) << '\n';
      // abort();
    }
  }

#ifdef PRINT_SAVED_CHIRAL_CENTRES
  cerr << "Found " << _saved_chiral_centre.number_elements()
       << " chiral centres to be saved\n";
  ...
  for (int i = 0; i < nb; i++) {
    ...
    atom_number_t a1;
    if (!valid_member_of_embedding(embedding, b->a1(),
                                   "Reaction_Site::do_makes_breaks: a1", a1)) {
      return 0;
    }

    atom_number_t a2;
    if (!valid_member_of_embedding(embedding, b->a2(),
                                   "Reaction_Site::do_makes_breaks: a2", a2)) {
      return 0;
    }
    ...
    if (!result.are_bonded(
            a1,
            a2)) {  // new bond - could enable the permanent aromatic thing if I wanted to
    ...
    result.recompute_implicit_hydrogens(
        a1);  // very important to recompute as soon as formed
  ...
  for (const Bond* b : _bonds_to_be_changed) {
    ...
    if (!result.are_bonded(a1, a2)) {
      cerr << "IWReaction::do_makes_breaks:change bond, atoms not bonded "
           << result.name() << '\n';
      return 0;
    } else {
      if (AROMATIC_BOND == b->btype()) {
  ...
  for (int i = 0; i < na; i++) {
    //  cerr << "Embedding contains " << embedding.number_elements() << " items, remove
    //  atom " << _atoms_to_be_removed[i] << '\n';
    ...
    if (atoms_to_be_removed.add_if_not_already_present(a)) {
      ...
      cerr << "Reaction_Site::do_makes_breaks:ignoring duplicate deletion, atom " << a
           << " in '" << result.name() << "'\n";
    }
  }
  ...
  if (nf > 1) {
    ...
    for (int i = 0; i < na; i++) {
      ...
      //    cerr << "Fragment being removed anchored at atom " << j << ' ' <<
      //    result.smarts_equivalent_for_atom(j) << '\n';
  ...
  if (na > 0 && nf > 1) {
    ...
    int* keep_frag = new_int(nf);
    std::unique_ptr<int[]> free_keep_frag(keep_frag);
...
// value matches `rs`
static int
CreateChiralCentreMatchingCip(Molecule& m, atom_number_t zatom, CahnIngoldPrelog rs) {
...
Reaction_Site::do_cahn_ingold_prelog_stereo(Molecule& result,
  ...
  if (!result.ok_atom_number(a)) {
    cerr << "Reaction_Site::do_cahn_ingold_prelog_stereo:invalid atom number in result "
         << cips.atom() << " in " << embedding << " result has " << result.natoms()
         << " atoms\n";
    return 0;
  }
...
int
Reaction_Site::_discern_chiral_centres_to_be_saved_around_removed_atoms(
    Molecule& result, const Set_of_Atoms& embedding, int offset,
    int* chiral_centre_available) {
  const int n = _atoms_to_be_removed.number_elements();

  // cerr <<
  // "Reaction_Site::_discern_chiral_centres_to_be_saved_around_removed_atoms:checking "
  // << n << " atoms to be remove, offset " << offset << '\n';
...
int
Reaction_Site::_do_invert_stereo_centres(Molecule& result, const Set_of_Atoms& embedding,
                                         int offset) const {
#ifdef DEBUG_DO_INVERT_STEREO_CENTRES
  cerr << "Reaction_Site::_do_invert_stereo_centres: inverting "
       << _stereo_centres_to_invert.number_elements() << " atoms\n";
...
#ifdef DEBUG_DO_INVERT_STEREO_CENTRES
    cerr << "Embedding member " << m << " is atom " << a << " present "
         << result.chiral_centre_at_atom(a) << " offset = " << offset << '\n';
#endif
...
int
Reaction_Site::_do_remove_stereo_centres(Molecule& result, const Set_of_Atoms& embedding,
                                         int offset) const {
...
int
Sidechain_Reaction_Site::add_reagents(const char* fname, FileType input_type,
                                      const Sidechain_Match_Conditions& smc) {
  if (FILE_TYPE_INVALID == input_type) {
    input_type = discern_file_type_from_name(fname);
    if (0 == input_type) {
      cerr << "Sidechain_Reaction_Site::add_reagents: cannot discern input type '"
           << fname << "'\n";
      return 0;
    }
  }
...
Sidechain_Reaction_Site::add_reagents(data_source_and_type<Molecule_and_Embedding>& input,
  ...
  while (nullptr != (m = input.next_molecule())) {
    ...
    if (!add_reagent(m, smc)) {
      cerr << "Sidechain_Reaction_Site::add_reagents: fatal error processing '"
           << m->name() << "'\n";
...
Sidechain_Reaction_Site::add_reagent(Molecule_and_Embedding* m,
  ...
  if (0 == nhits) {  // cannot be used by this sidechain
    ...
    cerr << "Sidechain_Reaction_Site::add_reagent::too many hits " << nhits << " (max "
  ...
  // cerr << "Sidechain_Reaction_Site::add_reagent:_matched_atom_changed? " <<
  // _matched_atom_changed << '\n';
  ...
  if (smc.process_hit_number() >= 0) {
    if (nhits < smc.process_hit_number() + 1)  // cannot be used by this sidechain
    {
      ...
      cerr << "hit number to process (" << smc.process_hit_number()
           << ") is out of range - #hits= " << nhits << '\n';
  ...
  if (smc.make_new_reagent_for_each_hit()) {
    ...
    for (int i = 1; i < nhits; i++) {
      ...
      if (_toggle_kekule_form.active() &&
          !mcopy->do_toggle_kekule_form(_toggle_kekule_form)) {
        return 0;
      }
...
void
Sidechain_Reaction_Site::MaybeIssueNoHitsWarning(
    const Molecule_and_Embedding& m, const Substructure_Results& sresults,
...
Sidechain_Reaction_Site::add_reagent(const Molecule& m,
                                     const Sidechain_Match_Conditions& smc) {
  std::unique_ptr<Molecule_and_Embedding> tmp =
      std::make_unique<Molecule_and_Embedding>(m);
...
int
Sidechain_Reaction_Site::add_reagent_embedding_identified(
    Molecule_and_Embedding* m, const Sidechain_Match_Conditions& smc) {
  if (_toggle_kekule_form.active() && !m->do_toggle_kekule_form(_toggle_kekule_form)) {
    return 0;
  }
...
int
Sidechain_Reaction_Site::do_makes_breaks(Molecule& result, const Set_of_Atoms& embedding,
                                         int offset, Enumeration_Temporaries& etmp) {
...
determine_atom_number(const Set_of_Atoms& scaffold_embedding,
                      ...
                      const Enumeration_Temporaries& etmp, const char* caller,
                      atom_number_t& zresult) {
  if (ma.in_scaffold()) {
    return valid_member_of_embedding(scaffold_embedding, ma.matched_atom(), caller,
                                     zresult);
  }
  ...
  if (!valid_member_of_embedding(e, ma.matched_atom(),
                                 "Sidechain_Reaction_Site::make_inter_particle_bonds: a1",
                                 zresult)) {
    return 0;
  }
...
#ifdef DEBUG_DETERMINE_ATOM_NUMBER
  cerr << "Growing molecule " << c << " had " << etmp.atoms_in_growing_molecule()[c]
       << " atoms\n";
...
IWReaction::_make_inter_particle_bond(Molecule& result,
  ...
  cerr << "IWReaction::_make_inter_particle_bond: bond " << b.a1() << " to " << b.a2()
       << " embedding " << *scaffold_embedding << '\n';
#endif

  atom_number_t a1;
  if (!determine_atom_number(*scaffold_embedding, b.a1(), etmp,
                             "IWReaction::_make_inter_particle_bond: a1", a1)) {
    return 0;
  }

  atom_number_t a2 = kInvalidAtomNumber;
  if (!determine_atom_number(*scaffold_embedding, b.a2(), etmp,
                             "IWReaction::_make_inter_particle_bond: a1", a2)) {
    return 0;
  }
  ...
  if (b.align_3d() > 0.0f) {
    if (!lillymol::Position3D(result, a1, b.align_3d(), a2)) {
      cerr << "IWReaction::_make_inter_particle_bond:cannot 3d align atoms " << a1
           << result.smarts_equivalent_for_atom(a1) << " and " << a2
           << result.smarts_equivalent_for_atom(a2) << '\n';
    }
  ...
  if (!result.add_bond(a1, a2, b.btype())) {
    cerr << "IWReaction::_make_inter_particle_bond:cannot make bond between " << a1
         << " and " << a2 << '\n';
    return 0;
  }
  result.set_implicit_hydrogens_known(a1, 0);
...
int
IWReaction::_set_bond_length(Molecule& result, const Set_of_Atoms* scaffold_embedding,
  ...
  atom_number_t a1;
  if (!determine_atom_number(*scaffold_embedding, b.a1(), etmp,
                             "IWReaction::_set_bond_length: a1", a1)) {
    return 0;
  }

  atom_number_t a2;
  if (!determine_atom_number(*scaffold_embedding, b.a2(), etmp,
                             "IWReaction::_set_bond_length: a1", a2)) {
    return 0;
  }

#ifdef DEBUG_SET_BOND_LENGTH
  cerr << "IWReaction::_set_bond_length:setting bond between " << a1 << " and " << a2
       << " to length " << b.desired_length() << '\n';
  cerr << result.smarts_equivalent_for_atom(a1) << " and "
       << result.smarts_equivalent_for_atom(a2) << '\n';
...
int
IWReaction::_set_bond_lengths(Molecule& result, const Set_of_Atoms* scaffold_embedding,
                              const Enumeration_Temporaries& etmp,
                              const Reaction_Site& r) const {
  int nb = r.number_bond_length_specifications();

  // cerr << "IWReaction::_set_bond_lengths:setting " << nb << " reaction bond length
  // specifications\n";
...
int
IWReaction::_set_dihedral_angles(Molecule& result, const Set_of_Atoms* scaffold_embedding,
                                 const Enumeration_Temporaries& etmp,
                                 const Reaction_Site& r) const {
...
int
IWReaction::_set_bond_angles(Molecule& result, const Set_of_Atoms* scaffold_embedding,
                             const Enumeration_Temporaries& etmp,
                             const Reaction_Site& r) const {
...
int
IWReaction::_do_3d_replacements(Molecule& result, const Set_of_Atoms* scaffold_embedding,
                                const Enumeration_Temporaries& etmp,
                                const Reaction_Site& r) const {
...
static int
InvertChiralCentreInvolving(Molecule& m, atom_number_t zatom) {
...
int
Scaffold_Reaction_Site::_do_intra_particle_replacements(
    Molecule& result, const Set_of_Atoms* scaffold_embedding) const {
  // cerr << "Scaffold_Reaction_Site:have " << _replace_atom.number_elements() << "
  // replacements\n";
  ...
  for (const Replace_Atom* r : _replace_atom) {
    ...
    atom_number_t ma1;
    if (!valid_member_of_embedding(
            *scaffold_embedding, m1,
            "Scaffold_Reaction_Site::_do_intra_particle_replacements:", ma1)) {
      return 0;
    }

    atom_number_t ma2;
    if (!valid_member_of_embedding(
            *scaffold_embedding, m2,
            "Scaffold_Reaction_Site::_do_intra_particle_replacements:", ma2)) {
      return 0;
    }
...
int
IWReaction::_do_replacement(Molecule& result, const Set_of_Atoms* scaffold_embedding,
...
int
Replace_Atom::DoReplacement(Molecule& result, const Set_of_Atoms* scaffold_embedding,
                            const Enumeration_Temporaries& etmp) const {
  atom_number_t a1;
  if (!determine_atom_number(*scaffold_embedding, _a1, etmp,
                             "Replace_Atom::DoReplacement:a1", a1)) {
    return 0;
  }

  atom_number_t a2;
  if (!determine_atom_number(*scaffold_embedding, _a2, etmp,
                             "Replace_Atom::DoReplacement:a2", a2)) {
    return 0;
  }

  // cerr << "Atoms are " << a1 << " and " << a2 << '\n';
  if (!result.stereo_preserving_substitute(a1, a2)) {
    cerr << "Replace_Atom::DoReplacement:failed, scaffold embedding "
         << (*scaffold_embedding) << '\n';
    cerr << *this << '\n';
    return 0;
  }
...
int
Reaction_Site::DoReplacements(Molecule& result, const Set_of_Atoms* scaffold_embedding,
                              Enumeration_Temporaries& etmp) const {
...
int
IWReaction::_do_replacements(Molecule& result, const Set_of_Atoms* scaffold_embedding,
                             const Enumeration_Temporaries& etmp) const {
  for (const Sidechain_Reaction_Site* s : _sidechains) {
...
No_Reaction::construct_from_msi_object(const msi_object& msi) {
  ...
  for (int i = 0; i < na; i++) {
    ...
    if (NAME_OF_QUERY_SMARTS_ATTRIBUTE == att->name()) {
      ...
      cerr << "No_Reaction::construct_from_msi_object: unrecognised attribute '"
           << att->name() << "'\n";
      return 0;
    }
...
int
No_Reaction::write_msi(std::ostream& os, int& object_id, int indentation) {
  IWString ind;
  ind.extend(indentation, ' ');
...
Reaction_Stereo_Centre::construct_from_msi_attribute(const msi_attribute* msi) {
  ...
  if (nw < 4 || nw > 5) {
    cerr << "Reaction_Stereo_Centre::construct_from_msi_attribute: attribute must have 4 "
            "or 5 tokens\n";
    cerr << m << '\n';
    return 0;
  }
  ...
  for (int i = 0; i < nw; i++) {
    ...
    if (!_ssc[i].construct(token)) {
      cerr << "Reaction_Stereo_Centre::construct_from_msi_attribute: cannot parse "
              "attribute "
           << i << " '" << token << "'\n";
      cerr << m << '\n';
      return 0;
...
int
IWReaction::_make_stereo_centres(Molecule& result, const Set_of_Atoms* scaffold_embedding,
                                 const Enumeration_Temporaries& etmp) const {
...
IWReaction::_make_cip_stereo_centres(Molecule& result,
                                     const Set_of_Atoms& scaffold_embedding,
                                     const Enumeration_Temporaries& etmp) const {
  //  cerr << "Processing " << _cip_stereo.size() << " CIP stereo centres,
  //  scaffold_embedding " << scaffold_embedding << '\n';
...
/*
  If we detect an invalid valence involving atoms we have added, try to find a different
  Kekule form. To make things easy, we only check for 5 bonded carbons in 6 membered rings
*/
...
//    continue;
    ...
    if (! result.in_ring_of_given_size (i, 6))     // only 6 membered aromatic rings have
Kekule forms continue;
...
static int
could_extend_aromatic_system(Molecule& m, const Ring& r, const int* process_these) {
  int new_double_bonds = 0;  // to possibly be aromatic, must have at least 1 double bond
                             // not already fully counted
  ...
  for (Ring_Bond_Iterator i(r); i != r.zend(); i++) {
    ...
    if (m.hcount(a1) > 1) {  // could imagine cases where this is wrong, but hopefully too
                             // rare to worry about
      return 0;
    }
  }
...
static int
identify_fused_system(Molecule& m, const Ring& r, int* process_these) {
...
static int
find_kekule_forms_for_bad_valence(Molecule& m, atom_number_t c, int* process_these) {
...
static int
find_kekule_forms_for_bad_valence(Molecule& m, atom_number_t c) {
...
static int
convert_implicit_hydrogen_to_explicit_connection(Molecule& m, Chiral_Centre& c) {
...
int
Reaction_Stereo_Centre::_find_missing_connection(Molecule& m, atom_number_t zatom,
  ...
  for (int i = 0; i < acon; i++) {
    ...
    //  cerr << "Atom " << zatom << " bonded to atom " << j << " involves? " <<
    //  c->involves(j) << '\n';
    if (c->involves(j)) {
      continue;
    }
...
static void
report_chiral_centre_status(const Molecule& m, atom_number_t a1, atom_number_t a2,
                            const char* origin) {
  if (a2 < 0) {
    return;
  }
  ...
  cerr << "Chiral centre component atoms " << a1 << " and " << a2 << " not bonded "
       << origin << '\n';
...
static int
chiral_component_bonded(const Atom* a, atom_number_t k) {
  if (k < 0) {
    return 1;
  }
...
static int
all_parts_of_chiral_centre_bonded_to_centre(const Molecule& m, const Chiral_Centre* c) {
  const Atom* a = m.atomi(c->a());
...
int
Reaction_Stereo_Centre::process(Molecule& result, const Set_of_Atoms* scaffold_embedding,
                                const Enumeration_Temporaries& etmp) const {
  atom_number_t a;
  if (!determine_atom_number(*scaffold_embedding, _ssc[0], etmp,
                             "Reaction_Stereo_Centre::process: a", a)) {
    return 0;
  }
  ...
  if (result.ncon(a) < 3) {
    cerr << "Reaction_Stereo_Centre::make_stereo_centre: atom " << a << " has "
         << result.ncon(a) << " connections. Impossible\n";
  ...
  } else if (!determine_atom_number(*scaffold_embedding, _ssc[1], etmp,
                                    "Reaction_Stereo_Centre::process: top front", a)) {
    return 0;
  } else {
    c->set_top_front(a);
  }
  ...
  } else if (!determine_atom_number(*scaffold_embedding, _ssc[2], etmp,
                                    "Reaction_Stereo_Centre::process: top back", a)) {
    return 0;
  } else {
    c->set_top_back(a);
  }
  ...
  } else if (!determine_atom_number(*scaffold_embedding, _ssc[3], etmp,
                                    "Reaction_Stereo_Centre::process: left down", a)) {
    return 0;
  } else {
    c->set_left_down(a);
  }
  ...
  // For the last connection, we may not know whether there is an atom or an implicit
  // hydrogen
  ...
  if (_ssc[4].is_whatever()) {
    ...
    c->set_right_down(kChiralConnectionIsImplicitHydrogen);
  } else if (!determine_atom_number(*scaffold_embedding, _ssc[4], etmp,
                                    "Reaction_Stereo_Centre::process: right down", a)) {
    return 0;
  } else {
    c->set_right_down(a);
  }
  ...
  if (0 == ih && 4 == acon)  // good
    ...
    cerr << "Reaction_Stereo_Centre::process:atom " << c->a()
         << " connectivity mismatch acon " << acon << " ih " << ih << "\n";
    c->debug_print(cerr);
    delete c;
    cerr << result.smiles() << '\n';
  ...
  if (!result.add_chiral_centre(c)) {
    cerr << "Reaction_Site::make_stereo_centre:: cannot add chiral centre to result "
            "molecule\n";
    c->debug_print(cerr);
    delete c;
...
Reaction_Wedge_Bond::construct_from_msi_attribute(const msi_attribute* att) {
  if (3 != att->number_int_values()) {
    cerr << "Reaction_Wedge_Bond::construct_from_msi_attribute: attribute must have 3 "
            "integer values\n";
    return 0;
  }
  ...
  if (Pair_of_Atoms::a1() == Pair_of_Atoms::a2()) {
    cerr << "Reaction_Wedge_Bond::construct_from_msi_attribute: atoms the same - "
            "impossible\n";
    return 0;
  }
...
int
Reaction_Wedge_Bond::write_msi(std::ostream& os, const IWString& ind,
                               const const_IWSubstring& attribute_name) const {
  os << ind << "  (A I " << attribute_name << " (" << _a1 << ' ' << _a2 << ' '
     << _direction << "))\n";
...
int
Reaction_Wedge_Bond::process(Molecule& result, const Set_of_Atoms& embedding,
                             int offset) const {
  atom_number_t a1 = embedding[_a1] + offset;
  atom_number_t a2 = embedding[_a2] + offset;

  if (!result.are_bonded(a1, a2)) {
    cerr << "Reaction_Wedge_Bond::do_place_wedge_bonds: atoms " << a1 << " and " << a2
         << " are not bonded\n";
    return 0;
  }

  // cerr << "Placing " << _direction << " wedge bond between atoms " << a1 << " and " <<
  // a2 << '\n';

  if (!result.set_wedge_bond_between_atoms(a1, a2, _direction)) {
    cerr << "Reaction_Wedge_Bond::do_place_wedge_bonds: cannot place wedge bond between "
            "atoms "
         << a1 << " and " << a2 << '\n';
    return 0;
  }
...
int
IWReaction::determine_matched_atoms(Molecule& m, Substructure_Results& sresults) {
...
//#define DEBUG_DO_TOGGLE_KEKULE_FORM
  ...
  for (int i = 0; i < nhits; i++)
  {
    ...
    if (! _toggle_kekule_form.process (m, *e, changed))
    {
      cerr << "Reaction_Site::_do_toggle_kekule_form:toggle kekule form failed for '" <<
m.name() << "'\n";
    }
...
// #define DEBUG_DO_RESTORE_SAVED_CHIRAL_CENTRES
  ...
  cerr << "Reaction_Site::do_restore_saved_chiral_centres:restoring " << n
       << " saved chiral centres\n";
#endif
...
Reaction_Site::_do_restore_saved_chiral_centre(Molecule& result,
  ...
  /* If the reaction is placing an explicit chiral centre on ZATOM, don't do anything
    ...
    for (int i = 0; i < _reaction_stereo_centre.number_elements(); i++)
    {
      ...
      if (! determine_atom_number (scaffold_embedding, scci, etmp,
  "IWReaction::_do_restore_saved_chiral_centre", a)) continue;
  ...
  if (4 == a->ncon())  // could still be chiral
    ;
  else if (3 == a->ncon() && 3 == a->nbonds() && 1 == result.implicit_hydrogens(zatom) &&
           1 == result.hcount(zatom))  // could still be chiral
    ;
  ...
  // The same atoms still attached, and there were 4 of them - should not happen�- we know
  // a bond was broken

  if (atoms_no_longer_attached.empty() &&
      4 == atoms_in_chiral_centre)  // should not happen, all atoms the same, but we know
                                    // a bond was broken? Maybe they broke and re-formed
                                    // something???
  {
    cerr << "IWReaction::_possibly_add_back_chiral_centre:strange, nothing has changed "
            "at atom "
         << zatom << '\n';
    return 1;
  }
...
#ifdef DEBUG_DO_RESTORE_SAVED_CHIRAL_CENTRES
  cerr << "After reaction " << atoms_no_longer_attached.number_elements()
       << " atoms no longer attached, " << atoms_newly_appeared.number_elements()
       << " atoms newly attached\n";
#endif

  // The case where an atom is replacing an implicit Hydrogen. All atoms the same, only 3
  // explicit atoms in the chiral centre, but now 4 connections
  ...
  if (1 == atoms_newly_appeared.number_elements() &&
      1 == atoms_no_longer_attached.number_elements()) {
  ...
  for (int i = 0; i < _replace_atom.number_elements(); i++) {
    ...
    atom_number_t a1;
    if (!determine_atom_number(scaffold_embedding, r->a1(), etmp,
                               "IWReaction::_do_restore_saved_chiral_centres: a1", a1)) {
      continue;
    }
    ...
    atom_number_t a2;
    if (!determine_atom_number(scaffold_embedding, r->a2(), etmp,
                               "IWReaction::_do_restore_saved_chiral_centres: a2", a2)) {
      continue;
    }
  ...
  // If there is one unmatched newly appeared atom, and the chiral centre had a lone pair,
  // make that match
...
int
CopyIsotope::Process(Molecule& result, const Set_of_Atoms& scaffold_embedding,
                     const Set_of_Atoms& sidechain_embedding,
                     const Enumeration_Temporaries& etmp) const {
  const int* atoms_in_growing_molecule = etmp.atoms_in_growing_molecule();

#ifdef DEBUG_COPY_ISOTOPE_PROCESS
  cerr << "CopyIsotope::process " << scaffold_embedding << " sidechain_embedding "
       << sidechain_embedding << '\n';
  cerr << "_from.in_scaffold " << _from.in_scaffold() << " _to.in_scaffold "
       << _to.in_scaffold() << '\n';
#endif
