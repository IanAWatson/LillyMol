...
Single_Substructure_Query::_default_values() {
  ...
  _environment_must_match_unmatched_atoms =
      file_scope_environment_must_match_unmatched_atoms;
  ...
  // special flag to indicate unknown. Resolved during first use
  _global_conditions_present = -1;

  // none of our root atoms have symmetry grouping info
...
Single_Substructure_Query::Single_Substructure_Query() : _max_query_atoms_matched(0) {
  _default_values();
...
#ifdef SHOW_SSQ_OK
  cerr << "Atoms are OK, check embeddings " << _embeddings.number_elements() << " and "
       << _query_atoms_matched.number_elements() << '\n';
#endif
...
Single_Substructure_Query::debug_print(std::ostream& os, const IWString& indentation) {
  ...
  if (_environment.number_elements()) {
    os << indentation << "Query contains " << _environment.number_elements()
       << " environment members\n";
  }
...
void
Single_Substructure_Query::_collect_all_atoms(
    extending_resizable_array<Substructure_Atom*>& completed) const {
...
Single_Substructure_Query::_determine_if_unmatched_atom_counts_are_present() {
  ...
  // cerr << "_determine_if_unmatched_atom_counts_are_present found " <<
  // _unmatched_atoms_attached_specified << '\n';
...
void
Single_Substructure_Query::_determine_if_fragment_ids_present() {
  _fragment_ids_present = 0;
  for (const Substructure_Atom* r : _root_atoms) {
    _fragment_ids_present += r->number_fragment_ids();
  }

  // Maybe this should be fatal.
  if (_fragment_ids_present == 1) {
    cerr << "Single_Substructure_Query::_determine_if_fragment_ids_present: only one"
            " fragment id specified - ignored for now\n";
  }
}
...
Single_Substructure_Query::_determine_if_symmetry_groups_present() {
  ...
  // cerr << "Single_Substructure_Query::_determine_if_symmetry_groups_present:checking "
  // << n << " root atoms\n";
  ...
  // cerr << "Single_Substructure_Query::_determine_if_symmetry_groups_present " <<
  // _first_root_atom_with_symmetry_group << '\n';
...
Single_Substructure_Query::set_find_unique_embeddings_only(int s) {
  if (0 == _save_matched_atoms && s > 0) {
    cerr << "Single_Substructure_Query::set_find_unique_embeddings_only: must save "
            "embeddings\n";
    return 0;
  }
...
Single_Substructure_Query::set_do_not_perceive_symmetry_equivalent_matches(int s) {
  if (0 == _save_matched_atoms && s) {
    cerr << "Single_Substructure_Query::set_do_not_perceive_symmetry_equivalent_matches: "
            "must save matched atoms\n";
    return 0;
  }
...
Single_Substructure_Query::set_save_matched_atoms(int s) {
  if (_do_not_perceive_symmetry_equivalent_matches && 0 == s) {
    cerr << "Single_Substructure_Query::set_save_matched_atoms: must save embeddings to "
            "determine symmetry equivalency\n";
    return 0;
  }
  ...
  if ((_sort_matches_by || _sort_by_preference_value) && 0 == s) {
    cerr << "Single_Substructure_Query::set_save_matched_atoms: must have embeddings to "
            "order preferences\n";
    return 0;
  }
...
Single_Substructure_Query::set_max_matches_to_find(int m) {
  ...
  if (_hits_needed.is_set()) {
    cerr << "Single_Substructure_Query::set_max_matches_to_find:not set due to "
            "_hits_needed\n";
  } else {
...
Single_Substructure_Query::_has_implicit_rings(Query_Atoms_Matched& matched_query_atoms,
  ...
  for (int i = 0; i < nm; i++) {
    ...
    for (int j = i + 1; j < nm; j++) {
      ...
      if (ai == aj->parent()) {  // for linear chains this won't happen, but leave the
                                 // condition here in case we generalise this
        continue;
      }
      ...
      if (!aj->has_ring_closure_bond_to(ai)) {  // atoms are bonded, but no ring closure
                                                // bond. Therefore an implicit ring
        return 1;
      }
    }
  }
...
int
Single_Substructure_Query::_fused_system_id_conditions_satisfied(
...
// #define DEBUG_CHECK_FUSED_SYSTEM_IDS
  ...
  cerr << "Checking " << nm << " matched atoms for fused system ids '" << m->name()
       << "', " << m->natoms() << " atoms\n";
...
#ifdef DEBUG_CHECK_FUSED_SYSTEM_IDS
    cerr << "Atom " << ai << " " << m->atomic_symbol(ai) << " fsid " << fsidi
         << " in a ring? " << m->is_ring_atom(ai) << '\n';
#endif

    //  if (! m->is_ring_atom(ai))     feb 2016. Very important change. fsids are useful
    //  to specify an atom that is not in the same ring system as another
    //    return 0;
...
#ifdef DEBUG_CHECK_FUSED_SYSTEM_IDS
      //    cerr << "atoms " << ai << ' ' << m->atomic_symbol(ai) << " fsidi " << fsidi <<
      //    " and " << aj << ' ' << m->atomic_symbol(aj) << " fsidj " << fsidj << "
      //    checking same ring system\n";
      cerr << "atoms " << ai << ' ' << m->atomic_symbol(ai) << " fsidi " << fsidi
           << " and " << aj << ' ' << m->atomic_symbol(aj) << " fsidj " << fsidj
           << " same ring system " << m->in_same_ring_system(ai, aj) << '\n';
#endif
...
int
Single_Substructure_Query::_ring_id_conditions_satisfied(
    Query_Atoms_Matched& matched_query_atoms) const {
  int nm = matched_query_atoms.number_elements();
...
#ifdef DEBUG_CHECK_RING_IDS
      cerr << "atoms " << ai << " ridi " << ridi << " and " << aj << " ridj " << ridj
           << " same ring " << m->in_same_ring(ai, aj) << '\n';
#endif
...
int
Single_Substructure_Query::_spinach_match_requirements_satisfied(
    Query_Atoms_Matched& matched_query_atoms, Molecule_to_Match& target_molecule) const {
...
// #define DEBUG_SPINACH_MATCH_REQUIREMENTS_SATISFIED
    ...
    cerr << "Query atom " << sai->unique_id() << " matched with atom " << matched_to
         << " spinach? " << target_molecule.is_spinach(matched_to) << '\n';
#endif
...
int
Single_Substructure_Query::_fragment_id_conditions_satisfied(
    Query_Atoms_Matched& matched_query_atoms) const {
  int nm = matched_query_atoms.number_elements();
  ...
  cerr << "_fragment_id_conditions_satisfied:examining " << nm
       << " matched query atoms\n";
#endif
...
#ifdef DEBUG_FRAGMENT_ID_CONDITIONS_SATISFIED
      cerr << " fragment match conditions i = " << i << " atom "
           << ai->current_hold_atom()->atom_number() << " mfi " << mfi << " fi " << fi
           << " j = " << j << " atom " << aj->current_hold_atom()->atom_number()
           << " mfj " << mfj << " fj " << fj << '\n';
#endif
...
int
Single_Substructure_Query::_embedding_ncon_satisfied(
    Query_Atoms_Matched& matched_query_atoms, const int* already_matched) const {
  if (!_ncon.is_set()) {
    return 1;
  }
  ...
  for (int i = 0; i < nm; i++) {
    ...
    for (int j = 0; j < acon; j++) {
      const Target_Atom* aj =
          a->other(j)
              .other();  // Target_Atom::other() returns a Bond_and_Target_Atom object
...
int
Single_Substructure_Query::_attached_heteroatom_count_satisfied(
    Query_Atoms_Matched& matched_query_atoms, const int* already_matched) const {
...
#ifdef DEBUG_ATTACHED_HETEROATOM_COUNT
    cerr << "Query atom " << q->unique_id() << " matched with atom " << a->atom_number()
         << " acon = " << acon << '\n';
#endif
...
#ifdef DEBUG_ATTACHED_HETEROATOM_COUNT
      cerr << "  atom " << aj->atom_number() << " z = " << aj->atomic_number()
           << " is attached";
      if (already_matched[aj->atom_number()]) {
...
#ifdef DEBUG_ATTACHED_HETEROATOM_COUNT
  cerr << "Final attached heteroatom count is " << ahc << " match is "
       << _attached_heteroatom_count.matches(ahc) << '\n';
#endif
...
int
Single_Substructure_Query::_ring_atoms_matched_satisfied(
...
int
Single_Substructure_Query::_heteroatoms_matched_satisfied(
    Query_Atoms_Matched& matched_atoms) const {
...
// embedding.
int
Single_Substructure_Query::_global_query_conditions_also_matched(
    Query_Atoms_Matched& matched_query_atoms, const int* already_matched,
  ...
  if (_fused_system_ids_present &&
      !_fused_system_id_conditions_satisfied(matched_query_atoms)) {
    return 0;
  }

  if (_spinach_match_requirements_present &&
      !_spinach_match_requirements_satisfied(matched_query_atoms, target_molecule)) {
    return 0;
  }

#ifdef DEBUG_MATCHED_ATOM_POST_CHECK
  cerr << "Checking " << _element_hits_needed.number_elements()
       << " element hits needed\n";
#endif
  ...
  if (_heteroatoms_matched.is_set() &&
      !_heteroatoms_matched_satisfied(matched_query_atoms)) {
    return 0;
  }

  if (_ring_atoms_matched.is_set() &&
      !_ring_atoms_matched_satisfied(matched_query_atoms)) {
    return 0;
  }
...
#ifdef DEBUG_MATCHED_ATOM_POST_CHECK
  cerr << "_unmatched_atoms_attached_specified value "
       << _unmatched_atoms_attached_specified << '\n';
#endif
  if (_unmatched_atoms_attached_specified) {
    if (!_unmatched_atoms_attached_matched(matched_query_atoms, already_matched,
                                           target_molecule)) {
      return 0;
    }
  }

  if (matched_query_atoms
          .empty()) {  // don't know how to handle the unmatched atoms stuff
    return 1;
  }
  ...
  if (static_cast<float>(0.0) != _min_fraction_atoms_matched ||
    ...
    float f = static_cast<float>(matched_query_atoms.number_elements()) /
              static_cast<float>(m->natoms());
  ...
  if (_geometric_constraints.number_elements() > 0) [[unlikely]] {
    ...
    for (const geometric_constraints::SetOfGeometricConstraints* constraint :
...
int
Single_Substructure_Query::_symmetry_group_specifications_matches(
    const Query_Atoms_Matched& matched_query_atoms, Molecule_to_Match& target) const {
  ...
  cerr << "Single_Substructure_Query::_symmetry_group_specifications_matches: " << n
       << " matched atoms\n";
#endif
...
#ifdef DEBUG_SYMMETRY_GROUP_SPECIFICATIONS_MATCHES
    cerr << " i = " << i << " symmetry group " << si << ", atom " << mi << " symm "
         << msim[mi] << '\n';
#endif
...
int
Single_Substructure_Query::AllMatchesFractionAtomsMatched(
    const Substructure_Results& sresults, Molecule_to_Match& target_molecule) const {
...
Single_Substructure_Query::_add_embedding(Query_Atoms_Matched& matched_atoms,
  ...
  {
    cerr << "Adding embedding matched atoms contains " << matched_atoms.number_elements()
         << " embedding " << new_embedding->number_elements() << '\n';
    cerr << (*new_embedding) << '\n';
    return 0;
  }
...
int
Single_Substructure_Query::_find_next_root_atom_embedding(
    Query_Atoms_Matched& matched_atoms, Molecule_to_Match& target_molecule,
  ...
  for (int i = istart; i < istop; i++) {
    ...
    int tmp = _find_embedding(target_molecule, a, matched_atoms, already_matched,
...
Single_Substructure_Query::_make_new_embedding(
  ...
  cerr << "Scanning " << number_matched_atoms
       << " matched atoms, _respect_initial_atom_numbering "
       << _respect_initial_atom_numbering << '\n';
#endif
...
Single_Substructure_Query::_got_embedding(Query_Atoms_Matched& matched_atoms,
                                          ...
                                          Substructure_Results& results) {
#ifdef DEBUG_GOT_EMBEDDING
  cerr << "Got embedding, " << matched_atoms.number_elements()
       << " atoms,  iroot = " << _iroot << " nroot = " << _root_atoms.number_elements()
       << '\n';
#endif

  if (_iroot < _root_atoms.number_elements() - 1) {
#ifdef DEBUG_GOT_EMBEDDING
    cerr << "iroot " << _iroot << ", " << matched_atoms.number_elements()
         << " atoms matched\n";
#endif

    return _find_next_root_atom_embedding(matched_atoms, target_molecule, already_matched,
                                          matched_by_global_specs, results);
  }

#ifdef DEBUG_GOT_EMBEDDING
  cerr << "Processing embedding with " << matched_atoms.number_elements()
       << " atoms matched: ";
  for (int i = 0; i < matched_atoms.number_elements(); i++) {
  ...
  if (!_global_query_conditions_also_matched(matched_atoms, already_matched,
                                             target_molecule)) {
    return 0;
  }

  if (_chirality.number_elements() &&
      !_chiral_atoms_matched(matched_atoms, target_molecule)) {
    return 0;
  }
...
#ifdef DEBUG_GOT_EMBEDDING
  cerr << "Got embedding " << *new_embedding << ", find unique? "
       << _find_unique_embeddings_only << '\n';
#endif
  ...
  if (_embeddings_do_not_overlap &&
      results.embedding_overlaps_previous_embeddings(*new_embedding)) {
#ifdef DEBUG_GOT_EMBEDDING
...
int
remove_atoms_with_same_or(Query_Atoms_Matched& matched_atoms, const int istart,
                          const int orid) {
  int rc = 0;
  int number_matched_atoms = matched_atoms.number_elements();
...
int
Single_Substructure_Query::_find_embedding(
    Molecule_to_Match& target_molecule, Query_Atoms_Matched& matched_atoms,
    int* already_matched, std::unique_ptr<int[]>& matched_by_global_specs,
    Substructure_Atom* root_atom, Substructure_Results& results) {
  ...
  cerr << "_find_embedding: " << number_initially_matched
       << " atoms initially matched, have " << results.number_embeddings()
       << " embeddings\n";
#endif
  ...
  int atom_to_process =
      number_initially_matched + 1;  // before we do the add - C++ arrays start with 0

  if (0 == root_atom->add_your_children(matched_atoms))  // single atom query
  {
    already_matched[root_atom->atom_number_matched()] = 1;
#ifdef DEBUG_FIND_EMBEDDING
    cerr << "_find_embedding:single atom query, matched, have "
         << results.number_embeddings() << " embeddings so far\n";
#endif
    return _got_embedding(matched_atoms, target_molecule, already_matched,
                          matched_by_global_specs, results);
  }
  ...
  while (
      atom_to_process >
      number_initially_matched)  // we ignore any which may have previously been matched
  {
...
#ifdef DEBUG_FIND_EMBEDDING
        cerr << "AlL query atoms matched, calling _got_embedding, B4 have "
             << results.number_embeddings() << " rc " << rc << '\n';
#endif
        if (_got_embedding(matched_atoms, target_molecule, already_matched,
                           matched_by_global_specs, results)) {
          rc++;

#ifdef DEBUG_FIND_EMBEDDING
          cerr << "Rc incremented to " << rc << " have " << results.number_embeddings()
               << " embeddings\n";
...
int
Single_Substructure_Query::_find_embedding(
    Molecule_to_Match& target_molecule, Target_Atom& a,
    Query_Atoms_Matched& matched_atoms, int* already_matched,
    std::unique_ptr<int[]>& matched_by_global_specs, Substructure_Atom* root_atom,
    Substructure_Results& results) {
...
#ifdef DEBUG_FIND_EMBEDDING
  cerr << "Single_Substructure_Query::_find_embedding:atom " << a.atom_number()
       << " begin have " << results.number_embeddings() << " embdddings\n";
#endif

  int rc = _find_embedding(target_molecule, matched_atoms, already_matched,
                           matched_by_global_specs, root_atom, results);

#ifdef DEBUG_FIND_EMBEDDING
  cerr << "Single_Substructure_Query::_find_embedding:atom " << a.atom_number()
       << " returning " << rc << " have " << results.number_embeddings()
       << " embdddings\n";
#endif
...
Single_Substructure_Query::_adjust_for_internal_consistency() {
  ...
  if (!create_molecule(tmp)) {
    cerr << "Single_Substructure_Query::_adjust_for_internal_consistency: create "
            "molecule failed\n";
    return 0;
  }
...
uint32_t
Single_Substructure_Query::_substructure_search(
    Molecule_to_Match& target_molecule, int* already_matched,
    std::unique_ptr<int[]>& matched_by_global_specs, Substructure_Results& results) {
  ...
  {
    if (already_matched[j]) {  // will only be the case when embeddings are not allowed to
                               // overlap
      continue;
    }
...
#ifdef DEBUG_SUBSTRUCTURE_QUERY
    cerr << "Atom " << j << " matches query atom 0, " << results.number_embeddings()
         << " embeddings so far, root " << _iroot << '\n';
#endif

    const int tmp =
        _find_embedding(target_molecule, target_atom, matched_atoms, already_matched,
                        matched_by_global_specs, root_atom, results);
    ...
    for (int i = 0; i < target_molecule.natoms(); ++i) {
      cerr << " alread matched " << i << ' ' << already_matched[i]
           << target_molecule.molecule()->smarts_equivalent_for_atom(i) << '\n';
    }
#endif
...
int
Single_Substructure_Query::_match_elements_needed(
    Molecule_to_Match& target_molecule) const {
...
int
RequiredMolecularProperties::MatchElementsNeeded(
    Molecule_to_Match& target_molecule) const {
  for (const Elements_Needed* e : _elements_needed) {
    if (!e->matches(target_molecule)) {
      return 0;
...
int
Single_Substructure_Query::_match_heteroatom_specifications(
    Molecule_to_Match& target_molecule) {
  if (_heteroatoms.empty()) {
...
int
Single_Substructure_Query::_match_ring_system_specifications(
    Molecule_to_Match& target_molecule, std::unique_ptr<int[]>& matched_by_global_specs) {
#ifdef DEBUG_MATCH_RING_SYSTEM_SPECIFICATIONS
  cerr << "Single_Substructure_Query::_match_ring_system_specifications:checking "
       << _ring_system_specification.number_elements() << " ring system specifications\n";
#endif
  ...
  for (int i = 0; i < nr; i++) {
    ...
    int m =
        _ring_system_specification[i]->matches(target_molecule, matched_by_global_specs);
...
int
Single_Substructure_Query::_match_ring_specifications(
    Molecule_to_Match& target_molecule, std::unique_ptr<int[]>& matched_by_global_specs) {
  int nr = _ring_specification.number_elements();

#ifdef DEBUG_MATCH_RING_SPECIFICATIONS
  cerr << "Single_Substructure_Query::_match_ring_specifications:tsting " << nr
       << " ring specifications\n";
#endif
...
int
Single_Substructure_Query::_match_nrings_specifications(
    Molecule_to_Match& target_molecule) {
  return _nrings.matches(target_molecule.nrings());
}

int
Single_Substructure_Query::_match_ring_type_specifications(
    Molecule_to_Match& target_molecule) {
  if (_aromatic_rings.is_set() &&
...
Single_Substructure_Query::_spinach_atoms_match(Molecule_to_Match& target) const {
  ...
  // cerr << "Single_Substructure_Query::_spinach_atoms_match:number_inter_ring_atoms " <<
  // number_inter_ring_atoms << '\n';
  return _inter_ring_atoms.matches(number_inter_ring_atoms);
}

/*
  If is very common for there to be no global conditions present, so we can often save
  time by detecting that once and then not checking on subsequent calls. Make sure you
  update this function as new global conditions are added!
*/
...
Single_Substructure_Query::_discern_global_conditions_present() {
  ...
  if (_natoms.is_set() || _heteroatoms_in_molecule.is_set() || _nrings.is_set() ||
      _number_isotopic_atoms.is_set() || _number_fragments.is_set() ||
      _ring_specification.number_elements() ||
      _ring_system_specification.number_elements() ||
      _elements_needed.number_elements() || _atoms_in_spinach.is_set() ||
      _inter_ring_atoms.is_set() || _aromatic_rings.number_elements() ||
      _non_aromatic_rings.is_set() || _isolated_rings.is_set() ||
      _isolated_ring_objects.is_set() || _fused_rings.is_set() ||
      _strongly_fused_rings.is_set() || _net_formal_charge.is_set() ||
...
int
Single_Substructure_Query::_match_global_specifications(
    Molecule_to_Match& target_molecule, std::unique_ptr<int[]>& matched_by_global_specs) {
#ifdef DEBUG_CHECK_GLOBAL_CONDITIONS
  cerr << "Checking global specifications: _natoms " << _natoms.is_set() << ", matches? "
       << _natoms.matches(target_molecule.natoms()) << '\n';
  ...
  if (_global_conditions_present < 0) {
    ...
    if (_global_conditions_present == 0 &&
        _required_molecular_properties.attributes_specified()) {
      _global_conditions_present = 1;
    }
  }
...
#ifdef DEBUG_CHECK_GLOBAL_CONDITIONS
  cerr << "Do we need to check ring specification(s) "
       << _ring_specification.number_elements() << '\n';
#endif
  ...
  if (_net_formal_charge.is_set() &&
      !_net_formal_charge.matches(target_molecule.net_formal_charge())) {
    return 0;
  }
...
int
Single_Substructure_Query::_aromatic_atoms_matches(
    Molecule_to_Match& target_molecule) const {
  const int matoms = target_molecule.natoms();
...
Single_Substructure_Query::_substructure_search(Molecule_to_Match& target_molecule,
  ...
  cerr << "Begin common _substructure_search code, " << _root_atoms.number_elements()
       << " root atoms\n";
  cerr << "_respect_initial_atom_numbering " << _respect_initial_atom_numbering << '\n';
#endif
  ...
  int* tmp = new_int(target_molecule.natoms());
  std::unique_ptr<int[]> free_tmp(tmp);

  uint32_t rc =
      _substructure_search(target_molecule, tmp, matched_by_global_specs, results);
...
#ifdef DEBUG_SUBSTRUCTURE_SEARCH
  cerr << "Return code from _substructure_search " << rc << " results contains "
       << results.number_embeddings() << " embeddings\n";
  if (results.number_embeddings()) {
    cerr << "     last embedding " << *results.embedding(results.number_embeddings() - 1)
         << '\n';
  }
...
Single_Substructure_Query::_one_time_initialisations() {
  ...
  _determine_if_fragment_ids_present();
...
Single_Substructure_Query::substructure_search(Molecule_to_Match& target_molecule,
  ...
  if (1 == rc ||
      0 == _preferences_present)  // if only one embedding, or no preferences, do nothing
    ;
  ...
  if (rc > 1 && _distance_between_hits.is_set()) {
    int tmp = results.remove_hits_violating_distance(
        target_molecule, _distance_between_hits,
        _matched_atoms_to_check_for_hits_too_close);  // the number of hits removed
  ...
  if (0 == rc) {  // no embeddings to worry about
    ...
    if (nf > 1 && rc > 1) {
      ...
      const extending_resizable_array<int>& hits_per_fragment =
          results.hits_per_fragment();

      int found_match = 0;
      for (int i = 0; i < nf; i++) {
#ifdef DEBUG_SUBSTRUCTURE_SEARCH
        cerr << hits_per_fragment[i] << " hits in fragment " << i << " matches "
             << _hits_needed.matches(hits_per_fragment[i]) << '\n';
#endif
  ...
  int minv;
  if (!_hits_needed.min(minv)) {  // no minimum specified (Hmmm, due to the logic above,
                                  // returning here should never happend)
    return rc;
  }
...
Single_Substructure_Query::_examine_bond_specifications() {
  ...
  for (int i = 0; i < nr; i++) {
    ...
    if (root_atom->involves_aromatic_bond_specifications(
  ...
  // cerr << "At end of _examine_bond_specifications, ring = " <<
  // _need_to_compute_ring_membership <<
...
Single_Substructure_Query::print_environment_matches(std::ostream& os) const {
  ...
  os << "  " << _matches_before_checking_environment
     << " matches before checking environment(s)\n";
  if (ne) {
    os << "  " << _no_match_to_environment
       << " embeddings failed to match the environment(s)\n";
  }
  if (nr) {
    os << "  " << _match_to_environemt_rejection
       << " embeddings matched an environment rejection\n";
  }

  if (_embeddings_violating_distance_constraints) {
    os << "  " << _embeddings_violating_distance_constraints
       << " embeddings violated a distance constraint\n";
  }
...
void
Single_Substructure_Query::identify_atom_numbers(
    extending_resizable_array<int>& a) const {
  for (int i = 0; i < _root_atoms.number_elements(); i++) {
...
void
Single_Substructure_Query::identify_atom_map_numbers(
    extending_resizable_array<int>& a) const {
  for (int i = 0; i < _root_atoms.number_elements(); i++) {
...
void
Single_Substructure_Query::assign_unique_id_from_atom_number_if_set(
...
Single_Substructure_Query::print_connectivity_graph(std::ostream& output) const {
  output << "Single_Substructure_Query::print_connectivity_graph:has "
         << _root_atoms.number_elements() << " root atoms\n";
  for (int i = 0; i < _root_atoms.number_elements(); ++i) {
    output << "ROOT " << i << '\n';
...
int
Single_Substructure_Query::_unmatched_atoms_attached_matched(
    const Query_Atoms_Matched& matched_query_atoms, const int* already_matched,
    Molecule_to_Match& target) const {
  for (Substructure_Atom* a : matched_query_atoms) {
    if (!a->unmatched_atoms_attached_matches(matched_query_atoms, already_matched,
                                             target)) {
      return 0;
    }
  }
...
Molecule_to_Match::AssignAtomTypes(Atom_Typing_Specification& atom_typing) {
  ...
  atom_type_t* atype = new atom_type_t[_natoms];
...
int
Single_Substructure_Query::_atom_type_groupings_matched(
    const Query_Atoms_Matched& matched_query_atoms) const {
  const int n = matched_query_atoms.number_elements();
#ifdef DEBUG_ATOM_TYPE_GROUPINGS_MATCHED
  cerr << "Single_Substructure_Query::_atom_type_groupings_matched:checking " << n
       << " matched atoms\n";
#endif
  for (int i = 0; i < n; ++i) {
    const Substructure_Atom* ai = matched_query_atoms[i];
...
#ifdef DEBUG_ATOM_TYPE_GROUPINGS_MATCHED
    cerr << " matched atom " << i << " has type " << atype << " group "
         << ai->atom_type_group() << '\n';
#endif
...
#ifdef DEBUG_ATOM_TYPE_GROUPINGS_MATCHED
      cerr << "   matched atom " << j << " has type "
           << aj->current_hold_atom()->atom_type() << " group " << aj->atom_type_group()
           << '\n';
#endif

      if (ai->atom_type_group() ==
          aj->atom_type_group())  // Same group, types must match.
      {
...
// encountered.
int
SeparatedAtoms::RotatableBondsBetween(Molecule& m, atom_number_t a1, atom_number_t a2,
...
int
SeparatedAtoms::RotatableBondsBetween(Molecule& m, atom_number_t a1,
...
int
SeparatedAtoms::Matches(Molecule& m, const Set_of_Atoms& embedding) const {
  atom_number_t a1 = embedding[_a1];
  atom_number_t a2 = embedding[_a2];
  // cerr << "Matched atoms are " << a1 << " and " << a2 << " betw " <<
...
void
IdentifyRegion(InterRingRegionData& irrd, atom_number_t zatom) {
...
int
Single_Substructure_Query::InterRingRegionsMatch(
  ...
  for (int i = 0; i < matoms; ++i) {
    ...
    for (int j = 0; j < number_region_specifications; ++j) {
      if (!_inter_ring_region[j]->Matches(target, region_number, irrd,
                                          matched_by_global_specs)) {
#ifdef DEBUG_INTER_RING_REGION_MATCH
...
void
SetRingAtoms(Query_Atoms_Matched& matched_query_atoms, int* ring_atom) {
...
int
Single_Substructure_Query::_substituent_satisfied(
    Molecule_to_Match& target_molecule, Query_Atoms_Matched& matched_query_atoms) const {
  const int matoms = target_molecule.natoms();
...
int
Single_Substructure_Query::_substituent_no_match_matches(
    Molecule_to_Match& target_molecule, Query_Atoms_Matched& matched_query_atoms) const {
  const int matoms = target_molecule.natoms();
...
int
Single_Substructure_Query::_matched_atoms_match_also_matched(
    Query_Atoms_Matched& matched_query_atoms, Molecule_to_Match& target) {
#ifdef DEBUG_MATCHED_ATOMS_MATCH_ALSO_MATCHED
  cerr << "Single_Substructure_Query::_matched_atoms_match_also_matched:checking "
       << _matched_atom_match.size() << " MatchedAtomMatch\n";
...
MatchedAtomMatch::Matches(Query_Atoms_Matched& matched_query_atoms,
                          Molecule_to_Match& target, int* already_matched) {
#ifdef DEBUG_MATCHED_ATOMS_MATCH_ALSO_MATCHED
  cerr << "MatchedAtomMatch::Matches: " << _atoms.size() << " atoms "
       << _positive_matches.size() << " positive amd " << _negative_matches.size()
       << " negative matches\n";
#endif
  const int matoms = target.natoms();
  ...
  for (int atom : _atoms) {
    if (!matched_query_atoms.ok_index(atom)) {
      cerr << "MatchedAtomMatch::Matches:invalid atom number " << atom
           << " embedding has " << matched_query_atoms.size() << " items\n";
      return 0;
    }
    ...
    if (_positive_matches.size() > 0) {
      ...
      for (Single_Substructure_Query* q : _positive_matches) {
        ...
        if (q->find_embedding(target, target_atom, matched_atoms, already_matched, root,
                              sresults)) {
          got_match = true;
#ifdef DEBUG_MATCHED_ATOMS_MATCH_ALSO_MATCHED
