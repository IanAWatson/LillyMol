...
Substructure_Atom::Substructure_Atom(const Element* e) : Substructure_Atom_Specifier(e) {
  _default_values();
...
Substructure_Atom::unique_id_from_initial_atom_number() {
  if (_initial_atom_number < 0) {
    cerr << "Substructure_Atom::unique_id_from_initial_atom_number: initial atom number "
            "not set\n";
    return 0;
  }
...
int
Substructure_Atom::collect_all_atoms(
...
Substructure_Atom::ok() const {
  ...
  if (nullptr != _parent && nullptr == _bond_to_parent) {
    cerr << "Substructure_Atom::ok: parent is not null, but bond to parent is, atom "
         << _unique_id << '\n';
  }
#endif
  ...
  if (_components.empty() && 1 == _operator.number_results())
    ;
  else if (_components.number_elements() ==
           _operator.number_results() - 1)  // true only during building
    ;
  else if (_components.number_elements() != _operator.number_results()) {
#ifdef SHOW_SUBSTRUCTURE_ATOM_OK
    cerr << "Components " << _components.number_elements() << " operator mismatch "
         << _operator.number_results() << '\n';
#endif
    return 0;
  }
...
Substructure_Atom::debug_print(std::ostream& os, const IWString& indentation) const {
  ...
  os << indentation << "Substructure Atom " << _unique_id << " (initial "
  ...
  if (nullptr != _anchor) {
    os << indentation << " Currently anchored at atom " << _anchor->atom_number()
       << ", _con = " << _con << '\n';
  } else {
  ...
  if (0 == nb) {
    ...
    for (int i = 0; i < nb; i++) {
      ...
      if (nullptr == other_query_atom) {
        os << indentation
           << "Very strange, atom at other end of bond is NULL, perhaps I am an "
              "environment atom\n";
      } else {
        const Target_Atom* other_atom = other_query_atom->current_hold_atom();

        if (nullptr == other_atom) {
          os << indentation << "    Substructure atom at other end ("
             << other_query_atom->unique_id() << ") not matched\n";
        } else {
          os << indentation << "    Substructure atom at other end ("
             << other_query_atom->unique_id() << ") is matched with atom "
...
int
Substructure_Atom::terse_details(std::ostream& os, const IWString& indentation) const {
  assert(os.good());
...
Substructure_Atom::print_connectivity_graph(std::ostream& output) const {
  const int uid =
      10000 * _unique_id + 1000 * _initial_atom_number + 100 * _atom_map_number;
  output << "atom " << _unique_id << " initial " << _initial_atom_number << " map "
         << _atom_map_number << ", UID " << uid << ", " << _children.number_elements()
         << " children. ";
  if (nullptr == _bond_to_parent) {
    output << " Root\n";
...
#ifdef DEBUG_HOLD
  cerr << "Atom " << _unique_id << " being released from atom "
       << _current_hold_atom->atom_number() << '\n';
#endif
...
Substructure_Atom::_matches(Target_Atom& target, const int* already_matched) {
  ...
  if (_attributes_specified > 0) {
    ...
    cerr << "Matching atom with " << _components.number_elements()
         << " components, underlying specifier match " << m << " match or rej "
  ...
  if (nc) {
    ...
    for (int i = 0; i < nc; i++) {
      //    cerr << "Do we need a result from component " << i << "? " <<
      //    _operator.result_needed(i) << '\n';
...
#endif
    if (!_environment.matches(target, target.atoms_in_target_molecule(),
                              already_matched)) {
      return !_match_as_match_or_rejection;
    }
...
Substructure_Atom::matches(Target_Atom& target, const int* already_matched) {
  ...
  cerr << " trying to match atom " << target.atom_number() << ' '
       << target.m()->atomic_symbol(target.atom_number()) << '\n';
#endif
...
Substructure_Atom::_ring_closure_bonds_match(Target_Atom* a) const {
  ...
  for (int i = 1; i < nb; i++)  // start with i = 1
  {
    ...
    if (nullptr ==
        (zbond = a->bond_to_atom(s->atom_number_matched()))) {  // not connected
      return 0;
    }
...
void
Substructure_Atom::set_parent(Substructure_Atom* a, Substructure_Bond* b,
...
int
Substructure_Atom::move_to_next_match_from_current_anchor(
    int* already_matched, const Query_Atoms_Matched& matched_atoms) {
  // assert (ok());
...
#ifdef DEBUG_MOVE_TO_NEXT_FROM_ANCHOR
  cerr << "move_to_next_match_from_current_anchor:: query atom " << _unique_id
       << " anchored at " << _parent->current_hold_atom()->atom_number() << '\n';
...
#ifdef DEBUG_MOVE_TO_NEXT_FROM_ANCHOR
    cerr << "Query atom " << _unique_id << ", _con " << _con << ", may go to atom "
         << a->atom_number() << " atn " << a->atomic_number() << '\n';
...
#ifdef DEBUG_MOVE_TO_NEXT_FROM_ANCHOR
    cerr << "move_to_next_match: query atom " << _unique_id << ", _con " << _con
         << " matched with atom " << a->atom_number() << '\n';
#endif
...
int
Substructure_Atom::_set_implicit_hydrogens(Molecule& m, atom_number_t a) const {
  int ih;
...
int
Substructure_Atom::_fill_min_ncon(Molecule& m, atom_number_t a) const {
...
int
Substructure_Atom::_add_bond_to_molecule(Molecule& m, const Substructure_Atom* other_atom,
  ...
  if (_unique_id == other_atom->unique_id()) {
    cerr << "Substructure_Atom::_add_bond_to_molecule: yipes, atoms the same "
         << _unique_id << '\n';
    return 0;
  }
  ...
  if (_unique_id >= matoms || other_atom->unique_id() >= matoms) {
    cerr
        << "Substructure_Atom::_add_bond_to_molecule: yipes, atom numbers out of range\n";
    cerr << "molecule contains " << matoms << " atoms, id's are " << _unique_id << " and "
         << other_atom->unique_id() << '\n';
    return 0;
  }

  if (m.are_bonded(_unique_id, other_atom->unique_id())) {
    cerr << "Substructure_Atom::_add_bond_to_molecule: yipes, atoms " << _unique_id
         << " and " << other_atom->unique_id() << " already bonded\n";
    return 0;
  }
...
int
Substructure_Atom::_add_bond_to_molecule(Molecule& m, const Substructure_Bond* b) const {
...
int
Substructure_Atom::create_molecule(Molecule& m, int fill_min_ncon,
  ...
  for (const Substructure_Atom* c : _children) {
    if (!c->create_molecule(m, fill_min_ncon, set_implicit_hydrogens)) {
      cerr << "Substructure_Atom::create_molecule: atom " << _unique_id << " atom number "
           << my_atom_number << " failed\n";
      return 0;
    }
  }
  ...
  for (const Substructure_Bond* b : _bonds) {
    if (!_add_bond_to_molecule(m, b)) {
      cerr << "Substructure_Atom::create_molecule: atom " << _unique_id << " atom number "
           << my_atom_number << " adding " << _bonds.number_elements() << " bonds\n";
      return 0;
    }
  }
...
void
Substructure_Atom::assign_unique_id_from_atom_number_if_set(
    extending_resizable_array<int>& numbers_in_use) {
  // cerr <<
  // "Substructure_Atom::assign_unique_id_from_atom_number_if_set:_initial_atom_number "
  // << _initial_atom_number << " _unique_id " << _unique_id << '\n';
...
int
Substructure_Atom::determine_start_stop(const Molecule_to_Match& target, int& istart,
                                        int& istop) const {
...
Substructure_Atom::bond_between_atom_map_numbers(int a1, int a2) const {
  ...
  cerr << "Substructure_Atom::bond_between_atom_map_numbers:looking for " << a1 << " and "
       << a2 << " I am " << _atom_map_number << '\n';
#endif
...
Substructure_Atom::_bond_between_atom_map_numbers(int a1, int a2) const {
  ...
  for (const Substructure_Atom* c : _children) {
#ifdef DEBUG_BOND_BETWEEN_ATOM_MAP_NUMBERS
    cerr << "Substructure_Atom::bond_between_atom_map_numbers:from " << _atom_map_number
         << " child is " << c->atom_map_number() << " CHILD\n";
#endif
...
Substructure_Atom::symmetry_groups_present() const {
  // cerr << "Substructure_Atom::symmetry_groups_present:checking " << _symmetry_group <<
  // ' ' << this << '\n';
...
int
Substructure_Atom::number_fragment_ids() const {
  int rc = (_fragment_id > 0);

  for (const Substructure_Atom* c : _children) {
    rc += c->number_fragment_ids();
  }
...
Substructure_Atom::assign_atom_map_numbers(int& amap) {
  ...
  // cerr << "Substructure_Atom::assign_atom_map_numbers:atom " << _unique_id << " map "
  // << _atom_map_number << " do I need " << amap << '\n';
...
int
UnmatchedAtomsAttached(const Molecule& m, int* already_matched,
...
int
Substructure_Atom::unmatched_atoms_attached_matches(
