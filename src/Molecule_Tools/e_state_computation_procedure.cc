...
double
Kier_and_Hall_valence_delta_value(atomic_number_t atomic_number_i, const int hcount) {
  int valance = 0;
...
value_of_Kier_and_Hall_relative_electronegativity(Molecule& m, const int i) {
  ...
  double delta_v =
      (double)Kier_and_Hall_valence_delta_value(m.atomic_number(i), m.hcount(i));
...
int
determine_Kier_and_Hall_relative_electronegativity(Molecule& m,
                                                   double relative_electronegativity[]) {
  int n_atoms = m.natoms();

  for (int i = 0; i < n_atoms; i++) {
    relative_electronegativity[i] =
        value_of_Kier_and_Hall_relative_electronegativity(m, i);
  }
...
determine_atom_e_state_index(Molecule& m, double e_state_index[],
  ...
  double* i_state = new double[n_atoms];
  std::unique_ptr<double[]> free_i_state(i_state);
  ...
  for (int i = 0; i < n_atoms; i++) {
    ...
    //      cerr << "Atom " << i << " type " << m.atomic_symbol(i) << " e_state_index from
    //      istate " << e_state_index[i] << endl;
    for (int j = 0; j < n_atoms; j++) {
      if (j != i) {
        int graph_distance = m.bonds_between(i, j) + 1;

        e_state_index[i] += (i_state[i] - i_state[j]) / (graph_distance * graph_distance);
        //          cerr << "Atom " << i << " to " << j << ", i_state " << i_state[j] << "
        //          dist " << graph_distance << " value now " << e_state_index[i] << endl;
      }
    }
  }
...
// #define DEBUG_ATOM_E_STATE_INDEX
  ...
  for (int i = 0; i < n_atoms; i++) {
    cerr << "Atom " << i << " type " << m.atomic_symbol(i) << " e_state_index "
         << e_state_index[i] << endl;
  }
#endif
...
determine_hydrogen_e_state_index(Molecule& m, double h_e_state_index[],
                                 const atomic_number_t* z) {
  int n_atoms = m.natoms();
  double* relative_electronegativity = new double[n_atoms];
  std::unique_ptr<double[]> free_relative_electronegativity(relative_electronegativity);
    ...
    if (0 == m.hcount(i)) {
      ...
      for (int j = 0; j < n_atoms; j++) {
        if ((j != i) && (1 != z[j])) {
          ...
          h_e_state_index[i] -=
              -(-0.2 - relative_electronegativity[j]) / (graph_distance * graph_distance);
        }
      }
    }
  }
...
determine_hydrogen_e_state_index(Molecule& m, double h_e_state_index[]) {
  ...
  atomic_number_t* z = new atomic_number_t[n_atoms];
  std::unique_ptr<atomic_number_t[]> free_z(z);
  ...
  for (int j = 0; j < nconi; j++) {
    if (2 == (m.btype_to_connection(ith_atom, j) % 8)) {
      ...
      for (int k = 0; k < nconj; k++) {
        ...
        if ((kth_atom != ith_atom) && (1 != m.atomic_number(kth_atom)) &&
            (1 == m.nbonds(kth_atom) - m.ncon(kth_atom))) {
          conjugated_bond++;
          break;
        }
      }
      ...
      atom_number_t jth_atom = m.other(ith_atom, j);
      if ((1 != m.atomic_number(jth_atom)) &&
          (1 == m.nbonds(jth_atom) - m.ncon(jth_atom))) {
        conjugated_bond++;
      }
    }
  }
