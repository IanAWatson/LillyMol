...
int
determine_atom_e_state_index(Molecule& m, double e_state_index[], const double* i_state,
                             const atomic_number_t* z) {
  const int n_atoms = m.natoms();
...
determine_hydrogen_e_state_index(Molecule& m, double h_e_state_index[],
  ...
  return determine_hydrogen_e_state_index(m, h_e_state_index, relative_electronegativity,
                                          z);
}
...
number_of_bond_in_conjugated_system(Molecule& m, const int ith_atom) {
  ...
  for (int j = 0; j < nconi; j++) {
    ...
    if (b->is_double_bond()) {
      ...
      //      if ((1 != m.atomic_number(jth_atom)) && (1 == m.nbonds(jth_atom) -
      //      m.ncon(jth_atom)))
