...
class Descriptor : public Set_or_Unset<float>, public Accumulator<float> {
 ...
 public:
  ...
  void
  set_best_fingerprint(int s) {
    _best_fingerprint = s;
  }

  int
  best_fingerprint() const {
    return _best_fingerprint;
  }

  void
  set_min_max_dy(float v1, float v2, float v3) {
    _min = v1;
    _max = v2;
    _dy = v3;
  }
  ...
  int
  produce_fingerprint() const {
    return _fingerprint_replicates;
  }

  void
  set_produce_fingerprint(int s) {
    _fingerprint_replicates = s;
  }

  int
  bit_replicates() const {
    return _fingerprint_replicates;
  }
  ...
  void
  set_default_value(float d) {
    _default_value.set(d);
  }
...
Descriptor::produce_fingerprint(int bitnum, Sparse_Fingerprint_Creator& sfc) const {
  ...
  // " bit " << bitnum << " value " << c << " rep " << _fingerprint_replicates << '\n';
...
static void
fill_descriptor_extremeties(Descriptor* d, const int resolution) {
  ...
  d[iwdescr_amw].set_min_max_resolution(100.0f, 600.0f,
                                        resolution);  // manually overwritten
...
GetDescriptor(const Descriptor* descriptors, int number_descriptors,
...
static int
descriptor_name_2_index(const Descriptor* d, int n, const const_IWSubstring& dname) {
  if (descriptor_prefix.empty()) {
...
static int
identify_descriptor(const IWString& dname, const Descriptor* descriptor,
...
Descriptor_Filter::build(const const_IWSubstring& s) {
  ...
  _descriptor_number =
      identify_descriptor(_descriptor_name, descriptor, NUMBER_DESCRIPTORS);
...
allocate_descriptors() {
  ...
  descriptor[iwdescr_ro5_on].set_name(
...
static int
write_fingerprint(Molecule& m, const Descriptor* descriptor, int n,
...
create_header(const Descriptor* descriptor,
              const IW_STL_Hash_Map_String& name_translation, const char output_separator,
              IWString& header) {
  header.resize(500);
  ...
  for (int i = 0; i < NUMBER_DESCRIPTORS; i++) {
    if (descriptor[i].active()) {
      ...
      if (iter == name_translation.end()) {
        ...
        if (verbose > 1) {
          cerr << "Feature " << descriptor[i].descriptor_name()
  ...
  if (not_found_in_name_translation) {
    cerr << "Warning " << not_found_in_name_translation
         << " features not found in name translation table\n";
  }
...
static int
WriteHeader(const Descriptor* descriptor, const IW_STL_Hash_Map_String& name_translation,
            const char output_separator, IWString_and_File_Descriptor& output) {
  IWString header;
...
static int
do_filtering(Molecule& m, IWString_and_File_Descriptor& output) {
...
int
OutputAsWholeNumber(float v, IWString& output) {
...
static int
write_the_output(Molecule& m, const char output_separator,
                 IWString_and_File_Descriptor& output) {
...
static int
print_array(const extending_resizable_array<int>& z, const char* c, IWString& output) {
  if (z.empty()) {
    return output.good();
  }
...
static void
compute_ring_substitution_descriptors(extending_resizable_array<int>& diffs, int n,
                                      int n2, int d) {
  assert(d > 0);
...
static int
compute_ring_substitution_descriptors(Molecule& m, const int* ncon,
...
// #define DEBUG_COMPUTE_RING_SUBSTITUTION_DESCRIPTORS
  ...
  if (2 == noffset) {  // a very common case
    compute_ring_substitution_descriptors(diffs, n, n2,
...
int
BondedToAnotherRing(Molecule& m, const int* ring_membership, atom_number_t zatom) {
...
static int
OrthoRingCount(Molecule& m, const int* ncon, const int* ring_membership,
               const Ring& ring) {
  int rc = 0;
...
static int
compute_ring_substitution_ratios(Molecule& m, const int* ncon, const Atom* const* atom,
...
static int
compute_between_ring_descriptors(Molecule& m, const int* ncon, const Atom* const* atom) {
  int nr = m.nrings();
...
static int
compute_adjacent_ring_fusion_descriptors(Molecule& m, const int* ncon,
                                         const int* ring_membership) {
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    for (const Bond* b : *ai) {
      ...
      for (const Bond* b : *ak) {
        //      cerr << "From " << k << " to " << b->other(k) << " bond in " <<
        //      b->nrings() << " rings\n";
        if (b->nrings())
          ;
...
compute_ring_substitution_descriptors(Molecule& m, const int* ncon) {
  ...
  for (int i = 0; i < nr; i++) {
    ...
    if (ri->is_aromatic()) {
      compute_ring_substitution_descriptors(
  ...
  if (fused_rings_present) {
    ...
    for (int i = 0; i < nr; i++) {
      ...
      for (int j = i + 1; j < nr; j++) {
        ...
        //      cerr << "Ring " << j << " has " <<
        //      rj->largest_number_of_bonds_shared_with_another_ring() << " bonds
        //      shared\n";
      ...
      compute_ring_substitution_descriptors(m, ncon, s,
                                            bonds_between_ring_substitutions_ring_system);
    }
  ...
  if (bonds_between_ring_substitutions_aliphatic_ring.number_elements() < 4) {
    ...
    int n4 = 0;
    for (int i = 4; i < bonds_between_ring_substitutions_aliphatic_ring.number_elements();
         i++) {
...
static void
compute_ramey_descriptors(Molecule& m, const atomic_number_t* z) {
  ...
  const auto oxygen_balance = -1600.0 * (2 * nc + static_cast<double>(nh) / 2.0 - no) /
...
static int
is_spiro_fused(Molecule& m, atom_number_t a) {
  int nr = m.nrings();
  for (int i = 0; i < nr; i++) {
    const Ring* ri = m.ringi(i);
...
static int
do_compute_spiro_fusions(Molecule& m, const int* ncon, const int* ring_membership) {
  int nr = m.nrings();
...
static int
do_compute_chirality_descriptors(Molecule& m, const atomic_number_t* z, const int* ncon) {
  int matoms = m.natoms();
...
void
compute_atom_distribution_along_longest_path(const Molecule& m, const int* dm,
...
static int
do_compute_mean_shell_occupancies(const Molecule& m, const int* dm) {
  const int matoms = m.natoms();
...
static int
do_compute_descriptors_related_to_centroid_atom(Molecule& m, const int* dm,
...
static int
do_compute_distances_from_longest_path_descriptors(Molecule& m, const int* dm,
                                                   atom_number_t a1, atom_number_t a2,
...
do_compute_distances_from_longest_path_descriptors(Molecule& m, const int* dm,
  ...
  if (1 == n) {
    return do_compute_distances_from_longest_path_descriptors(m, dm, a1[0], a2[0],
  ...
  return do_compute_distances_from_longest_path_descriptors(
...
static int
do_compute_distance_matrix_descriptors(Molecule& m, const atomic_number_t* z,
                                       const int* ncon, int* eccentricity,
                                       const int* dm) {
  const auto matoms = m.natoms();
  ...
  if (min_eccentricity > 0) {
    descriptor[iwdescr_ishape].set(
        static_cast<float>(max_eccentricity - min_eccentricity) /
...
do_compute_distance_matrix_descriptors(Molecule& m, const atomic_number_t* z,
  ...
  int* eccentricity = new_int(matoms + matoms);
  std::unique_ptr<int[]> free_eccentricity(eccentricity);

  int* dm = new int[matoms * matoms];
  std::unique_ptr<int[]> free_dm(dm);
  assert(nullptr != dm);
  ...
  do_compute_distances_from_longest_path_descriptors(
      m, dm, longest_path, eccentricity);  // re-use eccentricity array

  do_compute_descriptors_related_to_centroid_atom(
      m, dm, longest_path, z, ncon,
      eccentricity);  // eccentricity just used as a temporary array, existing contents
                      // destroyed
...
static int
compute_polar_bond_descriptors(Molecule& m, const atomic_number_t* z, const int* ncon) {
  int nb = m.nedges();
...
static int
compute_crowding_descriptors(Molecule& m, const int* ncon, Atom** atom) {
  const int matoms = m.natoms();
...
static int
compute_crowding_descriptors(Molecule& m, const int* ncon) {
  const Atom** a = new const Atom*[m.natoms()];
  std::unique_ptr<const Atom*[]> free_a(a);
...
static int
compute_bond_separation_parameters(Molecule& m, const int* isotope) {
  int matoms = m.natoms();
...
static int
compute_ring_fusion_descriptors(Molecule& m, const int* ncon,
                                const int* ring_membership) {
  int ar5 = 0;
  int ar6 = 0;
  int al5 = 0;
...
static int
atoms_not_already_marked(const Ring& r, int* atom_already_done) {
...
static int
compute_exocyclic_bonds(Molecule& m, const Ring& r, const int* ncon) {
  int rc = 0;
...
static int
compute_exocyclic_bonds(Molecule& m, const atomic_number_t* z, const Ring& r,
                        const int* ncon, int& double_bond_attachments,
                        int& singly_connected_attachments,
                        int& singly_connected_heteroatoms, int& singly_connected_donors) {
  int rc = 0;
...
static int
just_terminal_groups_outside_ring(const Molecule& m, const int* ncon, const Ring& r,
                                  atom_number_t a) {
...
static float
compute_ring_isolation(Molecule& m, const int* ncon, const Ring& r) {
  int ring_size = r.number_elements();
...
static int
heteroatoms_in_ring(const Ring* r, const atomic_number_t* z) {
  int rc = 0;
...
static int
compute_ring_descriptors(Molecule& m, const atomic_number_t* z, const int* ncon,
                         int* ring_already_done, int* atom_already_done) {
  int matoms = m.natoms();
  ...
  for (int i = 0; i < nr; i++) {
    ...
    exocyclic_bonds += compute_exocyclic_bonds(
        m, z, *ri, ncon, double_bond_attachments, singly_connected_attachments,
        singly_connected_heteroatoms, singly_connected_donors);
...
#ifdef DEBUG_ATOMS_NOT_ALREADY_MARKED
    cerr << "System contains " << system_size << " rings and " << atoms_in_system
         << " atoms\n";
#endif
...
compute_ring_descriptors(Molecule& m, const atomic_number_t* z, const int* ncon) {
  ...
  int* ring_already_done = new_int(nr);
  std::unique_ptr<int[]> free_ring_already_done(ring_already_done);
  int* atom_already_done = new_int(m.natoms());
...
static int
compute_hbond_descriptors(Molecule& m, const atomic_number_t* z, const int* ncon,
                          const Atom** atom) {
  int matoms = m.natoms();
...
static int
compute_substructure_based_charge_descriptors(Molecule& m, IWString& output,
                                              Query_and_Charge_Stats* query, int ii) {
  ...
  for (int i = 0; i < nmatches; i++) {
    ...
    xsum *=
        1.0 / static_cast<float>(atoms_matched);  // now the average of the coordinates
...
static int
ComputeLongCarbonChains(const Molecule& m, atom_number_t zatom, const atomic_number_t* z,
                        const int* ncon, int max_ncon, const int* ring_membership,
                        int* already_done) {
  ...
  for (const Bond* b : m[zatom]) {
    ...
    if (ncon[o] == 1) {
      ...
      rc +=
          ComputeLongCarbonChains(m, o, z, ncon, max_ncon, ring_membership, already_done);
    }
  }
...
// This is messy too.
static int
ComputeLongCarbonChains(const Molecule& m, const atomic_number_t* z, const int* ncon,
                        const int* ring_membership, int* already_done) {
  ...
  for (int i = 0; i < matoms; ++i) {
    ...
    int csize =
        ComputeLongCarbonChains(m, i, z, ncon, max_ncon, ring_membership, already_done);
    acc2.extra(csize);
  }
...
static int
ComputeSaturatedChain(Molecule& m, atom_number_t zatom, const int* ncon,
                      const int* ring_membership, int* already_done) {
  int rc = 1;
  already_done[zatom] = 1;
...
ComputeSaturatedChains(Molecule& m, const int* ncon, const int* ring_membership,
  ...
  const int matoms = m.natoms();
  for (int i = 0; i < matoms; ++i) {
    // cerr << i << " ring_membership " << ring_membership[i] << " ncon " << ncon[i] << "
    // saturated " << m.saturated(i) << '\n';
...
static int
compute_radha_entropy_descriptors(Molecule& m, atom_number_t zatom, const int* ncon,
                                  const int* ring_membership, const Atom** atom,
                                  int* already_done) {
  ...
  for (int i = 0; i < ncon[zatom]; i++) {
    ...
    if (ring_membership[j])
      ...
      rc += compute_radha_entropy_descriptors(m, j, ncon, ring_membership, atom,
                                              already_done);
    }
  }
...
static int
compute_radha_entropy_descriptors(Molecule& m, const int* ncon,
                                  const int* ring_membership, const Atom** atom,
                                  int* already_done) {
  int matoms = m.natoms();
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    int chain_length = compute_radha_entropy_descriptors(m, i, ncon, ring_membership,
                                                         atom, already_done);
...
static int
look_for_inter_ring_atoms(const Molecule& m, atom_number_t avoid, atom_number_t zatom,
                          int* inter_ring, const int* ring_membership,
                          const Atom** atom) {
  ...
  for (const Bond* b : *a) {
    ...
    if (ring_membership[j]) {  // found ring, great
      inter_ring[zatom] = 1;
      rc = 1;
    } else if (look_for_inter_ring_atoms(m, zatom, j, inter_ring, ring_membership,
                                         atom)) {
      inter_ring[zatom] = 1;
      rc = 1;
    }
...
static void
id_inter_ring_atoms(const Molecule& m, int* inter_ring, const int* ring_membership,
                    const Atom** atom) {
  int matoms = m.natoms();
...
static int
id_spinach(const Molecule& m, int* spinach, const int* ring_membership,
           const Atom** atom) {
  id_inter_ring_atoms(m, spinach, ring_membership, atom);

  const int matoms = m.natoms();

  // First task is to add doubly bonded, singly connected atoms to the ring and scaffold
  // atoms
  for (int i = 0; i < matoms; i++) {
    if (ring_membership[i] ||
        spinach[i]) {  // ring atoms or bridging atoms, add doubly bonded atoms
      const Atom* a = atom[i];
...
static int
compute_ring_chain_descriptors(Molecule& m, const atomic_number_t* z, const int* ncon,
                               const Atom** atom, const int* ring_membership) {
...
static int
WithinScaffoldBranched(const Molecule& m, atom_number_t zatom) {
...
static void
count_spinach_connections(const Molecule& m, const Ring& r, const int* spinach,
                          int& spinach_connections, int& non_spinach_connections) {
  ...
  for (int i = 0; i < ring_size; i++) {
    ...
    for (const Bond* b : *a) {
      ...
      //    cerr << "Atom " << j << " in ring, to atom " << l << " ?spinach " <<
      //    spinach[k] << '\n';
...
static int
compute_spinach_descriptors(Molecule& m, int matoms, int* spinach,
                            const atomic_number_t* z, const int* ncon,
                            const int* ring_membership, const Atom** atom) {
  assert(matoms > 0);
...
// }
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    if (spinach[i]) {
      for (int j = 0; j < icon; j++) {
        ...
        if (k < i ||
            0 == spinach[k]) {  // only count bonds once. Want both atoms in spinach
          continue;
        }
    ...
    } else  // atom I is NOT in spinach, in scaffold
    {
      for (int j = 0; j < icon; j++) {
        ...
        if (b->is_single_bond() && ncon[i] > 1 && ncon[k] > 1 &&
            0 == ring_membership[i] && 0 == ring_membership[k]) {
  ...
  if (matoms == spinach_atoms) {
    ...
    descriptor[iwdescr_fnrnspc].set(static_cast<float>(non_ring_non_spinach_atoms) /
                                    static_cast<float>(matoms - spinach_atoms));
  }

  if (bonds_in_spinach) {
    descriptor[iwdescr_rbfrspch].set(static_cast<float>(rotatable_bonds_in_spinach) /
  ...
  // cerr << "spinach_connections " << spinach_connections << " non_spinach_connections "
  // << non_spinach_connections << " terminal_rings " << terminal_rings << '\n';
...
static int
compute_spinach_descriptors(Molecule& m, const atomic_number_t* z, const int* ncon,
                            const int* ring_membership, const Atom** atom) {
  int matoms = m.natoms();

  int* spinach = new_int(matoms);
...
static int
compute_rule_of_five_stuff(Molecule& m, const atomic_number_t* z) {
  int matoms = m.natoms();
...
static double
identify_mr_halogen(const Molecule& m, const atomic_number_t* z, int* already_done) {
  int matoms = m.natoms();
...
static double
identify_mr_oxygen(const Molecule& m, const atomic_number_t* z, const Atom* const* atom,
                   int* already_done) {
  int matoms = m.natoms();
...
static double
identify_mr_sulphur(const Molecule& m, const atomic_number_t* z, const Atom* const* atom,
                    int* already_done) {
  double rc = 0.0;
...
static double
identify_mr_phosphorus(const Molecule& m, const atomic_number_t* z,
...
static double
identify_mr_nitro(const Molecule& m, const atomic_number_t* z, const Atom* const* atom,
                  int* already_done) {
  int matoms = m.natoms();
...
static double
identify_mr_nitrogen(Molecule& m, const atomic_number_t* z, const Atom* const* atom,
                     int* already_done) {
  int matoms = m.natoms();
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    if (INVALID_ATOM_NUMBER != doubly_bonded_heteroatom &&
        INVALID_ATOM_NUMBER != aromatic_neighbour) {
      already_done[i] = 14;
      rc += 3.8095;
...
static double
identify_mr_carbon(Molecule& m, const atomic_number_t* z, const Atom* const* atom,
                   int* already_done) {
  int matoms = m.natoms();
...
static int
compute_molar_refractivity(Molecule& m, const atomic_number_t* z, const Atom* const* atom,
  ...
  for (int i = 0; i < matoms; i++) {
    if (0 == already_done[i]) {
      if (report_unclassified_atoms) {
        cerr << "Unclassified CMR atom, molecule '" << m.name() << "', atom " << i << " '"
...
static float
identify_acm_n(const Molecule& m, int matoms, const atomic_number_t* z,
               const Atom* const* atom, int* already_done) {
  int nplus = 0;
  int n = 0;
...
static float
identify_acm_c(const Molecule& m, int matoms, const atomic_number_t* z,
               const Atom* const* atom, int* already_done) {
  int csp2 = 0;
  int csp3 = 0;
...
static float
identify_acm_halogen(const Molecule& m, int matoms, const atomic_number_t* z,
                     const Atom* const* atom, int* already_done) {
  int rc = 0;
...
static float
identify_acm_os(const Molecule& m, int matoms, const atomic_number_t* z,
                const Atom* const* atom, int* already_done) {
  int rc = 0;
...
static float
identify_acm_co(const Molecule& m, int matoms, const atomic_number_t* z,
                const Atom* const* atom, int* already_done) {
  int rc = 0;
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    if (already_done[c]) [[unlikely]] {  // strange
      continue;
    }
...
static float
identify_acm_oh(const Molecule& m, int matoms, const atomic_number_t* z,
                const Atom* const* atom, int* already_done) {
  int rc = 0;
...
static float
identify_acm_acids(const Molecule& m, int matoms, const atomic_number_t* z,
                   const Atom* const* atom, int* already_done) {
  int rc = 0;
...
static float
identify_acm_phosphoric_acids(const Molecule& m, int matoms, const atomic_number_t* z,
                              const Atom* const* atom, int* already_done) {
  int rc = 0;
...
static int
andrews_craik_martin(Molecule& m, const atomic_number_t* z, const Atom* const* atom) {
  int matoms = m.natoms();
...
static int
compute_halogen_attachments(Molecule& m, int matoms, const atomic_number_t* z,
                            const int* ncon, const Atom** atom) {
  int rc = 0;
  for (int i = 0; i < matoms; i++) {
...
static int
compute_electron_rich(const Molecule& m, atom_number_t a, const Atom** atom,
                      int* already_done) {
  already_done[a] = 1;
...
*/
static int
guanidine(Molecule& m, atom_number_t doubly_bonded_nitrogen, int* nitrogens,
          const Atom** atom, const atomic_number_t* z, const int* ncon,
          const int* ring_membership) {
...
compute_symmetry_related_descriptors(Molecule& m) {
  ...
  if (longest_path > 0) {
    descriptor[iwdescr_flsepsymatom].set(
        Fraction<float>(furthest_separated_symmetry_related_atoms, longest_path));
  } else {
...
static void
compute_4_connected_carbon_stuff(const Molecule& m, const atomic_number_t* z,
...
static int
strongly_fused(const Ring& r1, const Ring& r2, int matoms, int* tmp) {
  ...
  if (0 == max_difference_in_ring_size_for_strongly_fused) {  // no checking sizes
    ;
  } else if (r1.number_elements() - r2.number_elements() >
             max_difference_in_ring_size_for_strongly_fused) {
    return 0;
  } else if (r2.number_elements() - r1.number_elements() >
             max_difference_in_ring_size_for_strongly_fused) {
    return 0;
  }
...
static int
grow_fused_system(Molecule& m, int i, int* ring_already_done, int flag, int* atmp,
  ...
  for (int j = 0; j < number_neighbours; j++) {
    ...
    } else if (rj->largest_number_of_bonds_shared_with_another_ring() <
               2) {  // pair of rings not strongly fused
      continue;
    }
    ...
    rc += grow_fused_system(m, k, ring_already_done, flag, atmp,
...
static int
compute_planar_fused_rings(Molecule& m, const atomic_number_t* z, int* ring_already_done,
                           int* atmp) {
  int matoms = m.natoms();
  ...
  for (int i = 0; i < nr; i++) {
    ...
    if (ri->largest_number_of_bonds_shared_with_another_ring() >
        1) {  // in a strongly fused system
      continue;
    }

    int flag =
        matoms +
        2 * i;  // just a unique number that won't collide with a fused system identifier
  ...
  descriptor[iwdescr_htrcpfsy].set(
      static_cast<float>(heterocycles_in_planar_fused_systems));
...
/*
  Just check to see that there is at least one ring that is strongly fused and satisfies
  the size constraint
*/

static int
joined_rings_too_different_in_size(Molecule& m, const Ring& r, int* tmp) {
  int matoms = m.natoms();
...
static int
compute_non_planar_fused_rings(Molecule& m, const atomic_number_t* z,
  ...
  descriptor[iwdescr_nsfsdsys].set(
      static_cast<float>(number_strongly_fused_systems_found));
  descriptor[iwdescr_rnginsfs].set(static_cast<float>(rings_in_strongly_fused_systems));
  descriptor[iwdescr_lgstrfsy].set(
      static_cast<float>(largest_strongly_fused_system_size));
  descriptor[iwdescr_htrcsfsy].set(
      static_cast<float>(heterocycles_in_strongly_fused_systems));
...
static int
compute_fused_rings(Molecule& m, const atomic_number_t* z) {
  int nr = m.nrings();
  ...
  int* rtmp = new_int(nr);
  std::unique_ptr<int[]> free_rtmp(rtmp);

  int* atmp = new int[m.natoms()];
  std::unique_ptr<int[]> free_atmp(atmp);
...
template <typename T>
void
back_to_zero(T* v, int n, T vfrom, T vto) {
  for (int i = 0; i < n; ++i) {
    if (vfrom == v[i]) {
      v[i] = vto;
    }
...
static int
discern_conjugated_section(Molecule& m, atom_number_t zatom, const Atom** atom,
                           const int* is_aromatic, int* already_done, int flag) {
...
static int
compute_extended_conjugation(Molecule& m, const Atom** atom, const int* is_aromatic,
                             int* already_done) {
  int matoms = m.natoms();
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    int conjugated_section_size = discern_conjugated_section(
        m, i, atom, is_aromatic, already_done, number_conjugated_sections + 1);

#ifdef DEBUG_COMPUTE_EXTENDED_CONJUGATION
    cerr << "In '" << m.name() << "' found section of size " << conjugated_section_size
         << ", ndx " << (number_conjugated_sections + 1) << " starting with atom " << i
         << '\n';
#endif

    if (conjugated_section_size <=
        2) {  // Just an isolated double bond, or maybe a section that didn't extend
...
// #define TEST_EXTENDED_CONJUGATED_SECTIONS
  ...
  cerr << mcopy.smiles() << ' ' << m.name() << " extended " << number_conjugated_sections
       << ", " << atoms_in_conjugated_sections << " atoms\n";
...
static int
compute_double_bond_substitution(Molecule& m, const atomic_number_t* z, const int* ncon) {
  int nb = m.nedges();
...
static int
compute_amine_count(Molecule& m, const Atom** atom, const atomic_number_t* z,
                    const int* ncon, const int* ring_membership, int* already_done) {
...
static int
compute_pyridine_pyrrole(Molecule& m, const Atom** atom, const atomic_number_t* z,
                         const int* ncon, const int* ring_membership) {
  int matoms = m.natoms();
...
static int
compute_topological_descriptors(Molecule& m, const atomic_number_t* z, const int* ncon,
                                const int* ring_membership, const Atom** atom,
                                const int* is_aromatic, int* already_done) {
  int matoms = m.natoms();
  ...
  int atoms_with_pi_electrons = 0;  // aromatic, unsaturated and lone pair electrons -
                                    // this last type aren't really pi electrons...
  int electron_rich_sections =
      0;  // contiguous parts of the molecule with lots of electrons
  int atoms_in_electron_rich_sections =
      0;  // the number of atoms in electron rich sections
  ...
  for (int i = 0; i < matoms; i++) {
    ...
    if (electron_rich && 0 == already_done[i]) {
      ...
      //    cerr << electron_rich_sections << " electon rich sections, na = " <<
      //    atoms_in_electron_rich_sections << '\n';
    }
    ...
    for (int j = 0; j < icon; j++) {
      ...
      if (b->nrings()) {
        if (b->is_aromatic()) {  // Sept 2013, used to count these, but final result was
                                 // too correlated with aromatic atom count
          ;
        } else if (!b->is_single_bond()) {
          ring_multiple_bonds++;
        }
  ...
  descriptor[iwdescr_fbigatom].set(static_cast<float>(bigatom_count) /
                                   static_cast<float>(matoms));
  ...
  descriptor[iwdescr_hperatom].set(static_cast<float>(hcount) /
                                   static_cast<float>(matoms));
  ...
  if (heteroatom_count == matoms) {
    descriptor[iwdescr_fccsp3].set(
        static_cast<float>(0.0));  // hard to know what this should be
  } else {
    descriptor[iwdescr_fccsp3].set(static_cast<float>(csp3) /
                                   static_cast<float>(matoms - heteroatom_count));
  }
  ...
  if (descriptors_to_compute.ncon_descriptors) {
    ...
    for (int i = 1; i < 5; i++) {
      ...
      descriptor[offset + 2 * (i - 1) + 1].set(static_cast<float>(connected[i]) /
                                               static_cast<float>(matoms));
    }
  }
  ...
  int mbonds = m.nedges();
  if (0 == mbonds)
      [[unlikely]] {  // let's hope no-one does computations on single atom molecules
    cerr << "Warning, no bonds in molecule\n";
    mbonds = 1;
  }

  descriptor[iwdescr_mltbd].set(
      static_cast<float>(chain_multiple_bonds + ring_multiple_bonds));
  descriptor[iwdescr_fmltbd].set(
      static_cast<float>(chain_multiple_bonds + ring_multiple_bonds) /
      static_cast<float>(mbonds));
  descriptor[iwdescr_chmltbd].set(static_cast<float>(chain_multiple_bonds));
  descriptor[iwdescr_fchmltbd].set(static_cast<float>(chain_multiple_bonds) /
                                   static_cast<float>(mbonds));
  descriptor[iwdescr_rgmltbd].set(static_cast<float>(ring_multiple_bonds));
  descriptor[iwdescr_frgmltbd].set(static_cast<float>(ring_multiple_bonds) /
                                   static_cast<float>(mbonds));

  // int chain_atoms = matoms - ring_atom_count;
  // output << "frmcc " << static_cast<float>(singly_connected_atom) /
  // static_cast<float>(matoms)  << '\n'; output << "mcc " << singly_connected_atom <<
  // '\n';

  descriptor[iwdescr_dcca].set(static_cast<float>(two_connected_chain_atom));
  descriptor[iwdescr_fdcca].set(static_cast<float>(two_connected_chain_atom) /
                                static_cast<float>(matoms));
  ...
  if (m.nedges() == 0) {
    ...
    descriptor[iwdescr_frotbond].set(
        iwmisc::Fraction<float>(rotatable_bonds, m.nedges()));
  }
  ...
  if (ring_atom_count) {
    ...
    descriptor[iwdescr_rhaf].set(static_cast<float>(ring_heteroatom_count) /
                                 static_cast<float>(ring_atom_count));

    //   The fraction of rings atoms which are fused;

    descriptor[iwdescr_frafus].set(static_cast<float>(fused_ring_atom_count) /
                                   static_cast<float>(ring_atom_count));
  } else  // some of the rings descriptors are truly zero's, others are missing or zero by
          // definition
  {
  ...
  if (heavy_atom_count > 0) {
    descriptor[iwdescr_rngatmf].set(static_cast<float>(ring_atom_count) /
                                    static_cast<float>(heavy_atom_count));
  } else {
  ...
  descriptor[iwdescr_aromc].set(
      static_cast<float>(aromatic_atom_count - aromatic_heteroatom_count));
  descriptor[iwdescr_aliphc].set(static_cast<float>(aliphatic_carbon_count));
  if (ring_atom_count) {
    descriptor[iwdescr_fraromha].set(static_cast<float>(aromatic_heteroatom_count) /
                                     static_cast<float>(ring_atom_count));

    descriptor[iwdescr_aromdens].set(static_cast<float>(aromatic_atom_count) /
                                     static_cast<float>(matoms));
  } else {
  ...
  if (heavy_atom_count > 0) {
    descriptor[iwdescr_htroaf].set(static_cast<float>(heteroatom_count) /
                                   static_cast<float>(heavy_atom_count));
  } else {
  ...
  descriptor[iwdescr_ohsh].set(
      static_cast<float>(singly_connected_oxygen_or_sulphur_count));
  ...
  descriptor[iwdescr_fratmpie].set(static_cast<float>(atoms_with_pi_electrons) /
                                   static_cast<float>(matoms));

  descriptor[iwdescr_unsatura].set(static_cast<float>(unsaturation));
  descriptor[iwdescr_funsatura].set(static_cast<float>(unsaturation) /
                                    static_cast<float>(matoms));
  ...
  descriptor[iwdescr_faiercst].set(static_cast<float>(atoms_in_electron_rich_sections) /
                                   static_cast<float>(matoms));

  descriptor[iwdescr_platt].set(static_cast<float>(total_connectivity));
  descriptor[iwdescr_avcon].set(static_cast<float>(total_connectivity) /
                                static_cast<float>(matoms));
  if (matoms - aromatic_atom_count > 0) {
    descriptor[iwdescr_avalcon].set(static_cast<float>(total_aliphatic_connectivity) /
                                    static_cast<float>(matoms - aromatic_atom_count));
  }

  if (matoms - ring_atom_count > 0) {
    descriptor[iwdescr_avchcon].set(static_cast<float>(total_chain_connectivity) /
                                    static_cast<float>(matoms - ring_atom_count));
  }
...
static int
compute_distance_charge_descriptors(Molecule& m, IWString& output) {
  const int maxdist = 5;
  int matoms = m.natoms();
  ...
  for (int dist = 1; dist < maxdist; dist++) {
    ...
    if (number_atoms_this_separation) {
      ...
      output << "avqdif" << dist << " "
             << sum_charge_sepatation / static_cast<float>(number_atoms_this_separation)
             << '\n';
    } else {
      break;
    }
...
static int
compute_distance_4_charge_heteroatom_descriptors(Molecule& mol, IWString& output,
                                                 const atomic_number_t* z,
                                                 const int* ncon, atom_number_t* conn) {
...
static int
compute_distance_3_charge_heteroatom_descriptors(Molecule& mol, IWString& output,
                                                 const atomic_number_t* z,
                                                 const int* ncon, atom_number_t* conn) {
  (void)ncon;
...
static int
compute_distance_2_charge_heteroatom_descriptors(Molecule& m, IWString& output,
                                                 const atomic_number_t* z,
                                                 atom_number_t* conn) {
  int matoms = m.natoms();
...
static int
compute_distance_charge_heteroatom_descriptors(Molecule& m, IWString& output,
                                               const atomic_number_t* z, const int* ncon,
                                               atom_number_t* conn) {
...
static int
compute_charge_descriptors(Molecule& m, IWString& output, const atomic_number_t* z) {
  ...
  if (m.ok_2_atoms(most_negative_non_ring_atom, most_positive_non_ring_atom)) {
    output << "mnnrmpnr "
           << m.bonds_between(most_negative_non_ring_atom, most_positive_non_ring_atom)
           << '\n';
  }
  ...
  if (ring_atom_count) {
    ...
    if (m.ok_2_atoms(most_negative_ring_atom, most_positive_ring_atom)) {
      output << "mnrmpr "
             << m.bonds_between(most_negative_ring_atom, most_positive_ring_atom) << '\n';
    }
    if (m.ok_2_atoms(most_negative_ring_atom, most_positive_non_ring_atom)) {
      output << "mnrmpnr "
             << m.bonds_between(most_negative_ring_atom, most_positive_non_ring_atom)
             << '\n';
    }
    if (m.ok_2_atoms(most_negative_ring_atom, most_negative_non_ring_atom)) {
      output << "mnrmnnr "
             << m.bonds_between(most_negative_ring_atom, most_negative_non_ring_atom)
             << '\n';
    }
    if (m.ok_2_atoms(most_positive_ring_atom, most_negative_ring_atom)) {
      output << "mprmnr "
             << m.bonds_between(most_positive_ring_atom, most_negative_ring_atom) << '\n';
    }
    if (m.ok_2_atoms(most_positive_ring_atom, most_positive_non_ring_atom)) {
      output << "mprmpnr "
             << m.bonds_between(most_positive_ring_atom, most_positive_non_ring_atom)
             << '\n';
    }
    if (m.ok_2_atoms(most_positive_ring_atom, most_negative_non_ring_atom)) {
      output << "mprmnnr "
             << m.bonds_between(most_positive_ring_atom, most_negative_non_ring_atom)
             << '\n';
    }
    ...
    if (ring_heteroatom_count) {
      output << "avchrnghet "
             << sum_charge_ring_heteroatoms / static_cast<float>(ring_atom_count) << '\n';
    }
  }
...
static void
store_charge_assigner_results(const Molecule& m, const atomic_number_t* z,
                              const Atom** atom) {
  int npos = 0;
  int nneg = 0;
...
static int
iwdescriptors(Molecule& m, IWString_and_File_Descriptor& output, const atomic_number_t* z,
              int* ncon,  // not const because of charge assigner.
              const int* ring_membership, atom_number_t* conn, const Atom** atom,
              const int* is_aromatic, const char output_separator) {
  assert(m.ok());
  assert(output.good());
  ...
  int* already_done = new_int(matoms);
  std::unique_ptr<int[]> free_already_done(already_done);
  compute_topological_descriptors(m, z, ncon, ring_membership, atom, is_aromatic,
                                  already_done);
  ...
  if (descriptors_to_compute.donor_acceptor && donor_acceptor_assigner.active()) {
    int* da_results = new int[matoms];
    std::unique_ptr<int[]> free_da_results(da_results);
...
static int
iwdescriptors(Molecule& m, const char output_separator,
              IWString_and_File_Descriptor& output) {
  for (int i = 0; i < NUMBER_DESCRIPTORS; i++) {
    descriptor[i].reset();
  }
  ...
  int* nc = new int[matoms];
  std::unique_ptr<int[]> free_nc(nc);
  ...
  atom_number_t* conn = new atom_number_t[maxcon];
  std::unique_ptr<int[]> free_conn(conn);
  ...
  return iwdescriptors(m, output, z, nc, ring_membership, conn, atom, is_aromatic,
                       output_separator);
}
...
test_iwdescriptors(Molecule& m) {
  IWString* rsmi = new IWString[ntest];
  std::unique_ptr<IWString[]> free_rsmi(rsmi);
...
preprocess(Molecule& m) {
  if (1 == m.natoms() &&
      1 == m.atomic_number(
               0)) {  // we want to be able to compute a result for Hydrogen "molecule"
    return;
  }
...
static int
handle_zero_atom_molecule(const Molecule& m, IWString_and_File_Descriptor& output) {
  if (verbose) {
...
static int
iwdescriptors_alarm(data_source_and_type<Molecule>& input, const char output_separator,
                    IWString_and_File_Descriptor& output) {
  Molecule* m;
...
static int
iwdescriptors(data_source_and_type<Molecule>& input, const char output_separator,
              IWString_and_File_Descriptor& output) {
  assert(input.ok());
...
static int
iwdescriptors_filter(iwstring_data_source& input, const char output_separator,
                     IWString_and_File_Descriptor& output) {
  const_IWSubstring buffer;
...
static int
iwdescriptors_filter(const char* fname, const char output_separator,
                     IWString_and_File_Descriptor& output) {
  iwstring_data_source input(fname);
...
static int
iwdescriptors_rpipe_line(const const_IWSubstring& buffer, char output_separator,
                         IWString_and_File_Descriptor& output) {
  Molecule m;
  if (!m.build_from_smiles(buffer)) {
...
static int
iwdescriptors_rpipe(iwstring_data_source& input, char output_separator,
                    IWString_and_File_Descriptor& output) {
...
static int
iwdescriptors_rpipe(const char* fname, char output_separator,
                    IWString_and_File_Descriptor& output) {
  iwstring_data_source input(fname);
  if (!input.good()) {
...
static int
iwdescriptors(const char* fname, FileType input_type, const char output_separator,
              IWString_and_File_Descriptor& output) {
...
ReadDescriptorRanges(const const_IWSubstring& fname) {
  IWString tmp(fname);
  std::optional<w::Ranges> maybe_proto = iwmisc::ReadTextProto<w::Ranges>(tmp);
  if (!maybe_proto) {
...
DisplayFingerprintOptions(std::ostream& output) {
  ...
  output << " -G BEST           from calibration runs, certain descriptors have been "
            "designated\n";
  output << "                   as the 'best'. Use these designated to generate the "
            "fingperint\n";
  ...
  output << " replicates of that bit in the output. By default, all features get the "
            "same number\n";
}
...
iwdescr(int argc, char** argv) {
  ...
  if (cl.option_present('a')) {
    ...
    if (verbose) {
      cerr << "Will time out molecules that take more than " << alarm_time
           << " seconds\n";
    }
  }
  ...
  if (cl.option_present('G')) {
    ...
    while (cl.value('G', s, i++)) {
      ...
      for (int j = 0; s.nextword(dname, j, ',');) {
        ...
        cerr << "descriptor " << descriptor[k].descriptor_name() << " getting " << tmpr
  ...
  if (verbose) {
    ...
    if (number_filters) {
      cerr << rejected_by_filters << " molecules rejected by " << number_filters
           << " filters\n";
      for (int i = 0; i < number_filters; i++) {
  ...
  if (molecules_skipped_by_timer) {
    cerr << " Skipped " << molecules_skipped_by_timer << " molecules because of timer "
         << alarm_time << '\n';
  }
