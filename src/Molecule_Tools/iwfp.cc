...
#ifdef COUNT_TIMES_ATOM_IN_PATH
  cerr
      << "  -z <n>         perform <n> iterations of atom in path counter optimisation\n";
#endif
...
static int
do_append_sparse_fingerprints(const IWMFingerprint& fp, const IWString& tag,
...
static int
do_write_non_zero_bits(const Molecule& m, const IWMFingerprint& fp,
                       IWString& output_buffer) {
  if (m.name().empty()) {
    output_buffer << '.';
  } else {
...
static int
do_output_numbers(const Molecule& m, const IWMFingerprint& fp, IWString& output_buffer) {
  if (m.name().empty()) {
    output_buffer << '.';
  } else {
    append_first_token_of_name(m.name(), output_buffer);
...
static int
do_write_level2_fingerprint(IWMFingerprint& fp, IWString& output_buffer) {
...
static int
do_output(Molecule& m, IWMFingerprint& fp, IWString& output_buffer) {
  if (write_as_numbers) {
...
static int
iwfp_tester(Molecule& m, const IWMFingerprint& initial_fingerprint) {
  /// nt matoms = m.natoms();
  ...
  for (int i = 0; i < ntest; i++) {
    ...
    for (int j = 0; j < nbits; j++) {
      ...
      cerr << "Bit value difference, bit " << j << " expected " << expected_value[j]
           << " got " << myresult[j] << '\n';
      rc = 0;
    }
...
static int
write_header(IWString& output_buffer, int write_as_numbers) {
...
static int
iwfp(Molecule& m, IWString_and_File_Descriptor& output) {
  IWMFingerprint fp;
...
static int
iwfp(const const_IWSubstring& smi, IWString_and_File_Descriptor& output) {
  Molecule m;
...
static int
iwfp(iwstring_data_source& input, IWString_and_File_Descriptor& output) {
...
static int
iwfp(const char* fname, IWString_and_File_Descriptor& output) {
...
static int
iwfp(data_source_and_type<Molecule>& input, IWString_and_File_Descriptor& output) {
  Molecule* m;
...
static int
iwfp(const char* fname, FileType input_type, IWString_and_File_Descriptor& output) {
...
iwfp(int argc, char** argv) {
  Command_Line cl(argc, argv,
                  "vVc:E:aA:k:t:J:s:i:T:t:BfoN:O:g:lS:d:eM:x:Y:p:P:r:R:qz:U:");
  ...
  if (cl.option_present('S')) {
    if (!cl.option_present('f')) {
      cerr << "The -S option (input smiles tag) only makes sense with the -f (work as "
              "filter) option\n";
      usage(32);
    }
  ...
  if (cl.option_present('z')) {
    if (!cl.value('z', times_in_path_optimsation) || times_in_path_optimsation < 1) {
      cerr << "The number of times in fingerprints optimisation steps performed (-z) "
  ...
  if (cl.option_present('k')) {
    if (!cl.value('k', min_heteroatoms_at_ends) || min_heteroatoms_at_ends < 0 ||
        min_heteroatoms_at_ends > 2) {
      cerr << "The -k option must be followed by a whole number between 0 and 2\n";
      usage(3);
    }

    if (verbose) {
      cerr << "Paths must have at least " << min_heteroatoms_at_ends
           << " heteroatoms at their ends\n";
    }
  }
  ...
  if (write_sparse_fingerprints && iwmfingerprint_nbits() <= 2048) {
    cerr << "Warning, creating sparse fingerprints, but only " << iwmfingerprint_nbits()
         << " bits\n";
  }
  ...
  if (verbose) {
    ...
    if (bits_with_overflowing_counts) {
      cerr << bits_with_overflowing_counts
           << " bits overflowed 255 and generated overflow bits\n";
    }
  }
