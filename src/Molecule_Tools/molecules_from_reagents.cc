...
class Molecular_Properties {
 ...
 public:
  ...
  template <typename T>
  void compute_molecular_properties(T& m);
  template <typename T>
  void increment_molecular_properties(T& m);
  template <typename T>
  void increment_bonds(const T& m1, const resizable_array<bond_type_t>& bonds,
                       const T& m2);
...
void
Molecular_Properties::_increment_individual_bond(bond_type_t b, atomic_number_t z1,
                                                 atomic_number_t z2) {
  if (z1 > z2) {
    std::swap(z1, z2);
  }
...
Molecular_Properties::match(const Molecular_Properties& rhs) const {
  for (auto i = 0; i < NPROPERTIES; ++i) {
    //  cerr << "Molecular_Properties::match:i = " << i << " cmp " << _mp[i] << " and " <<
    //  rhs._mp[i] << endl;
...
class Aromatic_Properties {
 ...
 public:
  template <typename T>
  void compute_ring_and_aromatic_properties(T&);
  template <typename T>
  void increment_ring_and_aromatic_properties(T& m);
...
class Reagent : public Molecule {
 private:
  ...
  //  We do a substructure search to figure out which target molecules we could possibly
  //  match
 ...
 public:
  ...
  template <typename T>
  int establish_possible_matches_thread_safe(const resizable_array_p<T>& m,
  ...
  int
  is_set(const int b) const {
    return _can_match.is_set(b);
  }

  int
  can_match(int s) const {
    return _can_match.is_set(s);
  }
...
void
Reagent::compute_molecular_properties() {
  _mp.compute_molecular_properties(*this);

  // cerr << Molecule::name() << " computing properties " <<
  // examine_ring_related_properties << endl;
...
// Given `n` molecules allocate the _can_match array.
void
Reagent::AllocateCanMatch(int n) {
  int nb = n / 32 * 32;
  if (nb != n) {
    nb += 32;
  }
...
int
Reagent::establish_possible_matches(Molecule_to_Match* target, int n) {
...
Reagent::establish_possible_matches_thread_safe(const ::resizable_array_p<T>& m,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    //  else
    //    cerr << "Processing '" << mi.name() << " only matched " <<
...
class Molecule_to_Generate : public Molecule {
 ...
 public:
  ...
  const Molecular_Properties&
  mpr() const {
    return _mp;
  }

  const Aromatic_Properties&
  amp() const {
    return _amp;
  }
...
template <typename T>
int
Set_of_Molecules_to_Generate<T>::read_targets(const char* fname, FileType input_type) {
...
// clang-format off
  ...
  // clang-format on
  // clang-format off
  cerr << R"(Retrosynthetic tool. Examines two sets of (trimmed) reagents and a set of input molecules.
Reagents are filtered by those that are substructures of one or more of the input molecules.
Then look for pairs of reagents that can form any of the input molecules.
 -R <fname>    files of isotopically labelled reagents (sorted by atom count)
 -b <1,2,3>    bond types to use
 -x <n>        the reaction forms <n> extra rings compared to reagents
 -X <ele>      the reagents have extra <ele> atoms instead of isotopes
 -M <stem>     write reagents to files stem<filecount>
 -r <n>        report progress every <n> products
...
template <typename T>
int
replace_atoms_with_isotopes(T& m, const Element* e) {
  const auto matoms = m.natoms();
...
template <typename T>
int
replace_atoms_with_isotopes(resizable_array_p<T>& m, const Element* e) {
  for (T* t : m) {
    if (!replace_atoms_with_isotopes(*t, e)) {
      cerr << "No atoms of type '" << e->symbol() << "' in '" << t->name() << "'\n";
      return 0;
    }
  }
...
template <typename T>
void
remove_isotopes(resizable_array_p<T>& mols, const int niso) {
  for (T* m : mols) {
    int matoms = m->natoms();
    if (matoms > niso) {
      matoms = niso;
...
template <typename T>
int
write_set_of_molecules(const resizable_array_p<T>& mols,
...
// Add the indices of the non zero values in `v` to `nzero`.
void
...
int
are_the_same(const resizable_array<int>& a1, const resizable_array<int>& a2) {
  return a1 == a2;
}

template <typename T>
void
SortByAtomCount(const resizable_array_p<T>& mols) {
  std::sort(mols.begin(), mols.end(), [](const T* m1, const T* m2) {
    return m1->natoms() < m2->natoms();
  });
}

// Return true if the molecules in `mols` are sorted by atom count.
template <typename T>
int
...
template <typename T>
int
identify_isotopes_molecule(T& m, extending_resizable_array<int>& isotopes) {
  const auto matoms = m.natoms();

  Set_of_Atoms iso;

  for (auto i = 0; i < matoms; ++i) {
    const isotope_t j = m.isotope(i);
    if (0 == j) {
      continue;
    }
  ...
  // We need to get the atoms ordered with the isotopic atoms first, and in order of
  // increasing isotope First just move them to the front of the list
...
// Make sure that all molecules have the same number of isotopic atoms.
template <typename T>
int
identify_isotopes(const resizable_array_p<T>& mols,
  ...
  for (T* m : mols) {
    const auto niso = identify_isotopes_molecule(*m, isotopes);
    ...
    if (0 == rc) {
      ...
      cerr << "Found " << niso << " isotopic atoms in '" << m->name()
           << " but found " << rc << " in other molecules in the file\n";
      return 0;
    }
  }
...
template <typename T>
int
read_reagents(data_source_and_type<T>& input, resizable_array_p<T>& mols) {
  T* m;

  mols.reserve(1000);  // arbitrary number.
...
template <typename T>
int
read_reagents(const char* fname, FileType input_type, resizable_array_p<T>& mols) {
  if (input_type == FILE_TYPE_INVALID) {
    input_type = discern_file_type_from_name(fname);
    assert(0 != input_type);
  }
...
// Return the index of the highest non zero value in `f`.
static int
determine_highest_atom_count_to_find(const extending_resizable_array<int>& f) {
  for (auto [ndx, value] : std::ranges::views::enumerate(f | std::views::reverse)) {
    if (value != 0) {
      return ndx;
    }
  }
...
// Return the number of molecules in `mols` for which the found attribute is set.
static int
compute_nfound(const resizable_array_p<Molecule_to_Generate>& mols) {
  int rc = 0;
  for (const Molecule_to_Generate* m : mols) {
    if (m->found() > 0) {
      rc++;
    }
  }
...
static void
do_join(Molecule& m1, const Molecule& m2, const resizable_array<bond_type_t>& bonds) {
...
static int
find_a_matching_target_molecule(
    Molecule& m1, const IW_Bits_Base& m1_match_bits, Reagent& m2,
    const resizable_array<bond_type_t>& bonds,
...

{
  ...
  // cerr << "From '" << m1.smiles() << " and " << m2.smiles() << " what do we get, nb "
  // << bonds.size() << endl;
...
molecules_from_reactions(resizable_array_p<Molecule_to_Generate>& to_generate,
                         ...
                         const int nr, resizable_array_p<Reagent>* reagent,
                         const resizable_array<bond_type_t>& bonds,
                         IWString_and_File_Descriptor& output) {
  assert(2 == nr);
  ...
  const auto highest_atom_count_to_find =
      determine_highest_atom_count_to_find(atom_count_to_find);
  ...
  for (unsigned int i = 0; i < nr1; ++i) {
    ...
    for (unsigned int j = 0; j < nr2; ++j) {
      ...
      //    cerr << "Product of " << m.name() << " and " << mj->name() << " will contain "
      //    << (initial_matoms + mj->natoms()) << " atoms\n";
      ...
      //    cerr << "Passed atom count test " << m.smiles() << " " << r2[j]->smiles() <<
      //    endl;

      const auto f = find_a_matching_target_molecule(m, r1[i]->match_bits(), *(r2[j]),
                                                     bonds, to_generate, usmi_to_ndx);
      if (f >= 0) {
      ...
      if (report_progress()) {
        cerr << "Examined " << report_progress.times_called() << " molecules, found "
             << compute_nfound(to_generate) << " of " << to_generate.size() << ", joined "
             << molecules_joined << " molecules, completed " << i << " ("
             << initial_matoms << " atoms) of " << nr1 << " reagents\n";
      }
    }
  }
...
static int
any_reagents_can_make(const resizable_array_p<Reagent>& reagents, const int b) {
...
molecules_from_reactions(int argc, char** argv) {
  ...
  if (cl.option_present('x')) {
    ...
    if (verbose) {
      cerr << "Reaction results in " << extra_rings_formed
           << " extra rings compared to reagents\n";
    }
  }
  ...
  resizable_array_p<Reagent>* reagents = new resizable_array_p<Reagent>[nr];
  std::unique_ptr<resizable_array_p<Reagent>[]> free_reagents(reagents);
  ...
  extending_resizable_array<int>* isotopes_used = new extending_resizable_array<int>[nr];
  std::unique_ptr<extending_resizable_array<int>[]> free_isotopes_used(isotopes_used);
  ...
  for (int i = 0; i < nr; ++i) {
    ...
    for (int j = i + 1; j < nr; ++j) {
      ...
      if (!are_the_same(i1, i2)) {
        cerr << "Isotope count mismatch between reagent set " << i << " (" << i1.size()
             << ") and " << j << " (" << i2.size() << ")\n";
        return 2;
      }
    }
  ...
  if (verbose) {
    cerr << nr << " reagent sets, using " << niso << " isotopes, can generate "
         << combinations << " product molecules\n";
  }
  ...
  if (cl.option_present('b')) {
    if (niso != cl.option_count('b')) {
      cerr << "Reagents each have " << niso << " isotopes, but " << cl.option_count('b')
           << " bond making specifications, impossible\n";
      return 1;
    }
  ...
  for (const char* fname: cl) {
    if (!read_reagents(fname, input_type, to_generate)) {
  ...
  if (molecules_sorted_by_atom_count(to_generate)) {
    ...
    SortByAtomCount(to_generate);
  }

  const uint32_t ngenerate = to_generate.size();
  ...
  Molecular_Properties* mp = new Molecular_Properties[ngenerate];
  std::unique_ptr<Molecular_Properties[]> free_mp(mp);
  Aromatic_Properties* ap = new Aromatic_Properties[ngenerate];
  std::unique_ptr<Aromatic_Properties[]> free_ap(ap);
  for (unsigned int i = 0; i < ngenerate; ++i) {
...
#pragma omp for schedule(dynamic, 512) nowait
  ...
  if (1) {
    Molecule_to_Match* target = new Molecule_to_Match[ngenerate];
    std::unique_ptr<Molecule_to_Match[]> free_target(target);
    for (unsigned int i = 0; i < ngenerate; ++i) {
  ...
  // Remove reagents that cannot be used in making any product.
  for (int i = 0; i < nr; ++i) {
    auto& ri = reagents[i];
    ...
    for (int j = initial_size - 1; j >= 0; j--) {
      //    cerr << "Reagent set " << i << " reagent " << j << " '" << ri[j]->name() << "'
      //    nset " << ri[j]->nset() << endl;
      if (0 == ri[j]->nset()) {
        ri.remove_item(j);
      }
    ...
    if (verbose) {
      cerr << "Reagent set " << i << " started with " << initial_size << " reagents, "
           << final_size << " could match a product\n";
    }

    if (0 == final_size) {
      cerr << "Reagent set " << i
           << " does not match any of the proposed product molecules\n";
      return 2;
    }
  ...
  if (1)  // always
  {
    ...
    if (verbose) {
      cerr << "Dropped " << molecules_dropped << " of " << initial_size
           << " molecules due to lack of suitable reagents\n";
    }
  }

  IWString_and_File_Descriptor output(1);

  molecules_from_reactions(to_generate, atom_count_to_find, nr, reagents, bonds_to_add,
                           output);

  output.flush();

  if (verbose) {
    cerr << "Processed " << to_generate.size() << " molecules, found "
         << compute_nfound(to_generate) << endl;
