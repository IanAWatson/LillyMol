...
Reagent::establish_possible_matches(Molecule_to_Match* target, int n) {
  ...
  // cerr << " Searching " << this->smiles() << ' ' << this->name() << '\n';
...
Reagent::establish_possible_matches_thread_safe(const ::resizable_array_p<T>& m,
  ...
  for (auto i = 0; i < n; ++i) {
    ...
    if (mi.natoms() <=
        matoms) {  // clearly this reagent cannot be used to create that target
      continue;
    }
...
identify_isotopes(const resizable_array_p<T>& mols,
  ...
  for (unsigned int i = 0; i < n; ++i) {
    ...
    if (0 == rc) {
      ...
      cerr << "Found " << niso << " isotopic atoms in '" << mols[i]->name()
...
molecules_from_reactions(int argc, char** argv) {
  ...
  if (!molecules_sorted_by_atom_count(to_generate)) {
    cerr << "The set of molecules to be generated must be sorted by atom count, use "
            "msort\n";
    return 2;
  }
...
#pragma omp for schedule(dynamic, 512) nowait
  ...
  for (int i = 0; i < nr; ++i) {
    ...
    const int initial_size = ri.size();
