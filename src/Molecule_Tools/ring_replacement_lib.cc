...
Replacement::MakeVariant(Molecule& parent,
  ...
  // cerr << "Molecule now " << m->aromatic_smiles() << '\n';
...
bool
RingReplacement::set_ring_atom_smarts(const std::string& smarts) {
  std::unique_ptr<Substructure_Query> q = std::make_unique<Substructure_Query>();
  if (! q->create_from_smarts(smarts)) {
    cerr << "RingReplacement::set_ring_atom_smarts:cannot parse smarts '" << smarts << "'\n";
    return false;
  }
...
std::vector<Molecule>
RingReplacement::Process(Molecule& m) {
  std::vector<Molecule> result;  // empty for now
  ...
  for (uint32_t i = 1; i < generated.size(); ++i) {
    result.push_back(std::move(*generated[i]));
  }

  generated.resize_no_delete(0);
...
void
set_display_valence_error_messages(int s) {
  Replacement::_display_valence_error_messages = s;
}

void
set_warn_loss_of_aromaticity(int s) {
  Replacement::_warn_loss_of_aromaticity = s;
}

void
RingReplacement::set_min_support_requirement(uint32_t s) {
  _min_examples_needed = s;
}
...
uint32_t
RingReplacement::ReadReplacementRings(const std::string& fname) {
  if (_rings != nullptr) {
    cerr << "RingReplacement::ReadReplacementRings:replacement rings already defined\n";
    return 0;
  }

  IWString myfname(fname);
  myfname.ExpandEnvironmentVariablesInPlace();
  // cerr << "After ExpandEnvironmentVariablesInPlace '" << myfname << "'\n";

  if (! dash_s(myfname)) {
    IWString tmp;
    tmp << "${LILLYMOL_HOME}/data/ring_replacement/" << myfname;
    tmp.ExpandEnvironmentVariablesInPlace();
    if (! dash_s(tmp)) {
      cerr << "RingReplacement::ReadReplacementRings:cannot open '" << tmp << "'\n";
      return 0;
    }
  ...
  // cerr << "myfname '" << myfname << "'\n";

  _nreplacements = 0;
  _rings = new resizable_array_p<Replacement>[1];

  if (! ReadReplacementRings(myfname, 0)) {
    cerr << "RingReplacement::ReadReplacementRings:cannot read replacement rings "
         << myfname << '\n';
    return 0;
  }
