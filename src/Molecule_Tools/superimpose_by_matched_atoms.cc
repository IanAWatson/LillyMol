...
// By default we die if we encounter a molecule with zero coordinates.
static int ignore_zero_coordinates = 0;
...
static Accumulator<double> acc_initial_rms, acc_final_rms;

// If the -R option is specified, write the RMS values to here.
static IWString_and_File_Descriptor stream_for_rms;
static char rms_sep = ' ';
...
class Template_Molecule : public Molecule {
 private:
  ...
  double _report_rms(int template_embedding_number, const Molecule& m,
                     const Set_of_Atoms& m_embedding,
                     Accumulator<double>& acc, std::ostream& output) const;
  double _compute_rms(int ndx, const Molecule& m, const Set_of_Atoms& m_embedding,
                      double& maxd) const;

  int _write_matched_atoms(const int template_embedding, Molecule& m,
                           const Set_of_Atoms& e,
                           IWString_and_File_Descriptor& output) const;

  int _superimpose_by_matched_atoms(int template_embedding_number, Molecule& m,
                                    const Set_of_Atoms& e);
  int _superimpose_by_matched_atoms(Molecule& m, const Set_of_Atoms& e,
 ...
 public:
  ...
  int superimpose_by_matched_atoms(Molecule& m, const Set_of_Atoms& e,
                                   Molecule_Output_Object& output);

  //  int report_rms (const Molecule & m, const Set_of_Atoms & m_embedding, ostream &
  //  output) const;
};
...
int
Template_Molecule::identify_query_matches(
  ...
  for (int i = 0; i < queries.number_elements(); i++) {
    ...
    for (auto j = 0; j < nhits; ++j) {
      ...
      if (0 == default_matched_query_atoms.size()) {  // just take all the matched atoms
        *a = *e;
        ...
        for (unsigned int k = 0; k < default_matched_query_atoms.size(); k++) {
          const auto l = default_matched_query_atoms[k];
          if (!e->ok_index(l)) {
            cerr << "Template_Molecule::identify_query_matches:matched atom " << l
                 << " invalid index into embedding " << (*e) << '\n';
            return 0;
          }
          a->add(e->item(l));
        }
    ...
    if (verbose) {
      cerr << "Template molecule '" << Molecule::name() << "' " << nhits
           << " hits to substructure query\n";
    }
...
double
Template_Molecule::_report_rms(int template_embedding_number, const Molecule& m,
                               const Set_of_Atoms& m_embedding,
  ...
  output << Molecule::name() << " to " << m.name() << ' ' << m_embedding.size()
         << " matched atoms, RMS " << rms << ", max " << maxd << "\n";

  acc.extra(rms);

  if (stream_for_rms.is_open()) {
    stream_for_rms << Molecule::name() << rms_sep << m.name() << rms_sep
                   << static_cast<float>(rms) << rms_sep << 
                   static_cast<float>(maxd) << '\n';
    stream_for_rms.write_if_buffer_holds_more_than(4096);
  }
...
double
Template_Molecule::_compute_rms(int template_embedding_number, const Molecule& m,
                                const Set_of_Atoms& m_embedding, double& maxd) const {
  const Set_of_Atoms& template_embedding =
      *(_matched_query_atoms[template_embedding_number]);
...
static int
fill_a_array(const Molecule& m, const Set_of_Atoms& e, double* a) {
...
extern "C" void u3b_(const double* w, double* c1, double* c2, const long int* n,
                     const long int* mode, double* rms, double* u, double* t,
                     long int* ier);

int
Template_Molecule::_superimpose_by_matched_atoms(int template_embedding_number,
                                                 Molecule& m, const Set_of_Atoms& e) {
  const Set_of_Atoms& template_embedding =
      *(_matched_query_atoms[template_embedding_number]);
...
static void
write_atom(const char sep, const Molecule& m, const atom_number_t a,
           IWString_and_File_Descriptor& output) {
  output << m.name() << sep << a << sep << m.smarts_equivalent_for_atom(a) << sep
         << m.x(a) << sep << m.y(a) << sep << m.z(a);

  return;
}

int
Template_Molecule::_write_matched_atoms(const int template_embedding_number, Molecule& m,
                                        const Set_of_Atoms& e,
                                        IWString_and_File_Descriptor& output) const {
  const auto sep = ' ';

  output << "Template:" << sep << this->name() << sep << " embedding " << sep
         << template_embedding_number << sep << "molecule:" << sep << m.name() << '\n';
  ...
  output << "RMS" << sep << static_cast<float>(sqrt(sum / static_cast<double>(n))) << sep
         << "maxd" << sep << maxd << '\n';
...
Template_Molecule::_superimpose_by_matched_atoms(int template_embedding_number,
  ...
  if (report_rms_b4_and_after) {
    _report_rms(template_embedding_number, m, e, acc_initial_rms, cerr);
  }
  ...
  const auto rms = _report_rms(template_embedding_number, m, e, acc_final_rms, cerr);
...
int
Template_Molecule::_superimpose_by_matched_atoms(Molecule& m, const Set_of_Atoms& e,
                                                 Molecule_Output_Object& output) {
...
static int
superimpose_by_matched_atoms(Molecule& m, Molecule_Output_Object& output) {
  int rc = 0;
  ...
  for (int i = 0; i < queries.number_elements(); i++) {
    ...
    if (1 == nhits)
      ...
      cerr << nhits << " hits to query " << i << " for molecule '" << m.name()
           << "', not sure what to do, skipping\n";
      continue;
    }
...
ok_dimensionality(const Molecule& m) {
  ...
  return ignore_zero_coordinates;
}
...
static int
superimpose_by_matched_atoms(const char* fname, FileType input_type,
                             Molecule_Output_Object& output) {
...
superimpose_molecules(int argc, char** argv) {
  ...
  if (cl.option_present('z')) {
    ...
    for (int i = 0; cl.value('Z', z, i); ++i) {
      ...
      } else if (z == "ignorezero") {
        ignore_zero_coordinates = 1;
        if (verbose) {
          cerr << "Will ignore molecules with no coordinates\n";
        }
      } else if ("vH" == z) {
  ...
  if (cl.option_present('j')) {
    if (!cl.value('j', special_processing_for_jibo) ||
        special_processing_for_jibo <= 0.0) {
      cerr << "The weight for middle matched atoms (-j) must be a valid weight\n";
      usage(3);
    }

    if (verbose) {
      cerr << "Weight for matched atoms 1 and 2 set to " << special_processing_for_jibo
           << '\n';
    }
  }
  ...
  if (cl.option_present('R')) {
    ...
    if (! stream_for_rms.open(fname)) {
      cerr << "Cannot initialise stream for RMS '" << fname << "'\n";
      return 1;
    }
    if (verbose) {
      cerr << "Rms values written to '" << fname << "'\n";
    }
    stream_for_rms << "Target" << rms_sep << "Name" << rms_sep << "RMS" << rms_sep << "Maxd\n";
  }
  ...
  if (verbose) {
    ...
    if (report_rms_b4_and_after) {
      cerr << "Initial RMS btw " << acc_initial_rms.minval() << " and " <<
               acc_initial_rms.maxval() << " ave " << static_cast<float>(acc_initial_rms.average()) << '\n';
    }
    cerr << "Final   RMS btw " << acc_final_rms.minval() << " and " <<
             acc_final_rms.maxval() << " ave " << static_cast<float>(acc_final_rms.average()) << '\n';
  }
