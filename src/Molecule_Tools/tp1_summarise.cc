...
// clang-format off
  ...
  // clang-format on
  // clang-format off
  cerr << R"(Processes results from LillyMedchemRules into different outputs and summary statistics.
 -r             include the rejection reason with the output,
 -z             include zero demerit molecules in the output,
 -h             include a header record,
 -D             exclude the D prefix,
 -B <stem>      bad file stem (default 'bad'),
 -s <string>    separator between output fields,
 -u             change spaces in reason fields to underscores,
 -t             give report of which reasons hit,
 -T <fname>     write report on rejection reasons to <fname>,
 -b             in the -T file, produce two column format rejections | demerits,
 -X             produce table in LaTex format,
 -A             produce table in AsciiDoc format,
 -M             produce table in Markdown format,
 -m <n>         in the table file, discard any reason with fewer than <n> examples,
 -c             produce a demerit based scale factor file,
 -f <n>         numeric demerit value for rejections (default 100),
 -k             only process the survivors file (do not process bad0, bad1...),
 -w             the -tabular option was used when running the rules,
                  there is no information about demerit query results,
 -v             verbose output,
)";
  // clang-format on
...
static int
write_demerit_value(const const_IWSubstring& id, const int demerit,
...
static int
process_from_iwdemerit_tabular(const const_IWSubstring& buffer, int must_have_demerit,
                               IWString_and_File_Descriptor& output) {
  IWString id;
  ...
  if (last_token.empty()) [[unlikely]] {
    cerr << "process_from_iwdemerit_tabular::cannot retrieve last column, assuming zero "
            "demerits\n";
    demerits = 0;
...
static int
process_from_iwdemerit(const const_IWSubstring& buffer, int must_have_demerit,
                       IWString_and_File_Descriptor& output) {
...
static int
process_from_iwdemerit(iwstring_data_source& input, int must_have_demerit,
                       IWString_and_File_Descriptor& output) {
...
static int
process_bad12(const const_IWSubstring& buffer, IW_STL_Hash_Map_int& reason,
              IWString_and_File_Descriptor& output) {
  int i = 0;
  const_IWSubstring token;
...
static int
process_bad12(iwstring_data_source& input, IW_STL_Hash_Map_int& reason,
              IWString_and_File_Descriptor& output) {
  const_IWSubstring buffer;
  while (input.next_record(buffer)) {
...
static int
process_bad12(const IWString& fname, IW_STL_Hash_Map_int& reason,
              IWString_and_File_Descriptor& output) {
...
static int
process_bad1(const IWString& bad_stem, IW_STL_Hash_Map_int& reason,
             IWString_and_File_Descriptor& output) {
  IWString fname;
...
static int
process_bad2(const IWString& bad_stem, IW_STL_Hash_Map_int& reason,
             IWString_and_File_Descriptor& output) {
  IWString fname;
...
static int
process_from_iwdemerit(const char* fname, int must_have_demerit,
                       IWString_and_File_Descriptor& output) {
  iwstring_data_source input(fname);
  if (!input.good()) {
    cerr << "Cannot open '" << fname << "'\n";
...
static int
process_from_iwdemerit(const IWString& bad_stem, IWString_and_File_Descriptor& output,
...
static int
process_bad0(const const_IWSubstring& buffer, IW_STL_Hash_Map_int& reason,
             IWString_and_File_Descriptor& output) {
  int i = 0;
  const_IWSubstring token;
...
static int
process_bad0(iwstring_data_source& input, IW_STL_Hash_Map_int& reason,
             IWString_and_File_Descriptor& output) {
  const_IWSubstring buffer;
  while (input.next_record(buffer)) {
...
static int
process_bad01(const IWString& fname, IW_STL_Hash_Map_int& reason,
              IWString_and_File_Descriptor& output) {
  iwstring_data_source input(fname);
...
static int
process_bad0(const IWString& bad_stem, IW_STL_Hash_Map_int& reason,
             IWString_and_File_Descriptor& output) {
  IWString fname;
...
static int
all_files_present(const char* okfile, const IWString& bad_stem) {
  for (int i = 0; i < 3; i++) {
    IWString fname;
    fname << bad_stem << i << ".smi";

    if (verbose > 1) {
      cerr << "Checking '" << fname << "', result "
           << dash_f(fname.null_terminated_chars()) << endl;
    }
...
static int
process_single_run(const char* okfile, const IWString& bad_stem, int ndx,
                   IWString_and_File_Descriptor& output) {
...
template <typename T>
void
WriteMarkdownCell(T& os, const IWString& value) {
...
template <typename T>
int
Reason_and_Count::markdown_table(T& os) const {
  os << _count << " | ";
  WriteMarkdownCell(os, _reason);
...
template <typename T>
void
write_rejection_reason(const resizable_array_p<Reason_and_Count>& r, const int ndx,
                       T& output) {
  if (ndx < r.number_elements()) {
    ...
    } else if (markdown_table) {
      r[ndx]->markdown_table(output);
    } else {
      output << *(r[ndx]);
    }
  } else {
...
write_reasons(const IW_STL_Hash_Map_int& reason_bad,
              ...
              const int min_reasons_needed_for_output, T& output) {
...
template <typename T>
int
write_reasons(const IW_STL_Hash_Map_int& reason, const int min_reasons_needed_for_output,
              T& output) {
  resizable_array_p<Reason_and_Count> r;
...
tp1_summarise(int argc, char** argv) {
  Command_Line cl(argc, argv, "vB:S:rhzDs:tT:uj:f:XAP:cnkbm:wM");
  ...
  if (cl.option_present('c')) {
    ...
    if (verbose) {
      cerr << "Will produce a file where demerit values have been converted to scaling "
              "factors\n";
    }
  }
  ...
  if (latex_table + asciidoc_table + markdown_table > 1) {
    cerr << "Only one of -X, -A or -M can be specified\n";
    return 1;
  }
  ...
  if (accumulate_reasons) {
    cerr << "Encountered " << (reason_bad.size() + reason_survivor.size())
         << " different reasons\n";

    int min_reasons_needed_for_output = 0;

    if (cl.option_present('m')) {
      if (!cl.value('m', min_reasons_needed_for_output) ||
      ...
      if (verbose) {
        cerr << "Will not write any rejection/demerit reason to the -T file with fewer "
                "than "
             << min_reasons_needed_for_output << endl;
      }
    }

    if (nullptr == fname_for_reason_summary) {
      for (IW_STL_Hash_Map_int::const_iterator i = reason_bad.begin();
           i != reason_bad.end(); ++i) {
        if (i->second >= min_reasons_needed_for_output) {
          cerr << (*i).second << " occurrences of '" << (*i).first << "'\n";
        }
      }
      for (IW_STL_Hash_Map_int::const_iterator i = reason_survivor.begin();
           i != reason_survivor.end(); ++i) {
        if (i->second >= min_reasons_needed_for_output) {
          cerr << (*i).second << " occurrences of '" << (*i).first << "'\n";
        }
      }
      ...
      if (cl.option_present('b')) {
        ...
        if (verbose) {
          cerr << reason_bad.size() << " reasons associated with rejected molecules, "
