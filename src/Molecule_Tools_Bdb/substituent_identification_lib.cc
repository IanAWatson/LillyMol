...
struct ReplacementProduct {
  ...
  Molecule& fragment_added_molecule;
  const DBKey& rad_and_bit;
  const IWString& fragment_lost;
  const const_IWSubstring& fragment_added;
};
...
class SubstituentIdentification {
 private:
  ...
  // If non-null, normal replacement products are collected here instead of
  // being written to the command line output stream.
  std::vector<SubstituentReplacement>* _collected_replacements;
  ...
  int AddFragment(Molecule& m, atom_number_t zatom,
                  ...
                  bond_type_t bt) const;
  SubstituentReplacement MakeReplacementProductData(
      const ReplacementProduct& product) const;
  void EmitProduct(const ReplacementProduct& product,
                   IWString_and_File_Descriptor& output);
  int WriteProduct(const SubstituentReplacement& product,
 ...
 public:
  ...
  void set_only_produce_molecules_at_biggest_radius(int s) {
    _only_produce_molecules_at_biggest_radius = s;
  }
  void set_break_molecule_at_first_two_matched_atoms(int s) {
    _break_molecule_at_first_two_matched_atoms = s;
  }
  void set_matched_atoms_to_process(int s) {
    _matched_atoms_to_process = s;
  }
  void set_min_substituent_size(int s) {
    _min_substituent_size = s;
  }
  void set_max_substituent_size(int s) {
    _max_substituent_size = s;
  }
  void set_max_atoms_in_product(int s) {
    _max_atoms_in_product = s;
  }
  void set_min_examples_needed_for_addition(uint32_t s) {
    _min_examples_needed_for_addition = s;
  }
  void set_max_molecules_per_input_molecule(uint32_t s) {
    _max_molecules_per_input_molecule = s;
  }
  void set_remove_isotopes_from_product(int s) {
    _remove_isotopes_from_product = s;
  }
  void set_write_fragments_added(int s) {
    _write_fragments_added = s;
  }

  int GenerateReplacements(Molecule& m,
                           std::vector<SubstituentReplacement>& results);
...
SubstituentIdentification::_default_values() {
  ...
  _collected_replacements = nullptr;
...
void
SubstituentIdentification::CloseDatabases() {
  if (nullptr == _dbs) {
    return;
  }
...
int
SubstituentIdentification::InitialiseDefaultAtomTyping() {
  return _atom_typing_specification.build("UST:AFUY");
}
...
int
SubstituentIdentification::GenerateReplacements(
    Molecule& m, std::vector<SubstituentReplacement>& results) {
  UsedDuringLookups lookup_data;
  lookup_data.PrepareForNewMolecule();

  _collected_replacements = &results;
  const int write_parent_molecule = _write_parent_molecule;
  _write_parent_molecule = 0;

  IWString_and_File_Descriptor output(1);
  const int rc = _look_for_new_substituents(m, lookup_data, output);

  _write_parent_molecule = write_parent_molecule;
  _collected_replacements = nullptr;

  return rc;
}

std::vector<SubstituentReplacement>
SubstituentIdentificationLookup::GenerateReplacements(Molecule& m) {
  std::vector<SubstituentReplacement> results;
  _impl->GenerateReplacements(m, results);
  return results;
}
...
SubstituentIdentification::FormNewMoleculeAtAtomWithReplacement(Molecule& m, atom_number_t zatom,
  ...
  if (!m.valence_ok()) {
    ...
    ReplacementProduct product{m, proto, frag, rad_and_bit, fragment_lost.smiles, smiles};
    EmitProduct(product, output);
  }
...
SubstituentReplacement
SubstituentIdentification::MakeReplacementProductData(
    const ReplacementProduct& product) const {
  SubstituentReplacement result;
  result.molecule = product.product;
  result.smiles = product.product.smiles().AsString();
  result.name = product.product.name().AsString();
  result.donor = product.replacement.id(0);
  result.radius = product.rad_and_bit.radius;
  result.examples = product.replacement.n();
  result.fragment_lost = product.fragment_lost.AsString();
  result.fragment_added = product.fragment_added.AsString();

  return result;
}

void
SubstituentIdentification::EmitProduct(const ReplacementProduct& product,
                                       IWString_and_File_Descriptor& output) {
  SubstituentReplacement replacement = MakeReplacementProductData(product);

  if (_collected_replacements != nullptr) {
    _collected_replacements->push_back(std::move(replacement));
  } else {
    WriteProduct(replacement, output);
...
int
SubstituentIdentification::WriteProduct(const SubstituentReplacement& product,
                                        IWString_and_File_Descriptor& output) const {
  if (_output_is_textproto) {
    substituent_identification::Result result;
    result.set_smi(product.smiles);
    result.set_id(product.name);
    result.set_donor(product.donor);
    result.set_rad(product.radius);
    result.set_n(product.examples);
    if (_write_fragments_added) {
      result.set_frag(product.fragment_lost);
    }
    ...
    if (! printer.PrintToString(result, &buffer)) {
      cerr << "SubstituentIdentification::WriteProduct:cannot write '"
  ...
  output << product.donor << ":R" << product.radius << " N:" << product.examples;

  if (_write_fragments_added) {
    output << ' ' << product.fragment_lost << '%' << product.fragment_added;
  }
...
SubstituentIdentification::FormNewMoleculeAtAtom(Molecule& m, atom_number_t zatom,
  ...
  ReplacementProduct product{m, proto, frag, rad_and_bit,
                             lookup_data.smiles_of_fragment_lost(), smiles};
  EmitProduct(product, output);
...
SubstituentIdentification::AddDatabaseForLookup(const std::string& dbname) {
  ...
  new_dbs[_ndb] = db.release();

  delete[] _dbs;
  _dbs = new_dbs.release();
  ++_ndb;
...
SubstituentIdentificationLookup::SubstituentIdentificationLookup()
    : _impl(std::make_unique<SubstituentIdentification>()) {
  _impl->InitialiseDefaultAtomTyping();
}
...
void
SubstituentIdentificationLookup::close() {
...
void
SubstituentIdentificationLookup::set_default_new_molecule_starting_points(int s) {
  _impl->set_default_new_molecule_starting_points(s);
}

void
SubstituentIdentificationLookup::set_min_shell_radius(int s) {
  _impl->set_min_shell_radius(s);
}

void
SubstituentIdentificationLookup::set_only_produce_molecules_at_biggest_radius(int s) {
  _impl->set_only_produce_molecules_at_biggest_radius(s);
}

void
SubstituentIdentificationLookup::set_break_molecule_at_first_two_matched_atoms(int s) {
  _impl->set_break_molecule_at_first_two_matched_atoms(s);
}

void
SubstituentIdentificationLookup::set_matched_atoms_to_process(int s) {
  _impl->set_matched_atoms_to_process(s);
}

void
SubstituentIdentificationLookup::set_min_substituent_size(int s) {
  _impl->set_min_substituent_size(s);
}

void
SubstituentIdentificationLookup::set_max_substituent_size(int s) {
  _impl->set_max_substituent_size(s);
}

void
SubstituentIdentificationLookup::set_max_atoms_in_product(int s) {
  _impl->set_max_atoms_in_product(s);
}

void
SubstituentIdentificationLookup::set_min_examples_needed_for_addition(uint32_t s) {
  _impl->set_min_examples_needed_for_addition(s);
}

void
SubstituentIdentificationLookup::set_max_molecules_per_input_molecule(uint32_t s) {
  _impl->set_max_molecules_per_input_molecule(s);
}

void
SubstituentIdentificationLookup::set_remove_isotopes_from_product(int s) {
  _impl->set_remove_isotopes_from_product(s);
}

void
SubstituentIdentificationLookup::set_write_fragments_added(int s) {
  _impl->set_write_fragments_added(s);
}

int
SubstituentIdentificationLookup::GenerateReplacements(
    Molecule& m, std::vector<SubstituentReplacement>& results) {
  return _impl->GenerateReplacements(m, results);
}

int
SubstituentIdentificationMain(int argc, char** argv) {
  prog_name = argv[0];

  SubstituentIdentification substituent_identification;

  return substituent_identification(argc, argv);
}
