...
static int
choose_next_leader(const int* selected, int n) {
  for (int i = 0; i < n; i++) {
...
static int
choose_next_leaders(const int* selected, int n, int* leaders, int leaders_requested) {
  int leader_count = 0;
...
leader() {
  ...
  while ((icentre = choose_next_leader(selected + start, pool_size - start)) >= 0) {
    ...
    if (report_progress()) {
      cerr << "Formed " << cluster_id << " clusters, "
           << count_non_zero_occurrences_in_array(selected, pool_size)
           << " items in clusters\n";
    }
...
leader(int* leaders, int leaders_requested, float* cand_distances) {
  ...
  while ((leaders_found = choose_next_leaders(selected + start, pool_size - start,
                                              leaders, leaders_requested))) {
    for (int i = 0; i < leaders_found; i++) {
      leaders[i] += start;
    }

    if (1 == leaders_found || fingerprints[leaders[0]].tanimoto_distance(
                                  fingerprints[leaders[1]]) <= threshold) {
      cluster_id++;
...
#pragma omp parallel for schedule(dynamic, 256)
      ...
      // Both of these will end up being actual centers so amortize FPDB load costs by
      // comparing everything else to both
      ...
      for (int i = start; i < pool_size; i++) {
        if (!selected[i]) {
          fingerprints[i].tanimoto_distance_2(fingerprints[leaders[0]],
                                              fingerprints[leaders[1]], cand_distances);
...
static int
do_previously_selected(const IW_General_Fingerprint& gfp, float threshold) {
  GFP_Standard sgfp;
...
static int
do_previously_selected(iwstring_data_source& input, float threshold) {
  IW_TDT tdt;
...
static int
do_previously_selected(const char* fname, float threshold) {
...
static int
WriteNeighboursAsProtos(int clusters_formed, IWString_and_File_Descriptor& output) {
  int start = 0;
...
allocate_pool(int s) {
  ...
  if (nullptr == leader_item || nullptr == selected || nullptr == fingerprints ||
      nullptr == distances) {
...
gfp_leader_standard(int argc, char** argv) {
  ...
  if (cl.option_present('p')) {
    ...
    int* leaders = new int[p];
    std::unique_ptr<int[]> free_leaders(leaders);
