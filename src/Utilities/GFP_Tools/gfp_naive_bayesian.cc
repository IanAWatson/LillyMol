...
static unsigned int
bit_number_adjusted_for_count(unsigned int b, int& c) {
  if (1 == c) {
    return b;
  }
...
static int
read_activity_data_record(const const_IWSubstring& buffer, IW_STL_Hash_Map_int& activity,
...
static int
read_activity_data(iwstring_data_source& input, IW_STL_Hash_Map_int& activity,
                   resizable_array_p<IWString>& class_names, IWString& activity_name) {
  IW_STL_Hash_Map_int string_to_class_number;
  ...
  while (input.next_record(buffer)) {
    ...
    if (!read_activity_data_record(buffer, activity, string_to_class_number,
                                   class_names)) {
...
static int
read_activity_data(const const_IWSubstring& fname, IW_STL_Hash_Map_int& activity,
                   resizable_array_p<IWString>& class_names, IWString& activity_name) {
...
class FP_And_Bit_Hasher {
 public:
  size_t
  operator()(const FP_And_Bit& f) const {
    return f.second;
  }
};
...
class Fingerprint_Info_Read_From_Model {
 private:
  ...
  int _matches(const int ndx, const IWString& tag,
 ...
 public:
  ...
  int
  number_fingerprints() const {
    return _fx.size() + _sp.size();
  }
...
Fingerprint_Info_Read_From_Model::add(const const_IWSubstring& buffer) {
  if (3 != buffer.nwords()) {
    cerr << "Fingerprint_Info_Read_From_Model::add:invalid fingerprint specification '"
         << buffer << "'\n";
    return 0;
  }
  ...
  if (GFP_NB_SP == fptype) {
    ...
    cerr << "Fingerprint_Info_Read_From_Model::add:unrecognised fingerprint type '"
         << buffer << "'\n";
    return 0;
  }
...
int
Fingerprint_Info_Read_From_Model::_add(const int ndx, const const_IWSubstring& tag,
...
int
Fingerprint_Info_Read_From_Model::_matches(
    const int ndx, const IWString& tag,
    const std::unordered_map<int, IWString>& ndx_to_tag) const {
  const auto f = ndx_to_tag.find(ndx);
  if (f == ndx_to_tag.end()) {
...
Fingerprint_Info_Read_From_Model::check_fingerprint_validity() const {
  ...
  for (int i = 0; i < ::number_fingerprints(); ++i) {
    if (!_matches(i, fixed_fingerprint_tag(i), _fx)) {
      cerr << "Fingerprint_Info_Read_From_Model::check_fingerprint_validity:fixed size "
              "fingerprint mismatch, i = "
           << i << " tag " << fixed_fingerprint_tag(i) << endl;
      rc = 0;
    }
  }
  ...
  for (int i = 0; i < ::number_sparse_fingerprints(); ++i) {
    if (!_matches(i, sparse_fingerprint_tag(i), _sp)) {
      cerr << "Fingerprint_Info_Read_From_Model::check_fingerprint_validity:sparse "
              "fingerprint mismatch, i = "
           << i << " tag " << sparse_fingerprint_tag(i) << endl;
      rc = 0;
    }
  }
  ...
  cerr << "Fingerprint_Info_Read_From_Model::check_fingerprint_validity:inconsistent "
          "fingerprints\n";

  for (auto i : _fx) {
    cerr << i.first << " is " << fixed_fingerprint_tag(i.first) << " model expected "
         << i.second << endl;
  }

  for (auto i : _sp) {
    cerr << i.first << " is " << sparse_fingerprint_tag(i.first) << " model expected "
         << i.second << endl;
  }
...
class Bayesian_Classification {
 private:
  ...
  int _score_while_writing_bit_contributions(const IW_General_Fingerprint& fp, double* rc,
                                             IWString_and_File_Descriptor& output) const;
  int _write_bit_weight(const int ndx, const IWString* class_label,
                        IWString_and_File_Descriptor& output) const;
 ...
 public:
  ...
  int do_write(const IWString* class_label, const IWString& activity_name,
               const char* fname) const;
  int do_write(const IWString* class_label, const IWString& activity_name,
               IWString_and_File_Descriptor& output) const;

  int build(const char* fname, IWString*& cname, IWString& activity_name,
            Fingerprint_Info_Read_From_Model& firfm);
  int build(iwstring_data_source& input, IWString*& cname, IWString& activity_name,
            Fingerprint_Info_Read_From_Model& firfm);
};
...
Bayesian_Classification::initialise(const IW_STL_Hash_Map_int& activity) {
  ...
  if (0 == _nclasses) {
    cerr << "Bayesian_Classification::initialise:no classes in " << activity.size()
         << " data values\n";
    return 0;
  }
...
static int
increment_bit(const int fpi, const unsigned int b, Bit_Frequency& bit_frequency,
              int c = 1) {
...
static int
count_occurrences(IW_General_Fingerprint& fp, Bit_Frequency& bit_frequency) {
...
int
Bayesian_Classification::determine_bit_counts(IW_General_Fingerprint& fp, const int c) {
  _items_in_class[c]++;
  _n++;
...
Bayesian_Classification::determine_bit_counts(const char* fname,
  ...
  if (!input.good()) {
    cerr << "Bayesian_Classification::determine_bits_set:cannot open fingerprint file '"
         << fname << "'\n";
    return 0;
  }
...
int
Bayesian_Classification::determine_bit_counts(
    IW_TDT& tdt, const IW_STL_Hash_Map_int& activity,
    const IW_STL_Hash_Set& training_set_identifiers,
    resizable_array_p<IW_General_Fingerprint>& test_set_fingerprints) {
...
int
Bayesian_Classification::determine_bit_counts(
    iwstring_data_source& input, const IW_STL_Hash_Map_int& activity,
    const IW_STL_Hash_Set& training_set_identifiers,
    resizable_array_p<IW_General_Fingerprint>& test_set_fingerprints) {
  IW_TDT tdt;
  while (tdt.next(input)) {
    if (!determine_bit_counts(tdt, activity, training_set_identifiers,
                              test_set_fingerprints)) {
...
Bayesian_Classification::determine_bit_counts(
  ...
  return determine_bit_counts(input, activity, training_set_identifiers,
                              test_set_fingerprints);
}
...
Bayesian_Classification::initialise_bit_weights() {
  ...
  for (auto i = 0; i < _nclasses; ++i) {
    _baseline_class_probability[i] =
        static_cast<double>(_items_in_class[i]) / static_cast<double>(_n);

    if (verbose) {
      cerr << "Class contains " << _items_in_class[i] << " items, p "
           << _baseline_class_probability[i] << endl;
    }
  ...
  if (verbose > 1) {
    cerr << "Global bit frequency contains " << global_bit_frequency.size()
         << " features\n";
  }
  ...
  for (auto i = global_bit_frequency.begin(); i != global_bit_frequency.end(); ++i) {
    for (auto cls = 0; cls < _nclasses; ++cls) {
      ...
      double w = (static_cast<double>(items_with_bit) + laplace_correction_factor) /
                 (static_cast<double>(i->second) * _baseline_class_probability[cls] +
                  laplace_correction_factor);
      //    double w = (static_cast<double>(items_with_bit) + laplace_correction_factor) /
      //    ((static_cast<double>(i->second)+laplace_correction_factor) *
      //    _baseline_class_probability[cls]); double w =
      //    (static_cast<double>(items_with_bit) + laplace_correction_factor) /
      //    ((static_cast<double>(i->second)) * _baseline_class_probability[cls]);

#ifdef BIT_EXAMINATION_ENABLED
      if (examine_all_bits || bits_to_examine.size()) {
        if (examine_all_bits || need_to_examine_bit(bits_to_examine, i->first.second)) {
          cerr << "fp " << i->first.first << " bit " << i->first.second << " in class "
               << cls << " items_with_bit " << items_with_bit << " total bit count "
               << i->second << " baseline " << _baseline_class_probability[cls] << ", w "
               << w << " logw " << log(w) << endl;
          //        cerr << i->first.second << ' ' << items_with_bit << ' ' << i->second
          //        << ' ' << _baseline_class_probability[cls] << ' ' << w << "\n";
        }
      }
#endif

#ifdef DEBUG_IWNB
      cerr << "fp " << i->first.first << " bit " << i->first.second << " in class " << cls
           << " " << items_with_bit << " times, total " << i->second << ", w " << w
           << " logw " << log(w) << endl;
      cerr << i->first.second << ' ' << items_with_bit << ' ' << i->second << ' '
           << _baseline_class_probability[cls] << ' ' << w << "\n";
#endif
...
/*
  This is complicated by the fact that we must look across all classes to figure out the
  number of instances of each fingerprint
*/
...
int
Bayesian_Classification::_score_while_writing_bit_contributions(
    const IW_General_Fingerprint& fp, double* rc,
    IWString_and_File_Descriptor& output) const {
  output << "ID: " << fp.id() << '\n';
...
int
Bayesian_Classification::score(IW_TDT& tdt, double* rc) const {
  int fatal;
...
int
Bayesian_Classification::score(iwstring_data_source& input, double* rc) const {
  IW_TDT tdt;
...
int
Bayesian_Classification::build(const char* fname, IWString*& cname,
...
int
Bayesian_Classification::_write_bit_weight(const int ndx, const IWString* class_label,
                                           IWString_and_File_Descriptor& output) const {
  output << GFP_NB_CLASS << class_label[ndx] << '\n';

  for (auto i : _bit_weight[ndx]) {
    output << output_separator << i.first.first << output_separator << i.first.second
           << output_separator << i.second << '\n';
    output.write_if_buffer_holds_more_than(4096);
  }
  output << "|\n";
...
int
Bayesian_Classification::_read_bit_weight(Bit_Weight& bw, iwstring_data_source& input) {
  const_IWSubstring buffer;
  while (input.next_record(buffer)) {
    if ('|' == buffer) {
      break;
    ...
    if (!token.numeric_value(fp) || fp < 0) {
      cerr << "Bayesian_Classification::_read_bit_weight:invalid fp specification '"
           << buffer << "'\n";
      return 0;
    }
    ...
    if (f != bw.end()) {
      cerr << "Bayesian_Classification::_read_bit_weight:duplicate bit information '"
           << buffer << "'\n";
      return 0;
    }
...
int
Bayesian_Classification::build(iwstring_data_source& input, IWString*& class_label,
                               IWString& activity_name,
                               Fingerprint_Info_Read_From_Model& firfm) {
  const_IWSubstring buffer;
  ...
  if (GFP_NB_HEADER != buffer) {
    cerr << "Bayesian_Classification::build:header record not correct, expected '"
         << GFP_NB_HEADER << "', got '" << buffer << "'\n";
    return 0;
  }
  ...
  if (!buffer.starts_with(GFP_NB_ACT)) {
    cerr << "Bayesian_Classification::build:expected '" << GFP_NB_ACT << "' got '"
         << buffer << "'\n";
    return 0;
  }
  ...
  while (input.next_record(buffer)) {
    if (buffer.starts_with(GFP_NB_FX)) {
      if (!firfm.add(buffer)) {
        cerr << "Bayesian_Classification::build:invalid fixed size fingerprint "
                "specification '"
             << buffer << "'\n";
        return 0;
      }
      ...
      if (!firfm.add(buffer)) {
        cerr
            << "Bayesian_Classification::build:invalid sparse fingerprint specification '"
            << buffer << "'\n";
        return 0;
      }
    } else {
  ...
  if (!buffer.starts_with(GFP_NB_CLASSES)) {
    cerr << "Bayesian_Classification::build:class labels, not '" << GFP_NB_CLASSES
         << "', got '" << buffer << "'\n";
    return 0;
  }

  _nclasses =
      buffer.nwords() - 1;  // must be class record indicator and at least two classes

  if (_nclasses < 2) {
    cerr << "Bayesian_Classification::build:invalid class label record - must be at "
            "least 3 tokens '"
         << buffer << "'\n";
    return 0;
  }
  ...
  if (!buffer.starts_with(GFP_NB_BP)) {
    cerr << "Bayesian_Classification::build:expected '" << GFP_NB_BP << "' got '"
         << buffer << "', confused\n";
    return 0;
  }
  ...
  IWString* tmps = new IWString[_nclasses];
  std::unique_ptr<IWString[]> free_tmps(tmps);

  i = 0;
  buffer.nextword(token, i);

  for (int j = 0; buffer.nextword(token, i); ++j) {
    if (!token.numeric_value(_baseline_class_probability[j]) ||
        _baseline_class_probability[j] < 0.0 || _baseline_class_probability[j] > 1.0) {
  ...
  for (int i = 0; i < _nclasses; ++i) {
    ...
    if (!buffer.starts_with(GFP_NB_CLASS)) {
      cerr << "Bayesian_Classification::build:expecting class identifier '"
           << GFP_NB_CLASS << "', got '" << buffer << "'\n";
      return 0;
    }

    if (2 != buffer.nwords()) {
      cerr << "Bayesian_Classification::build:class information must contain "
           << GFP_NB_CLASS << " and class label, '" << buffer << "' not recognised\n";
      return 0;
    }
    ...
    if (f == label_to_ndx.end()) {
      cerr << "Bayesian_Classification::build:huh, where did class label '" << token
           << "' from '" << buffer << "' come from?\n";
      return 0;
    }

    if (!_read_bit_weight(_bit_weight[f->second], input)) {
      cerr << "Bayesian_Classification::build:cannot read bit weight data, after line "
...
Bayesian_Classification::do_write(const IWString* class_label,
  ...
  for (int i = 0; i < number_fingerprints(); ++i) {
    output << GFP_NB_FX << output_separator << i << output_separator
           << fixed_fingerprint_tag(i) << '\n';
  }

  for (int i = 0; i < number_sparse_fingerprints(); ++i) {
    output << GFP_NB_SP << output_separator << i << output_separator
           << sparse_fingerprint_tag(i) << '\n';
  }
...
int
Output_Creator::initialise(const IWString* cname, int nc) {
  _nclasses = nc;
...
int
Output_Creator::do_output(const IW_TDT& tdt, const double* c,
                          IWString_and_File_Descriptor& output) {
  IWString id;
...
gfp_naive_bayesian(iwstring_data_source& input,
                   Bayesian_Classification& bayesian_classification,
                   const IWString* class_names, IWString_and_File_Descriptor& output) {
  int nclasses = bayesian_classification.nclasses();

  double* c = new double[nclasses];
  std::unique_ptr<double[]> free_c(c);
...
static int
gfp_naive_bayesian(const char* fname, Bayesian_Classification& bayesian_classification,
                   const IWString* class_names, IWString_and_File_Descriptor& output) {
  iwstring_data_source input(fname);
...
static void
write_header(const IWString activity_name, const int nclasses,
             IWString_and_File_Descriptor& output) {
...
class NB_Cross_Validation {
 ...
 public:
  ...
  void
  set_extra_files_are_test_sets(bool s) {
    _extra_files_are_test_sets = s;
  }
  ...
  void
  set_activity_name(const IWString& s) {
    _activity_name = s;
  }
...
NB_Cross_Validation::report_performance_measures(std::ostream& os) {
  ...
  os << "Trained on " << ntrain << " of " << _nfingerprints << " fingerprints, test "
     << (_nfingerprints - ntrain) << " (";
  for (auto i = 0; i < _nclasses; ++i) {
    if (i > 0) {
      os << ',';
    }
...
NB_Cross_Validation::report_overall_performance_measures(std::ostream& os) {
  os << "Performed " << _accuracy_accumulator.n()
     << " cross validations. Accuracy between " << _accuracy_accumulator.minval()
     << " and " << _accuracy_accumulator.maxval() << " ave "
     << static_cast<float>(_accuracy_accumulator.average()) << endl;
...
NB_Cross_Validation::initialise_cross_validation(const const_IWSubstring& c) {
  ...
  if (!c.split(c1, '/', c2) || 0 == c1.length() || 0 == c2.length()) {
    cerr << "NB_Cross_Validation::initialise_cross_validation:cross validation must be "
            "of the form N/P, '"
         << c << "' invalid\n";
    return 0;
  }
  ...
  if (!c1.numeric_value(_nsplit) || _nsplit < 2) {
    cerr << "NB_Cross_Validation::initialise_cross_validation:invalid number of splits '"
         << c << "'\n";
    return 0;
  }
  ...
  if (!c2.numeric_value(p) || p < 1 || p >= 100) {
    cerr << "NB_Cross_Validation::initialise_cross_validation:invalid training set "
            "percent '"
         << c << "'\n";
    return 0;
  }
...
NB_Cross_Validation::_assign_train_test(iwstring_data_source& input) {
  ...
  if (_extra_files_are_test_sets) {
    std::fill(_training_set, _training_set + _nfingerprints,
              1);  // all are excluded from training set to start
    s = 0;
  } else {
    std::fill(_training_set, _training_set + _nfingerprints,
              0);  // nothing in training set
    s = 1;
  }
...
gfp_naive_bayesian(int argc, char** argv) {
  ...
  if (cl.option_present('D')) {
    ...
    if (verbose && bits_to_examine.size() > 0) {
      cerr << "Will write detailed information on " << bits_to_examine.size()
           << " bits\n";
    }
  }
#endif
  ...
  */
  ...
  if (cl.option_present('e')) {
    ...
    if (verbose) {
      cerr << "Will adjust bit numbers of count is > 1, dividing by "
           << distinguish_bits_by_count << endl;
    }
  }
  ...
  if (cl.option_present('B')) {
    ...
    if (verbose) {
      cerr << "Will only write class predictions if the value is >= "
           << all_scores_above_threshold << endl;
    }
  }
  ...
  if (cl.option_present('p')) {
    ...
    if (verbose) {
      cerr << "Will drop bits present in " << min_support_level
           << " molecules or fewer\n";
    }
  }
  ...
  if (cl.option_present('U'))
    ...
    if (1 != cl.number_elements()) {
      cerr << "When training set identifiers come via -R option, can be only one "
              "argument\n";
      usage(1);
    }

    if (!cl.option_present('S')) {
      cerr << "If doing multiple training set splits, must specify an output file name "
              "stem via the -S option\n";
      usage(1);
    }
  ...
  if (cl.option_present('U')) {
    ...
    if (!firfm.check_fingerprint_validity()) {
      ...
      return 1;
             ...
             !cl.option_present('f'))  // train and test gfp on command line
  {
    Bayesian_Classification bayesian_classification;
  ...
  } else  // one or more cross validation runs within one fingerprint file
  {
    ...
    if (!cl.option_present('f')) {
      cerr << "Must specify whether the extra files are TRAIN or TEST set identifiers "
              "(-f)\n";
      usage(2);
    } else {
