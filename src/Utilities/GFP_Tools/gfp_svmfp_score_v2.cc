...
class SvmModel {
  private:
    ...
    // A linear correction generated by rescoring the training set.
...
SvmModel::Initialise(const GfpModel::SvmfpModel& model_proto,
  ...
  // The linear scaling function is currently implemented differently.
  // If the file is present in the model directory, we use it.
  // Might be cleaner to have that info embedded in the proto...

  IWString tmp = dir;
  tmp << "/output_scaling.dat";
  if (fs::exists(tmp.AsString())) {
    std::optional<linear_scaling::LinearScalingData> maybe_proto =
        iwmisc::ReadBinaryProto<linear_scaling::LinearScalingData>(tmp);
    if (! maybe_proto) {
      cerr << "SvmModel::Initialise:cannot read linear scaling proto '" << tmp << "'\n";
      return 0;
    }

    if (! _linear_scaling_fn.Build(*maybe_proto)) {
...
SvmModel::Score(IW_General_Fingerprint& gfp) {
  ...
  _linear_scaling_fn.MaybeTransform(result);
