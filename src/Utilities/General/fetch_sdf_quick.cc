...
class Identifier_Info_String {
 ...
 public:
  ...
  int
  contains(const IWString& s) const {
    return _identifier_to_fetch.contains(s);
  }

  int
  size() const {
    return _identifier_to_fetch.size();
  }
...
int
Identifier_Info_String::initialise(const IWString& s1, const IWString& s2) {
...
int
Identifier_Info_String::write_unwritten_items(
    IWString_and_File_Descriptor& output) const {
  if (verbose) {
    cerr << _identifier_to_fetch.size() << " identifiers not in sd file\n";
  }

  for (IW_STL_Hash_Map_String::const_iterator i = _identifier_to_fetch.begin();
       i != _identifier_to_fetch.end(); i++) {
...
class Identifier_Info_Count {
 ...
 public:
  ...
  const IWString&
  data_for_id(const IWString& id) const {
    return unused_srring;
  }  // is never called

  int
  add(const IWString& s) {
    _identifier_to_fetch[s] = 1;
    return _identifier_to_fetch.size();
  }
};
...
Identifier_Info_Count::write_unwritten_items(IWString_and_File_Descriptor& output) const {
  ...
  for (IW_STL_Hash_Map_int::const_iterator i = _identifier_to_fetch.begin();
       i != _identifier_to_fetch.end(); i++) {
...
template <typename T>
int
read_identifiers_to_fetch(iwstring_data_source& input, T& identifiers_to_fetch) {
...
template <typename T>
int
read_identifiers_to_fetch(const char* fname, T& identifiers_to_fetch) {
...
static int
insert_identifier_file_info(IWString& sd, const IWString& tag, const IWString& zinfo) {
  if (0 == zinfo.length()) {
    return 1;
  }
...
static int
read_next_sd(iwstring_data_source& input, IWString& sd, IWString& id, int& nostruct) {
...
int
fetch_sdf_quick_rx(IWString& sd, const IWString& id, int nostruct,
...
template <typename T>
int
fetch_sdf_quick(IWString& sd, const IWString& id, int nostruct, T& identifiers_to_fetch,
                IWString_and_File_Descriptor& output) {
  int to_be_output = 0;
  ...
  if (suppress_normal_output)
    ...
    if (tag_for_identifier_file_info.length() > 0) {
      insert_identifier_file_info(sd, tag_for_identifier_file_info,
                                  identifiers_to_fetch.data_for_id(id));
    }
...
int
fetch_sdf_quick_rx(iwstring_data_source& input, Fetch_via_Regexp& fetch_via_regexp,
                   IWString_and_File_Descriptor& output) {
  input.set_dos(1);
  input.set_strip_trailing_blanks(1);
...
template <typename T>
int
fetch_sdf_quick(iwstring_data_source& input, T& identifiers_to_fetch,
                IWString_and_File_Descriptor& output) {
  input.set_dos(1);
  input.set_strip_trailing_blanks(1);
...
template <typename T>
int
fetch_sdf_quick(const char* fname, T& identifiers_to_fetch,
                IWString_and_File_Descriptor& output) {
  iwstring_data_source input(fname);
...
static int
fetch_sdf_quick_rx(const char* fname, Fetch_via_Regexp& fetch_via_regexp,
                   IWString_and_File_Descriptor& output) {
  iwstring_data_source input(fname);
...
summary_after_fetching(std::ostream& output) {
  if (verbose) {
    output << sdfile_records_read << " sd file records read, " << items_written
           << " items written\n";
...
static int
fetch_sdf_quick_rx(const Command_Line& cl, Fetch_via_Regexp& fetch_via_regexp,
                   IWString_and_File_Descriptor& output) {
...
template <typename T>
int
fetch_sdf_quick(const Command_Line& cl, T& identifiers_to_fetch,
                IWString_and_File_Descriptor& output) {
  if (cl.option_present('X')) {
    const char* fname = cl.option_value('X');
...
fetch_sdf_quick(int argc, char** argv) {
  ...
  if (cl.option_present('T')) {
    ...
    if (verbose) {
      cerr << "Will put '" << tag_for_identifier_file_info
  ...
  if (cl.option_present('g')) {
    ...
    if (verbose) {
      cerr << "Will ignore possibly invalid sdf records with missing or empty "
              "identifier\n";
    }
  }
  ...
  if (cl.option_present('e')) {
    ...
    if (!cl.option_present('a')) {
      cerr << "The -a option not specified, only first conformation of each molecule "
              "written\n";
    }
  }
