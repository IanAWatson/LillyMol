...
class Column_Specification {
 ...
 public:
  ...
  int
  column_described_by_name() const {
    return _column_names.number_elements();
  }

  int identify_column_numbers(int* process_column, const int ncols, const int ndx) const;
  int identify_column_names(const resizable_array_p<IWString>& header,
                            int* process_column, const int ncols, const int ndx) const;
...
/*
  For each -e and -E, we build a column specification object. they can then examine a
  header and determine the columns
*/
...
class Set_of_Column_Match_Specification {
 private:
  ...
  int _write_selected_columns_header(const IWString& headr,
  ...
  int _process_record(const const_IWSubstring& line,
...
static int
build_column_specifications(
    const Command_Line& cl, const char flag,
  ...
  for (int i = 0; cl.value(flag, e, i); ++i) {
    ...
    if (!e.split(a, ':', b) || 0 == a.length()) {
      cerr << "Set_of_Column_Match_Specification::build:invalid column/contents "
              "specification '"
           << e << "' must be of the form 'col:contents'\n";
      return 0;
    }
    ...
    if (!t->build(a, b)) {
      cerr << "Set_of_Column_Match_Specification::build:invalid column specification '"
           << e << "'\n";
      delete t;
      return 0;
    }
...
int
Set_of_Column_Match_Specification::build(const Command_Line& cl, const char flag) {
...
Set_of_Column_Match_Specification::build_extra_columns(const Command_Line& cl,
  ...
  for (int i = 0; cl.value(flag, s, i); ++i) {
    ...
    if (!t->build(s)) {
      delete t;
      cerr << "Set_of_Column_Match_Specification::build_extra_columns:invalid column "
              "specifier '"
           << s << "'\n";
      return 0;
    }
...
Set_of_Column_Match_Specification::process(iwstring_data_source& input,
  ...
  if (!_identify_columns_to_process(first_record)) {
    cerr << "Set_of_Column_Match_Specification::build:cannot identify columns to "
            "process\n";
    cerr << first_record << "\n";
    return 0;
  }
  ...
  if (_extra_columns_to_write.number_elements()) {
    if (!_identify_columns_to_write(first_record)) {
      cerr << "Set_of_Column_Match_Specification::process:cannot discern extra columns "
              "to write\n";
      return 0;
    }
  }
...
int
Set_of_Column_Match_Specification::_write_selected_columns_header(
    const IWString& header, IWString_and_File_Descriptor& output) const {
  int i = 0;
  const_IWSubstring token;

  int need_separator = 0;

  for (int c = 0; header.NextWord(token, i, input_separator) && c < _ncols; ++c) {
...
Column_Match_Specification::matches(const const_IWSubstring& s) {
  // cerr << "Column_Match_Specification::matches what about " << s << " contents " <<
  // _column_contents << "'\n";
  if (_is_regexp) {
...
Column_Match_Specification::set_is_regexp(const int s) {
  ...
  if (!iwre2::RE2Reset(_rx, _column_contents)) {
    cerr << "Column_Match_Specification::set_is_regexp:invalid regexp "
         << _column_contents << "'\n";
    return 0;
  }
...
Column_Specification::identify_column_numbers(int* process_column, const int ncols,
  ...
  for (int i = 0; i < _column_numbers.number_elements(); ++i) {
    ...
    if (c >= ncols) {
      cerr << "Column_Match_Specification::identify_numeric_column_numbers:out of range "
           << c << " max is " << ncols << endl;
      return 0;
    }
...
Column_Specification::identify_column_names(const resizable_array_p<IWString>& header,
                                            int* process_column, const int ncols,
                                            const int ndx) const {
  for (int c = 0; c < _column_names.number_elements(); ++c) {
    ...
    if (!found_match) {
      cerr << "Column_Match_Specification::identify_column_names:no match for " << si
           << endl;
      if (verbose) {
...
static void
tokenise(const IWString& s, const char input_separator, resizable_array_p<IWString>& f) {
  int i = 0;
  const_IWSubstring token;
  for (int col = 0; s.NextWord(token, i, input_separator); ++col) {
    f.add(new IWString(token));
  }
}

int
Set_of_Column_Match_Specification::_identify_columns_to_process(
    const IWString& first_record) {
  int need_to_check_names = 0;

  // cerr << "Checking " << _column_specifications.number_elements() << " column
  // specifications\n";
  ...
  for (int i = 0; i < _column_specifications.number_elements(); ++i) {
    if (!_column_specifications[i]->identify_column_numbers(_process_column, _ncols, i)) {
      cerr << "Set_of_Column_Match_Specification::_identify_columns_to_process:specifier "
           << i << " failed numeric identification, ncols " << _ncols << endl;
      return 0;
    }
  ...
  for (int i = 0; i < _column_specifications.number_elements(); ++i) {
    if (!_column_specifications[i]->identify_column_names(header, _process_column, _ncols,
                                                          i)) {
      return 0;
    }
  }
...
int
Set_of_Column_Match_Specification::_identify_columns_to_write(
    const IWString& first_record) {
  int need_to_check_names = 0;
  ...
  for (int i = 0; i < _ncols; ++i) {
    ...
    if (_process_column[i] >= 0) {
      cerr << "Set_of_Column_Match_Specification::_identify_columns_to_write:column " << i
           << " set for searching\n";
      continue;
    }
...
static void
append_to_buffer(const const_IWSubstring& token, bool& need_separator, IWString& buffer) {
  if (!need_separator) {
    need_separator = true;
  } else {
...
Set_of_Column_Match_Specification::_process(iwstring_data_source& input,
  ...
  while (input.next_record(buffer)) {
    ...
    if (!_process_record(buffer, output)) {
      cerr << "Set_of_Column_Match_Specification::_process_record:fatal error "
              "processing\n";
      cerr << buffer << endl;
      return 0;
    }
...
Set_of_Column_Match_Specification::_process_record(const const_IWSubstring& line,
  ...
  int i = 0;
  const_IWSubstring token;
  for (int c = 0; line.NextWord(token, i, input_separator) && c < _ncols; ++c) {
    if ((0 == c && first_column_contains_identifiers && -1 == _process_column[0]) ||
        (SPECIAL_VALUE_ALWAYS_WRITE == _process_column[c])) {
      if (just_matched_columns) {
...
static int
grep_by_column(const char* fname, Set_of_Column_Match_Specification& socs,
               IWString_and_File_Descriptor& output) {
...
grep_by_column(int argc, char** argv) {
  ...
  if (cl.option_present('m')) {
    ...
    if (verbose) {
      cerr << "Will write a record if at least " << min_matches_needed
           << " conditions matched\n";
    }
  }
  ...
  if (cl.option_present('o')) {
    ...
    if (!char_name_to_char(tmp)) {
      cerr << "The output delimiter must be a single character, '" << tmp
           << "' is invalid\n";
      usage(4);
    }
