...
static int
determine_columns_to_process(int columns_in_input, const IWString* column_name,
...
void
compute_cutoffs(Accumulator<double>& acc, double sigma, double& lower_cutoff,
                double& upper_cutoff) {
...
void
compute_cutoffs(Median_Determination<double>& md, double sigma, double& lower_cutoff,
                double& upper_cutoff) {
...
void
write_central_value(Accumulator<double>& acc, std::ostream& os) {
  os << " ave " << acc.average();
}

void
write_central_value(Median_Determination<double>& md, std::ostream& os) {
  os << " median " << md.median();
}

static int
do_only_write_selected_columns_to_discard_stream(
    const const_IWSubstring& buffer, IWString_and_File_Descriptor& stream_for_discards) {
  int i = 0;
  int col = 0;
...
static int
rmsigma(const const_IWSubstring& buffer, const int columns_in_input) {
  ...
  for (int i = 0; i < columns_in_input; i++) {
    ...
    if (!buffer.NextWord(token, j, input_separator)) {
    ...
    if (missing_value == token) {
      ...
      if (skip_records_with_missing_values >= 0 &&
          missing_values_this_row >= skip_records_with_missing_values) {
    ...
    if (!token.numeric_value(
            d))  // should never be a problem, this is 2nd time we read the file
    {
      cerr << "Invalid numeric token '" << token << "'\n";
      return 0;
    }

    //  cerr << "Value " << d << " in column " << i << ", lc " << lower_cutoff[i] << " uc
    //  " << upper_cutoff[i] << endl;
...
template <typename T>
int
rmsigma(iwstring_data_source& input, const int columns_in_input, T* acc,
        IWString_and_File_Descriptor& output) {
  const_IWSubstring buffer;
  while (input.next_record(buffer)) {
    int nw = buffer.nwords();
    if (columns_in_input != nw) {
      cerr << "Column count mismatch, " << columns_in_input << " vs " << nw << " line "
           << input.lines_read() << '\n';
      return 2;
    }
    ...
    for (int i = 0; i < nw; i++) {
      ...
      if (!buffer.NextWord(tmp, j, input_separator)) {
        cerr << "Cannot extract word " << i << " on line " << input.lines_read() << '\n';
        return 3;
      }
      ...
      if (!tmp.numeric_value(d)) {
        cerr << "Non numeric value found '" << tmp << "' line " << input.lines_read()
             << '\n';
        return 4;
      }
...
template <typename T>
int
rmsigma(iwstring_data_source& input, int columns_in_input,
        IWString_and_File_Descriptor& output) {
  T* acc = new T[columns_in_input];
  std::unique_ptr<T[]> free_acc(acc);
...
static int
rmsigma(iwstring_data_source& input, IWString_and_File_Descriptor& output) {
  const_IWSubstring buffer;
  if (!input.next_record(buffer)) {
  ...
  for (int i = 0; i < columns_in_input; i++) {
    if (ignore_column[i] && process_column[i]) {
      cerr << "Inconsistent specifications for column " << (i + 1) << " ignore "
           << ignore_column[i] << " process " << process_column[i] << '\n';
      return 0;
    }
  ...
  if (verbose > 2) {
    for (int i = 0; i < columns_in_input; i++) {
      cerr << " col " << i << " ignore " << ignore_column[i] << " proc "
           << process_column[i] << '\n';
    }
  }
  ...
  if (verbose) {
    ...
    for (int i = 0; i < columns_in_input; i++) {
      ...
      if (0 == records_discarded_for_being_too_high[i] &&
          0 == records_discarded_for_being_too_low[i] &&
          0 == down_column_missing_value_count[i]) {
        continue;
      }
    ...
    for (int i = 0; i < across_row_missing_value_count.number_elements(); i++) {
      if (across_row_missing_value_count[i]) {
        cerr << across_row_missing_value_count[i] << " rows had " << i
             << " missing values\n";
      }
    }
  }
...
static int
rmsigma(const char* fname, IWString_and_File_Descriptor& output) {
...
// clang-format off
  ...
  cerr << " -z <column>    specify columns to ignore\n";
...
rmsigma(int argc, char** argv) {
  ...
  if (cl.option_present('z')) {
    if (cl.option_present('c')) {
      cerr << "The -z and -c options are mutually exclusive\n";
      usage(2);
    }

    IWString tmp;
    int i = 0;
    while (cl.value('z', tmp, i++)) {
      int j;
      if (!tmp.numeric_value(j) || j < 1) {
        cerr << "Column specifiers (-z option) must be whole non-negative numbers\n";
        usage(3);
      }
  ...
  if (cl.option_present('c')) {
    if (cl.option_present('z')) {
      cerr << "The -c and -z options are mutually exclusive\n";
      usage(2);
    }
  ...
  if (cl.option_present('i')) {
    IWString token = cl.string_value('i');
    if (! char_name_to_char(token)) {
      cerr << "Invalid input file specification '" << token << "'\n";
      input_separator = token[0];
    }
  }
