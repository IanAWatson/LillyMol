...
def _add_nanobind_runfiles_to_path() -> None:
  candidates = ['nanobind']
  for envvar in ('TEST_SRCDIR', 'RUNFILES_DIR'):
    root = os.environ.get(envvar)
    if root:
      candidates.extend([
          os.path.join(root, '_main/nanobind'),
          os.path.join(root, 'nanobind'),
      ])
...
_add_nanobind_runfiles_to_path()
...
class GfpNearNeighbourServerNanobindTest(unittest.TestCase):

  def setUp(self):
    self.server = lillymol_gfp_server.GfpNearNeighbourServer(_testdata_path())
    self.query = {
  ...
  def test_pool_size(self):
    self.assertEqual(self.server.pool_size(), 10)

  def test_search_proto(self):
    request = nn_request_pb2.NnRequest(**self.query)
    reply = nn_request_pb2.NnReply()
    reply.ParseFromString(self.server.search_proto(request.SerializeToString()))
  ...
  def test_search_batch_proto(self):
    ...
    reply.ParseFromString(self.server.search_batch_proto(request.SerializeToString()))
    self.assertEqual([r.status for r in reply.reply], [nn_request_pb2.OK, nn_request_pb2.BAD_SMILES])
