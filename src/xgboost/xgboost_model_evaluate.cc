...
class Options {
  ...
  public:
    ...
    int WriteRegressionResult(float value, IWString_and_File_Descriptor& output);
    int WriteClassificationResult(float value, const IWString* class_labels, IWString_and_File_Descriptor& output);
...
int
Options::WriteRegressionResult(float value,
...
int
Options::WriteClassificationResult(float value,
...
class XGBDModel {
  private:
    ...
    class_label_translation::ClassLabelTranslation _class_label_translation;

    // If this is a classification model.
    IWString* _class_labels;
  ...
  int ReadClassLabelTranslation(IWString& fname);
  ...
  public:
    ...
    int classification() const {
      return _proto.classification();
    }

    const IWString* class_labels() const {
...
int
XGBDModel::ReadClassLabelTranslation(IWString& fname) {
  std::optional<class_label_translation::ClassLabelTranslation> proto =
    iwmisc::ReadTextProto<class_label_translation::ClassLabelTranslation>(fname);
  if (! proto) {
    cerr << "XGBDModel::ReadClassLabelTranslation:cannot read '" << fname << "'\n";
    return 0;
  }

  _class_label_translation = std::move(*proto);

  if (_class_label_translation.to_numeric().size() != 2) {
    cerr << "XGBDModel::ReadClassLabelTranslation:more than two classes\n";
    cerr << _class_label_translation.ShortDebugString() << '\n';
    return 0;
  }

  // Should check that there are 2 classes in the proto...
  _class_labels = new IWString[2];

  for (const auto& [k, v] : _class_label_translation.to_numeric()) {
    if (v == 0) {
      _class_labels[0] = k;
    } else if (v == 1) {
      _class_labels[1] = k;
...
ScoreAndWrite(XGBDModel& model,
  ...
  if (model.classification()) {
    for (int i = 0; i < items_in_chunk; ++i) {
      output << arrays.id[i];
      options.WriteClassificationResult(arrays.pred[i], model.class_labels(), output);
    ...
    for (int i = 0; i < items_in_chunk; ++i) {
      output << arrays.id[i];
      options.WriteRegressionResult(arrays.pred[i], output);
...
XGBDModelEvaluate(iwstring_data_source& input,
  ...
  while (input.next_record(buffer)) {
    ...
    if (model.classification()) {
      options.WriteClassificationResult(*v, model.class_labels(), output);
    } else {
      options.WriteRegressionResult(*v, output);
    }
  }
